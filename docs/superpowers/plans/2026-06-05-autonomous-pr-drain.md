...
function Invoke-HprCommand {
  ...
  # Use Set-Content with explicit UTF-8 encoding to avoid UTF-16/BOM artifacts from *> redirection.
  $output = & $Command 2>&1 | Out-String
  Set-Content -Encoding utf8NoBOM -Path $EvidencePath -Value $output
...
foreach ($number in $openPrs) {
  ...
  # Verify a final state is present (not still initialised as 'pending').
  ...
  # Verify the row is not still in the initial 'all-pending' state (every disposition field must be filled).
  # Extract the ledger row for this PR number and count 'pending' occurrences across its columns.
  $rowPattern = "\|[^\n]*#?$number[^\n]*\n"
  if ($ledger -match $rowPattern) {
    $row = $Matches[0]
    $pendingCount = ([regex]::Matches($row, '\bpending\b')).Count
    # A row is considered uninspected when Salvaged content, Review comments handled,
    # Validation evidence, and Close or merge note are still all 'pending'.
    if ($pendingCount -ge 4) { throw "PR #$number ledger row appears uninspected — $pendingCount fields still show 'pending'" }
  }
}
```
...
foreach ($prSummary in $openPrs) {
  ...
  # Also verify no individual thread has truncated comment pages before disposition.
  foreach ($thread in $threadObject.data.repository.pullRequest.reviewThreads.nodes) {
    if ($thread.comments.pageInfo.hasNextPage) { throw "PR #$number thread at $($thread.path) has paginated comments; fetch remaining pages before disposition" }
  }
  ...
  # Fetch the base ref if it is not already present locally (handles non-main bases such as stacked PRs).
  git show-ref --verify --quiet "refs/remotes/$baseRef"
  if ($LASTEXITCODE -ne 0) { git fetch origin $detailObject.baseRefName }
...
```powershell
# Run all baseline commands and capture every exit code, even on failure.
# Do NOT use Invoke-HprCommand here — baseline recording must never abort early.
$baselineCommands = @(
  @{ Path = 'docs/evidence/2026-06-05/hpr-baseline-npm-ci.txt';   Script = { npm.cmd ci } },
  @{ Path = 'docs/evidence/2026-06-05/hpr-baseline-check.txt';    Script = { npm.cmd run check } },
  @{ Path = 'docs/evidence/2026-06-05/hpr-baseline-lint.txt';     Script = { npm.cmd run lint } },
  @{ Path = 'docs/evidence/2026-06-05/hpr-baseline-test.txt';     Script = { npm.cmd test } },
  @{ Path = 'docs/evidence/2026-06-05/hpr-baseline-build.txt';    Script = { npm.cmd run build } }
)
$exitCodes = @{}
foreach ($cmd in $baselineCommands) {
  $output = & $cmd.Script 2>&1 | Out-String
  $exitCodes[$cmd.Path] = if ($LASTEXITCODE -ne $null) { $LASTEXITCODE } else { 0 }
  New-Item -ItemType Directory -Force (Split-Path -Parent $cmd.Path) | Out-Null
  Set-Content -Encoding utf8NoBOM -Path $cmd.Path -Value $output
}
$exitCodes | ConvertTo-Json | Set-Content -Encoding utf8NoBOM -Path 'docs/evidence/2026-06-05/hpr-baseline-exit-codes.txt'
```

Expected: all pass, or failures are recorded in the baseline section with links to the exact evidence files. All five commands must run regardless of individual failures — never abort early during baseline capture.
...
```powershell
# Stage the ledger, live-state snapshot, all baseline evidence files, and plan.md.
git add docs/hpr-pr-resolution-ledger-2026-06-05.md docs/evidence/2026-06-05/hpr-pr-live-state docs/evidence/2026-06-05/hpr-baseline-*.txt docs/evidence/2026-06-05/hpr-baseline-exit-codes.txt plan.md
