...
curl -sS -X POST https://meechiescoloringbook.vercel.app/api/image-generation \
  ...
  -d @docs/evidence/2026-08-25/prompt-boundary-live-request.json
```
...
| File | What it is |
|---|---|
| `prompt-boundary-live-request.json` | A schema-valid replay envelope reconstructed from the exact retained prompt and matching visible spec |
...
The original full transport envelope was not retained, so the replay JSON is not represented as
a byte-for-byte historical request. The prompt, response summary, and JPEG were first committed
together in `4c29e0f404ca33e36557bf25ee8931b8138e758f` at `2026-08-25T02:23:53Z`.
The response records the JPEG's Git blob SHA and its 270880-character base64 payload length;
those values link the inspected repository artifact to the summarized response. This is
traceable visual evidence, not a signed provider receipt.
