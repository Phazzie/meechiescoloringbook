...
| 7 | Ghost workflow: legacy generation workflow path not on active UI path | Deferred with documentation | `src/lib/core/generation-workflow.ts`, `tests/unit/generation-workflow.test.ts`, `docs/seams.md` | Decide whether to retire the legacy workflow or rewire it to the active app path in a dedicated migration. |
