...
## Unreleased
- Added a sliding-window `RateLimitSeam` and request-budget guard on the five AI-provider-backed routes (`/api/generate`, `/api/image-generation`, `/api/chat-interpretation`, `/api/meechie-studio-text`, `/api/wig-try-on`) to return `429` responses with a `Retry-After` header once a client exceeds its per-route budget.
