...
## Unreleased
- Coming back to the studio now brings back the question you were answering, not just your answer. It has always saved your evidence as you type and put it back on the next visit — but it never saved which of the eight modes you were in, so it dropped your words into whichever one the studio opens on. You would come back to an excuse you had typed for Rate His Excuse sitting under Who Fucked Up?, and the next Generate Verdict spent a real AI call reading it as that instead.
- The studio now says what it brought back, which it never used to say at all. A line above the evidence box names the question your work is under and when it was saved, and it closes when you dismiss it, pick a mode, or generate.
- If the studio cannot be sure which question your work belongs to, it tells you instead of guessing quietly. There are two ways that happens and they get different wording: a draft saved before this change recorded no question at all, and a draft whose question has since been removed from the studio. Either way your evidence still comes back and the line asks you to check the question before you generate.

- Every one of the eight modes is on the home page now. It used to show three, picked by the calendar, and link only those three — so on any given day five of them had no link on the home page or in the menu bar, and the only places in the app that listed them were the page you land on when you lose your connection and the one you land on when an address is wrong. On the day this was changed the three on offer were the same three already in the menu bar at the top of every page, so the strip was offering nothing new at all.
- The rotation is still there, but it now highlights rather than hides: one mode is spotlighted for the month and two for the week, and a line under the strip says so and names the date the set next changes.
- The highlight is now right on the day you are reading it. It was worked out once when the site was built and could never change afterwards, so an installed copy of the app opened on its front door showing whichever three modes were current on build day, with a badge asserting which month it was. Leaving the app open across a Monday or the first of a month now moves the spotlight on without a reload.
- The month and the week are both counted in UTC. They used to be counted differently from each other, so for the first hours of every month a reader west of UTC was served one set and then watched it change under them a moment later.
- The mode you pick now decides which of Meechie's tools reads your evidence, on any of the eight. Picking one outside the featured three was not possible before; the studio would have quietly answered as a different mode.
- Meechie now tells you whether she could actually rule on what you gave her. She has been sending that back on every single answer — "enough to work with", "need more tea", or "can't work with this" — and the studio showed none of it, so an answer she had refused to make looked exactly like a finished verdict. When she flags one, the card says so and offers to take you back to the evidence box, and the "Create Coloring Page" button carries the same warning before you spend a generation on it.
- Her note on what would sharpen the verdict is on screen for the first time. It has been coming back with every answer you have ever paid for and going nowhere, while the three rewrite buttons left you guessing at exactly what it says.
- The severity number says what it is. It used to read as a bare "8/10" next to the verdict, which looks like a mark out of ten for Meechie's answer; it is her read on how bad the situation is, and it now says so. An answer she gave no number to shows nothing rather than a blank score.
- A page reopened from the vault, or brought back after a refresh, shows her ruling again when the record can prove she made it — so a verdict she could not rule on still carries its warning after you reload. A page saved from the toolkit, or saved before the studio recorded any of this, shows its words and its note as before but no ruling, because nothing about it can show she gave one.
- A Rate This Excuse score saved to the vault is no longer relabelled as severity when you reopen it in the studio. The two numbers are both out of ten and mean close to opposite things — 1 on an excuse means it insulted everyone in the room, 1 on a situation means it barely registers — and the score was being copied from one into the other. The score itself is untouched: it is the headline of that page, where it always was.
...
- A page you save is now filed under the settings it was actually drawn from. Making a page, changing a setting and then saving used to store the new settings alongside the old picture — so a page made on US Letter and saved on A4 reported dimensions that never produced it, and a page made under a busy theme and saved under a plain one was recorded as both at once.
...
- A draft you come back to after a refresh now saves the style it is currently wearing. Changing a theme or a voice setting on a restored draft and then saving used to store the draft's older style beside the newly styled page.
- A generation that comes back without a picture no longer leaves anything behind for a later save to file as a page. The trace still shows what was asked for.
- The downloads for a page are now made for the paper it was actually drawn on. Changing Page Size while a page was still being made produced a PDF and a share image sized for the new setting, beside a picture made for the old one. The download row names that paper too, so a US Letter page is no longer offered as "A4 — ready to print".
- Coming back to a draft after a refresh no longer makes the next page with a wig you cannot see. A draft saved while a wig was selected restored that wig invisibly, and the next page you made was styled with it.
