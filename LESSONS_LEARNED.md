...
## 2026-07-06 (3) — a per-field length cap doesn't bound a combined string built from those fields
- Date: 2026-07-06
- Context: Codex reviewed PR #220 and found that `ImageGenerationSeam`'s validator caps `prompt` and `negativePrompt` independently at `MAX_PROMPT_LENGTH` (8000 each), but the adapter's `buildPrompt` concatenates them into one string (`${prompt}\n\nNegative prompt: ${negativePrompt}`) before sending it to xAI — so two near-max-length fields together produce a payload roughly double the intended provider limit.
- Lesson: A cap on each of two fields doesn't imply a cap on their concatenation unless something checks the combined length specifically. This is easy to miss when the two fields are validated in one schema but combined later, in a different file (the adapter), by a separate function the validator has no visibility into.
- Action: When a validator's job is to bound what an adapter actually sends over the wire, and the adapter builds that payload by combining more than one validated field, add a combined-length check (or share a single constant/helper between the validator and the adapter) rather than trusting that per-field caps compose correctly.

## 2026-07-07 — a config preflight can still charge quota if it only covers part of what the real seam call needs
- Date: 2026-07-07
- Context: Auditing PR #220's own 11 fresh review threads found that `checkWigTryOnConfig` (added 2026-07-06 (2) to catch a missing `GEMINI_API_KEY` before charging rate-limit quota) only ever checked that one raw key — but `createWigTryOnSeam`'s lazy `configSeam.getConfig()` also validates the entire `AppConfigSeam` schema (every `XAI_*` var too), and that call happens deep inside the pipeline, after quota was already charged. A narrow preflight fixed one gap (missing Gemini key) while leaving a structurally identical gap open one layer down (missing/invalid xAI vars) for the same route. The same class of bug independently applied to the image/generate routes: the circuit breaker's open/closed state was only checked inside the seam's `generate()` call, after `enforceAiRateLimit` had already run, so a client hitting a fail-fast 503 during an outage could still exhaust its own quota for zero paid work.
- Lesson: "Preflight before charging quota" only closes the gap for the specific failure mode the preflight checks. If the real work behind the rate limiter can still fail for a *different*, cheap-to-check reason (a broader config schema, a breaker that's already open), that failure mode burns quota exactly the same way the original bug did, just via a different door.
- Action: When adding a preflight to avoid charging quota for doomed work, enumerate every cheap, synchronous way the real call can still fail after the preflight passes — not just the one failure mode a specific review comment named — and check all of them before the rate limiter runs.
