...
## 2026-09-04
- Date: 2026-09-04
- Context: A reviewer pointed out that PR #295 newly lets `/m/<slug>` reach `/api/generate`, which constructs `ImageGenerationSeam` — a seam `assumption-alarm.json` lists under `blockedSeams` for stale fixtures — and that the only waiver on record was scoped to a review-comment repair in May.
- Lesson: A fixture waiver is scoped to the change that wrote it. Reading an older waiver as covering a later change is the same borrowed-authority error as citing an unrelated Assumption for an unrelated endpoint, which this run had already made once with `WigTryOnSeam`. Both times the waiver or Assumption existed and was real; both times it was about something else, and the resemblance was doing the work that evidence should have done.
- Action: When a change newly reaches a real-world seam, **check that seam's `Last probe` date in `docs/seams.md` — do not rely on `assumption-alarm.json`'s `blockedSeams`.** That list is derived from `TBD (blocked)` markers, not from age, so a seven-month-old probe date passes it exactly as a seven-day-old one does. **And the seam-level date is not sufficient either — check the fixtures themselves.** `probes/browser-seams.probe.mjs:200-202` writes only `sample.json` for `SessionSeam`, `AuthContextSeam` and `CreationStoreSeam`; it never writes a `fault.json` for any of them. So running that probe and advancing `Last probe` makes all three read as fresh while their fault fixtures are exactly as old as before. A seam-level date answers "when did some probe last run", not "is every scenario's fixture current", and the seven-day gate is about the second. This run learned that the hard way: it audited only the seam `blockedSeams` named (`ImageGenerationSeam`) and missed `SessionSeam` and `CreationStoreSeam`, both dated 2026-02-05 and both newly reached by the same change. An earlier version of this Action said to check `blockedSeams`, which would have reproduced exactly that miss. Then: write a waiver scoped to *that* change with its own validation plan, or refresh the fixtures. Never cite a waiver whose Status text names a different piece of work.
