...
## 2026-07-02 (3)
- Date: 2026-07-02
- Context: Codex review on PR #211 flagged two remaining gaps in the RateLimitSeam work: no fault fixture per `src/lib/seams/AGENTS.md`, and rate-limit checks running before request-body parsing (so malformed JSON burned paid-provider quota).
- Lesson: A stateful seam's "fault" fixture doesn't need a scenario-selector mock if a single input can deterministically force the failure path — `{ limit: 0, ... }` triggers RATE_LIMITED on the very first, stateless call, satisfying the red-proof requirement the same way SafetyPolicySeam's unsafe-content fixtures do. Separately, guard ordering matters: a budget meant to protect paid-provider calls should sit after cheap, local validation (JSON parsing) that can never reach a provider, not before it.
- Action: Added `faultConsumeInput` + a dedicated red-proof test to RateLimitSeam, and reordered all six gated routes to call `parseRequestBody` before `enforceRateLimit`.
