...
## 2026-09-12
- Date: 2026-09-12
- Context: The triage analyzer had two guards on its rows. One rejected a row whose width did not match the header; the other rejected a full-width row whose `PR` cell did not name a PR. A review found that `| 348 | title |` — too narrow **and** missing its `#` — passed both: the PR-cell guard only judges full-width rows, and the width guard only inspected rows the parser had already recognised, which requires the cell to parse. The analyzer advanced its provenance line having never measured that pull request, and the validator reported an empty backlog.
- Lesson: **Two guards that each require a different precondition leave the intersection of their blind spots uncovered.** Each check was correct and each had a test. What neither had was a reason to run on the input the other one needed to be well-formed, so a value carrying both faults was invisible to both. Adding a third guard for the combination would be the wrong fix — there are four combinations of two faults and it does not scale.
- Action: Order checks by what they need, cheapest precondition first, and give the one that needs nothing the widest scope. Width needs no parsing, so it now runs over every non-divider row of the table and runs before any cell is read; a PR cell is only parsed once the row is known to be the right shape. When adding a guard, ask which existing guard's precondition it depends on, and check that something still catches input failing both.

## 2026-09-12
- Date: 2026-09-12
- Context: `summarizeConflictPaths` sorted conflicted paths with `localeCompare` and no locale argument, which reads the host's collation. `sv-SE` orders `ä` after `z`; `en-US` orders it before. So the same `git merge-tree` measurement wrote different bytes into `docs/triage-table.md` depending on which machine ran the script — and the byte-identical-on-re-run property I had proved and recorded held only per machine.
- Lesson: **A generated file whose bytes depend on the machine that generated them is not evidence.** The idempotence proof was real and still did not cover this, because I ran it twice on one host: repeating a measurement in the same environment tests the code, not the environment's contribution to the answer. Anything that reads ambient state — locale, timezone, filesystem order, environment variables — is an input, and an input that is not in the file is an input nobody can check.
- Action: In code whose output is committed, use comparators and formatters that take no ambient input: UTF-16 code-unit ordering rather than `localeCompare` (an explicit locale still defers to the ICU version Node was built against), and an explicit UTC date rather than the host's. When proving idempotence, ask what the second run shared with the first that a reader on another machine would not.

## 2026-09-12
- Date: 2026-09-12
- Context: I fixed the `Conflicting paths` cell by writing every filename git named, verbatim, into a fenced block in the same markdown file the analyzer parses. The next review round found three defects, all of them mine and all of them the same: a conflicted filename shaped like `| #999 | not | a | table | row | but | a | filename |` was read as a PR row, because row discovery scanned to end of file; a file named exactly `<!-- conflicting-paths:begin -->` was counted as a second block marker, so a refresh that *succeeded* produced a table the next run refuses to read; and a file named `   ` read as the empty line that terminates git's path list, so the whole measurement was recorded as "git named no files". Each filename is legal on every filesystem this runs on.
- Lesson: **A writer that emits arbitrary content into a file its own reader parses has handed the reader's grammar to whoever names the content.** I had already accepted twice on this same PR that a filename may contain a pipe, and may begin or end with a space — and fixed both *inside a cell*, where the value is escaped. Putting the same values into the document as whole lines moved the problem from escaping to grammar, and nothing in the cell-level fixes transferred. Deciding to store untrusted-shaped data verbatim is a decision about the parser, not about the storage.
- Action: Recognise structure only where structure can legally be, rather than by matching the shape of a line anywhere in the file. Rows come only from the contiguous table (`tableRowEnd`), markers only from outside a fence (`fencedLines`, following CommonMark so a shorter backtick run cannot close a longer fence), and a terminator only from a line with nothing in it. Then prove it with the hostile value round-tripped through the real writer: the test builds the block with `renderConflictDetails` from those exact filenames and asserts the readers see one row, one block and a table that still validates.

## 2026-09-12
- Date: 2026-09-12
- Context: The same round found that my real-file test split the `Conflicting paths` cell on comma-space to check each entry against the complete list behind it. A conflicted file named `a, b.md` would be written correctly into both the cell and the block by production code, and then read by the test as two entries, `a` and `b.md`. `npm test` would fail on a correct refresh — so a legitimate measurement could not be committed.
- Lesson: **A test that parses a value apart inherits every ambiguity the format has; one that regenerates it inherits none.** The reviewer's alternative — build the cell from the block's paths with the same `summarizeConflictPaths` production uses, and compare the whole string — is both simpler and strictly stronger: it asserts the entire cell rather than each entry, and it cannot be confused by a separator appearing inside a value. My version was doing the same parsing job twice, once in production with escaping and once in the test without it.
- Action: When a test checks that a derived value matches its source, derive it again and compare, rather than taking the derived value apart. Where that is impossible, the test must be able to accept every value the production code can legally produce — and the way to find out is to feed it one, which is what the red proof for this now does.

## 2026-09-12
- Date: 2026-09-12
- Context: The triage table's `Conflicting paths` column exists because an earlier generator wrote the literal string `Has conflicts: .` into forty-five rows, naming no files. I fixed that, and then wrote a summariser that collapses a directory contributing several files to `dir/* (N files)` — because one stale PR conflicts on twenty-five paths and a cell holding all of them is unreadable. A review on PR #356 pointed out that `docs/evidence/2026-09-07/* (14 files)` does not name the fourteen, so the table still did not hold the files git named and a reviewer still could not audit a conflict without rerunning the analyzer. The column I had repaired was failing its purpose again, for a different reason, in code I wrote to repair it.
- Lesson: **An abbreviation is allowed; being the only record is not.** Both defects had the same shape — the cell said something about the conflict instead of naming it — and the second was harder to see because the abbreviation is genuinely useful: `dir/* (14 files)` tells a reader the conflict is regenerated evidence rather than a seam contract, which the raw list buries. The mistake was treating readability and completeness as a trade to be made once in one place, when they belong in two places: a summary a reader scans, and a complete record it points at.
- Action: When shortening a value for presentation, write the unshortened one somewhere a reader can reach, from the same computation in the same run — here `refreshRows` carries every path rather than a count, and the block below the table is rewritten whole beside the cells, so the two cannot disagree. Then test the relationship, not the halves: a unit test reads the committed file and fails if any `dir/* (N files)` does not expand to exactly N paths under that directory. The block uses a code fence rather than table cells, because a fence needs no escaping and `\|` is invisible to a reader who copies a path into a shell.

## 2026-09-12
- Date: 2026-09-12
- Context: `DECISIONS.md` has carried the sentence "a rule a parser gains belongs to every reader of the format" since the ninth review round on PR #356, written after I gave `findMalformedRows` to the analyzer and not to `validate-pr-backlog.js`. In the eleventh round I gave the analyzer `findRowsWithBadPrCell` and a duplicate-header check, and again not to the validator. The twelfth round's review found both: a full-width row written `348` vanished from the validator's selection with no reason, and a header carrying `Dry-run` twice let one row answer yes and no at once with the earlier column silently winning.
- Lesson: **Writing a rule down is not a mechanism, and restating it is not either.** I had the rule, in the file I was editing, in an entry about failing to apply it — and failed to apply it twice more. What made it possible was that the shared guard was scoped to one caller: `findDuplicateColumns` hard-coded the analyzer's three columns, so there was no way for the validator to use it even if I had thought to. A guard shared in name but scoped to one caller's needs is not shared.
- Action: When a guard is added for one reader of a format, either give it a parameter for what the caller reads and call it from every reader in the same change, or the change is not finished. Checked here by grepping for the other importers of the parser before considering the fix done — `scripts/validate-pr-backlog.js` now parses nothing itself and imports every guard from the analyzer.

## 2026-09-11
- Date: 2026-09-11
- Context: Rewriting `docs/triage-table.md`'s columns looked like a documentation edit. It is not: `scripts/analyze-merge-conflicts.js` reads the PR numbers out of that file and writes back by fixed index — `parts[5]` assumed to be a bucket, `parts[6]` unconditionally replaced with a merge note. A refresh would have overwritten every disposition in the new table with "Merges cleanly against origin/main.", destroying the triage and leaving a table that still looked filled in. Two review bots on PR #356 found it; I had read the script's PR-number parser and stopped there.
- Lesson: **A generated file's schema belongs to its generator, and a file can be generated without looking generated.** This one has a hand-written header, hand-written prose and hand-written dispositions, so nothing about opening it says "a script writes here". The give-away was in its own purpose comment, which I had read: it names the script as the writer.
- Action: When editing any file a script in `scripts/` names as an output, read the writer's *reconstruction* code, not only its parser — the parser tells you what it reads, and the damage is in what it writes. Where the schema is a table, address cells by header name so the coupling fails loudly: `parseTableColumns` now does that, and `tests/unit/analyze-merge-conflicts.test.ts` reads the committed table so the next drift is a red test rather than a lost decision.
...
- Lesson: **A carried-forward finding records that something is wrong, never how much.** The run that writes it is measuring a different feature and has no reason to count who reaches the defect; the run that takes it must re-derive the blast radius before deciding it is worth taking, and must not quote the earlier note as if it had.
- Action: Before adopting any item off a carried-forward list, grep for the *producers* of the value the defect attaches to, not only the consumers. One command here — `grep -rn "fontStyle: '" src/lib/core/` — moved the finding from "an edge case a spec could reach" to "the majority path", which is what made it this run's feature rather than a footnote in someone else's.

## 2026-09-10
- Date: 2026-09-10
- Context: Four surfaces host `PageLookControls`, and each needs two `EffectivePageLook` values — what the page will be made with, and what it would be made with if the reader cleared their override. Every one of them wrote the same destructure-and-rebuild by hand, in two Svelte dialects. Adding a fourth look field would have been eight edits, and the failure mode of missing one is silent: a copy that omits a field still compiles and still renders, it just describes the page incompletely.
- Lesson: **A named type stops the shape drifting; it does not stop the copies drifting.** `EffectivePageLook` was introduced in Run 25 for exactly this repetition and did half the job — the four call sites still each spelled out the field list, and the type only checks the result, not that the list is complete at each site.
- Action: When a structural type is extracted because a shape was repeated, extract the *constructor* with it. `pageLookOf` is that constructor here, and adding a fourth field became one line in `page-style.ts` rather than eight edits across four files.
