...
- Action: When a change newly reaches a seam, check `assumption-alarm.json`'s `blockedSeams` for it and write a waiver scoped to *that* change, with its own validation plan — or refresh the fixtures. Never cite a waiver whose Status text names a different piece of work.
