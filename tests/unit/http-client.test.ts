...
describe('http-client', () => {
	...
	describe('postJson', () => {
		...
		it('throws HttpError with contract-shaped payload on non-OK responses', async () => {
			const payload = {
				ok: false,
			...
			const err = await postJson('/api/test', {}).catch((e) => e);
			expect(err).toBeInstanceOf(HttpError);
			expect((err as HttpError).status).toBe(400);
			expect((err as HttpError).payload).toEqual(payload);
			expect((err as HttpError).message).toMatch(/postJson: HTTP 400 Bad Request from \/api\/test/);
		});

		it('throws HttpError with arbitrary JSON payload on non-OK responses', async () => {
