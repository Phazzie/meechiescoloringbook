...
- Assumption:
  ...
  - Validation: Not exercised locally - `vite preview` does not read `vercel.json`, so no local run can confirm it. The gap it closes was measured: against a production preview, `/service-worker.js` and `/_app/immutable/entry/start.*.js` returned 200 with none of the headers while document responses carried all of them. Validate by requesting `/service-worker.js` and an `/_app/immutable` asset from the deployed site and confirming the three headers are present.
  - Status: Open - pending first deploy.
