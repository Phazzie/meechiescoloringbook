...
- Consequences: A pull request that ships an evidence folder now has that folder read by CI, on the committed bytes, before any repository-controlled lifecycle script has run. A folder with no dated evidence is not asked to justify anyone else's. Seven rules apply: the chain's exit status and its audit, check and test stages; the outer transcript and `test.txt` being one run rather than two that agree; every chain stage's artifact present, inventoried and current; `e2e.txt` Row 2 carrying its own exit status, per-test lines and the config override it names; every rewind reporting contract tests with none failing; each chain artifact agreeing with its own summary and reporting `overallStatus: ok`; and lint/build transcripts green where present.
