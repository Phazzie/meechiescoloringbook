...
## 2026-07-05 - Upgrade zod v3 to v4 across the contract layer

- Date: 2026-07-05
- Decision: Bump the `zod` dependency from `^3.25.76` to `^4.4.3` and fix every confirmed v4 breaking-change call site so seam contract behavior is unchanged.
- Context: A scheduled repo-health pass ranked the ten hardest outstanding upgrades/fixes and selected this one as the hardest: `zod` is imported by 57 files across every seam contract in the Seam-Driven Development layer, and v4 removed the single-argument `z.record(valueSchema)` form used by `MeechieVoiceSeam` (legacy + self-contained), `ImageGenerationSeam`, `TelemetrySeam`, and `ProviderAdapterSeam`. Upgrading also surfaced two undocumented breaking changes caught only by `npm run check`/`npm test`: `ZodIssue.path` is now typed `PropertyKey[]` instead of `(string | number)[]`, and regex/format string failures now report issue code `invalid_format` instead of the removed `invalid_string`, which `SpecValidationSeam`'s adapter (both flat and self-contained copies) switched on directly.
- Alternatives: Stay on zod v3; rejected because it is the most stale, broadest cross-cutting dependency in the repo and the underlying CVE/feature drift only grows with time. Rewrite the affected schemas to avoid `z.record()` instead of upgrading; rejected because the single-argument form is valid v3 API, not a bug, so this would just delay the real upgrade. Silence the `svelte-check` errors with `as` casts; rejected because that hides a real runtime issue code change instead of fixing the adapter's fault-classification logic.
- Consequences: `z.record()` calls now pass an explicit key schema (`z.record(z.string(), ...)`); `formatPath` in both `SpecValidationSeam` adapter copies accepts `PropertyKey[]` and stringifies non-numeric segments with `String()` instead of template-literal interpolation (symbols would otherwise throw); and label-format validation failures are now classified by `invalid_format` so `LABEL_INVALID_CHARS` fault output is preserved byte-for-byte. No contract, fixture, or mock shape changed.
- Revisit criteria: Revisit if a future zod v4.x release changes `invalid_format`'s `format` discriminator for regex checks, or if any new seam schema reintroduces a single-argument `z.record()` call (grep `z\.record\(` before merging).
- Plan:
  - Goal: Upgrade `zod` to v4 with zero observable contract-behavior change.
  - Seams: MeechieVoiceSeam (legacy + self-contained), ImageGenerationSeam, TelemetrySeam, ProviderAdapterSeam, SpecValidationSeam (legacy + self-contained).
  - Files: `plan.md`, `package.json`, `package-lock.json`, `contracts/meechie-voice.contract.ts`, `contracts/provider-adapter.contract.ts`, `src/lib/seams/meechie-voice-seam/contract.ts`, `src/lib/seams/image-generation-seam/validators.ts`, `src/lib/seams/telemetry-seam/validators.ts`, `src/lib/adapters/spec-validation.adapter.ts`, `src/lib/adapters/spec-validation-seam/index.ts`, `docs/evidence/2026-07-05/*`.
  - Commands: `npm install zod@^4.4.3`, `rg -n "z\.record\("`, `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run verify`, `git diff --check`.
- Self-critique: The riskiest assumption was that a grep for `invalid_type_error`/`required_error`/`errorMap`/`.flatten()`/hardcoded zod message strings was enough to find every v4-incompatible call site; it was not — `svelte-check` caught two more (the `PropertyKey[]` path type and the `invalid_string` -> `invalid_format` code rename) that a text grep could not have found because they are type/enum changes, not literal API removals. The full local gate (`check`, `test`, `lint`, `build`, `verify`) is what actually proved the upgrade safe, not the initial grep.

- Cipher Gate:
  - Date: 2026-07-05
  - Seams: MeechieVoiceSeam, ImageGenerationSeam, TelemetrySeam, ProviderAdapterSeam, SpecValidationSeam
  - Evidence: docs/evidence/2026-07-05/test.txt; docs/evidence/2026-07-05/verify.txt; docs/evidence/2026-07-05/chamber-lock.json; docs/evidence/2026-07-05/shaolin-lint.json; docs/evidence/2026-07-05/seam-ledger.md; docs/evidence/2026-07-05/clan-chain.md; docs/evidence/2026-07-05/proof-tape.md
  - Summary: Upgraded `zod` from v3 to v4, fixed the resulting `z.record()` single-argument removal across four seam contracts, and fixed `SpecValidationSeam`'s issue-code/path-type handling for the v4 `invalid_format` rename and `PropertyKey[]` path typing; full local gate (`check`, `lint`, `test` 519 passed, `build`, `verify`) is green with zero contract behavior change.
  - Risks: This upgrade was verified against the existing fixture-backed contract test suite, not a live re-probe of any external provider; if a future zod point release changes another issue-code discriminator not exercised by current fixtures, it would need its own fault-fixture coverage to catch.

## 2026-07-05 - Decouple SpecValidationSeam issue mapping from zod (PR #219 review follow-up)

- Date: 2026-07-05
- Decision: Remove the `zod` import from `src/lib/core/spec-validation-issue-mapping.ts` (introduced earlier the same day as a Sourcery-requested dedupe) in favor of a local structural `ValidationIssueLike` type, and replace the SPEC_INVALID fallback's raw `issue.message` passthrough with a fixed seam-owned message for every non-`custom` issue code.
- Context: An automated Codex review on PR #219 raised two findings against the just-extracted shared helper: (1) core domain logic importing `ZodIssue` from `zod` violates AGENTS.md's "core domain logic must not depend on third-party libraries" mandate; (2) the SPEC_INVALID fallback forwarded zod's auto-generated `issue.message` verbatim for every spec field not explicitly reclassified (e.g. invalid `outputFormat`/`pageSize` enums, out-of-range `whitespaceScale`), and zod v4 rewrote its default message wording, so the earlier same-day Cipher Gate's "zero contract behavior change" claim was true only for fixture-covered paths, not for that untested fallback text. No fixture or test in this repo currently exercises a non-custom fallback issue, so the wording drift shipped unnoticed.
- Alternatives: Leave the `ZodIssue` import and cast around it; rejected because it does not remove the coupling the mandate exists to prevent. Author a distinct stable message for all ~18 currently-unmapped spec fields; rejected as unnecessary scope growth for this fix — the seam already collapses all of them to the single generic `SPEC_INVALID` code, so a single stable generic message preserves that granularity without inventing per-field copy this PR has no fixture coverage to pin down. Keep forwarding `issue.message` for `custom`-code refinements too; rejected because those messages are authored directly in the contract's `.refine()` calls (e.g. the shading fixture's message) and are already seam-owned and stable regardless of validator version.
- Consequences: `mapZodIssueToSpecValidationIssue` is renamed `mapValidationIssueToSpecValidationIssue` and now takes a `ValidationIssueLike` (`{ path, code, message }`) instead of zod's `ZodIssue`; both `SpecValidationSeam` adapter copies pass `zod`'s issues to it unchanged (structurally compatible, no cast needed). Every fallback issue whose code is `custom` still returns its authored `message` verbatim; every other fallback issue now returns the fixed string `'Spec field failed validation.'` instead of validator-generated text. Added `tests/unit/spec-validation-issue-mapping.test.ts` covering both branches plus the existing `ITEM_NUMBER_OUT_OF_RANGE` classification, since no prior fixture reached the generic fallback.
- Revisit criteria: Revisit if a specific unmapped field (e.g. `outputFormat`) needs a distinct user-facing message; add a fixture-backed classification branch at that point instead of widening the generic fallback.
- Self-critique: The riskiest assumption is that no current caller (UI or API consumer) depends on the exact previous zod-generated fallback text for an unmapped field; since no fixture or test asserted that text, and the seam's `code` for these cases was already the generic `SPEC_INVALID`, this is treated as safe to firm up rather than a behavior change worth a deeper audit.

- Cipher Gate:
  - Date: 2026-07-05
  - Seams: SpecValidationSeam
  - Evidence: `npm run check`, `npm test` (522 passed, 1 skipped), `npm run lint`, `npm run build`, `npm run verify` all green locally after this follow-up.
  - Summary: `SpecValidationSeam`'s shared issue-mapping helper no longer imports `zod`, and its fallback message for non-`custom` issues is now a fixed seam-owned string instead of forwarding the validator's auto-generated (and now version-dependent) text.
  - Risks: The generic fallback message is intentionally non-specific (`'Spec field failed validation.'`); if product wants per-field wording for currently-unmapped fields, that needs its own fixture-backed classification branch, not a further widening of this fallback.
