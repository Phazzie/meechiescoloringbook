...
- Date: 2026-09-06
- Decision: Add `scripts/evidence-guard.mjs` (`npm run evidence:guard`), run it from `.github/workflows/verify.yml` **before** `npm install` and before `npm run verify`, and require every transcript in a dated evidence folder to carry its own `<name> exit=<code>` status as a convention recorded in `docs/evidence/README.md`.
