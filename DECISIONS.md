...
## 2026-09-06 - Make committed evidence checkable by a tracked guard

- Date: 2026-09-06
- Decision: Add `scripts/evidence-guard.mjs` (`npm run evidence:guard`) and `scripts/chain-intact.mjs` (`npm run chain:intact`), run both from `.github/workflows/verify.yml` **before** `npm install` and before `npm run verify`, install dependencies with `npm ci` rather than `npm install`, and require every transcript in a dated evidence folder to carry its own `<name> exit=<code>` status as a convention recorded in `docs/evidence/README.md`.
- Context: Three separate times this repository committed evidence that proved nothing and looked fine — a `verify-outer.txt` that stopped after the audit gate, an `e2e.txt` whose Row 2 was one character while three documents cited "41 passed" from it, and a proof tape that inventoried 1233 bytes against a 6677-byte file. Each was found by a person opening the file. Every other check compared one claim against another claim.
- Alternatives: Route the guard's reads through a seam adapter; rejected because all thirteen scripts in `scripts/` read `node:fs` directly, and a guard that reaches the filesystem through the application cannot report that the application is broken. Keep the checks in the session's capture script; rejected because that is where they lived for eleven rounds while the log described them as existing. Compare a content digest per inventoried artifact; deferred — it needs a schema change in `proof-tape.mjs`, and the same-length-edit hole it closes is partly covered by the new self-agreement rule below.
- Consequences: A pull request that ships an evidence folder now has that folder read by CI, on the committed bytes, before any repository-controlled lifecycle script has run, and every pull request has the gate's own definition read the same way — `npm run verify` is defined in the branch under review, so replacing it with `true`, dropping a stage, or pointing `test` away from the suite would otherwise make CI green having verified nothing. A folder with no dated evidence is not asked to justify anyone else's. Eight rules apply: (1) the chain's exit status, its audit, check and test stages, and no failure in its own summaries; (2) the outer transcript and `test.txt` being one run rather than two that agree, neither reporting a failure; (3) every chain stage's artifact present, inventoried, current, not a replay of a run filed under another folder or date, and not dated after today, the tape excluding nothing but its own two outputs, and `proof-tape.md` present and agreeing with `proof-tape.json` on the run it describes and on every file, size and freshness mark in it; (4) `e2e.txt` Row 1 — the mandated command — passing or carrying a reason and an unexpired, real calendar-date waiver, and Row 2 carrying its own exit status, per-test lines and the config override it names; (5) every `probe-*.txt` carrying its own exit status; (6) every rewind reporting a non-zero count of contract tests with none failing; (7) each chain artifact agreeing with its own summary — every entry counted by some column, not merely every column truthful — and reporting `overallStatus: ok`; `cipher-gate.json` reporting `status: ok` with every file it cites confirmed to exist; and `clan-chain.json` reporting no dirty seams and naming the same clean ones as `seam-ledger.json` and `clan-chain.md`; (8) lint and build transcripts inventoried by the tape and green where present.
- Revisit criteria: Add the digest comparison when `proof-tape.mjs` records one. Revisit the deliberate omission of "require evidence for seam-changing diffs" if the repository decides CI should classify diffs by path. Make every evidence selector enforce the `YYYY-MM-DD` folder shape: `scripts/clan-chain.mjs` and `scripts/proof-tape.mjs` take the lexicographically newest directory under `docs/evidence/` with no date filter, so a folder such as `docs/evidence/zzz/` becomes the chain's input. CI now refuses a change that adds one, which closes the path through review; the selectors themselves are chain scripts that `CLAUDE.md` says not to edit without a plan, so that half is recorded here rather than done in this pull request.

- Cipher Gate:
  - Date: 2026-09-06
  - Seams: none — this change touches `scripts/`, `.github/workflows/`, `docs/` and the run log only. No contract, mock, adapter, fixture or seam test is modified, and `npm run chamber:lock` reports all 38 seams ok before and after.
  - Evidence: scripts/evidence-guard.mjs; .github/workflows/verify.yml; docs/evidence/README.md; docs/evidence/2026-09-05/verify-outer.txt; docs/evidence/2026-09-05/test.txt; docs/evidence/2026-09-05/e2e.txt; docs/evidence/2026-09-05/lint.txt; docs/evidence/2026-09-05/build.txt; docs/evidence/2026-09-05/proof-tape.json; docs/evidence/2026-09-05/rewind-CreationStoreSeam-self-contained.txt; WORST_TO_BEST_LOG.md
  - Evidence note: the paths above are the 2026-09-05 folder, which is this branch's evidence for the heads that carried all but the last of this work and which ships in the same pull request. The gate is run before the chain by the documented capture order, so it can only cite folders that already exist; the current head's run writes `docs/evidence/2026-09-06/`, which the tracked guard checks in CI alongside it.
  - Summary: Moved the evidence checks out of an untracked session script into the repository, then hardened them across eleven review rounds against reproduced defeats: a transcript whose result came from its own stdout, an appended retry inheriting an earlier pass, an entire consistent run replayed under a new date, a config override described but never shown, an artifact omitted from the tape treated as current, a same-length edit to a chain artifact, a rewind reporting `Tests 0 passed` under `--passWithNoTests`, and the plain-English proof summary deleted outright — which no rule could reach, because the tape writes it after taking the inventory that every other rule reads.
  - Risks: The guard reads text produced by tools it does not control, so a reporter format change can make it reject valid evidence — three such false rejections were found and fixed during review, including CRLF transcripts and Windows-written `evidenceDir` paths. Two SonarCloud issues on this branch remain unidentified because sonarcloud.io is unreachable from the capture environment; `sonarjs.configs.recommended` is clean on the file and the differential all-rules scan shows no new rule class, so the remaining candidate is `.github/workflows/verify.yml`. Recorded rather than guessed at.
