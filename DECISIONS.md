...
## 2026-06-19 - Address remaining unresolved review threads from PR #160 and PR #166

- Date: 2026-06-19
- Decision: Land the genuinely still-open review threads from two prior PRs (#160 - image-generation/http-resilience/studio-state fixes; #166 - RateLimitSeam) in one combined PR, since both branched from the same base commit and touch disjoint files. Threads already fixed by a later commit on either PR's own branch (but left unmarked on GitHub) were left as-is rather than re-applied.
- Context: A scheduled routine surveyed PRs opened in the prior 5 days and found #160 and #166 tied at 9 unresolved review threads each. Diffing each thread's suggested fix against the current code on both branches showed PR #166 had already fixed 7 of its 9 threads in a later commit without resolving them on GitHub, leaving 2 genuinely open: a sourcery-ai architectural suggestion to move `RateLimitCheckInput` validation out of `checkAndConsume` to an external boundary, and a chatgpt-codex-connector report that `hitsByKey` in `src/lib/seams/rate-limit-seam/limiter.ts` grows unboundedly for client keys that are seen once and never return. PR #160's 9 threads were all still open.
- Alternatives: Resolve each GitHub thread in place on the original PR branches; rejected because automation on this repo conventionally lands fixes via a new superseding PR rather than pushing further commits to someone else's PR branch. Implement sourcery-ai's `checkAndConsume` refactor; rejected (see Risks) because every other seam in this codebase validates its own contract input inside the seam itself (`validators.ts` per seam), and moving validation to an external boundary for only this seam would be an inconsistent, unrequested architecture change.
- Consequences: `RateLimitSeam`'s limiter now sweeps every key's hit history (not just the key in the current request) on every `checkAndConsume` call, dropping keys whose hits have all expired so idle/one-off client keys no longer accumulate in memory forever. `isTimeoutError` now also matches messages containing "timeout" (not just "timed out"), which is an intentional behavior change restoring stricter timeout classification in `generate-pipeline.ts`/`provider-adapter.adapter.ts`; one existing unit test fixture that incidentally relied on the old gap (`new Error('Network timeout')` expected to classify as a generic network error) was corrected to use a non-timeout message, and a new regression test pins the corrected classification.
- Revisit criteria: Revisit the full-map sweep-per-call approach if profiling ever shows it costing meaningfully under real traffic with very high key cardinality; a periodic/throttled sweep would be the next step.
- Plan:
  - Goal: Close out the remaining unresolved review feedback from PR #160 and PR #166 without introducing a new architecture inconsistent with the rest of the codebase.
  - Seams: RateLimitSeam (limiter behavior change only; contract unchanged).
  - Files: `src/lib/seams/rate-limit-seam/limiter.ts`, `src/lib/seams/rate-limit-seam/test.ts`, `src/lib/core/http-resilience.ts`, `src/lib/core/image-generation-pipeline.ts`, `src/routes/studio-state.svelte.ts`, `tests/unit/image-generation-pipeline.test.ts`, `tests/unit/studio-state.test.ts`, `tests/unit/provider-adapter-helpers.test.ts`, `DECISIONS.md`.
  - Commands: `npx vitest run src/lib/seams/rate-limit-seam/test.ts tests/unit/studio-state.test.ts tests/unit/image-generation-pipeline.test.ts tests/unit/http-resilience.test.ts tests/unit/provider-adapter-helpers.test.ts`, `npm run rewind -- --seam RateLimitSeam`, `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run verify`.
- Self-critique: The first pass at the idle-key fix only deleted an empty array on the *current* request's key, which does nothing for keys that are never queried again; the actual fix had to sweep the whole map using each key's own stored `windowMs`, not the current call's. Caught the test-fixture regression (`Network timeout` message) by running the full suite after the `isTimeoutError` change rather than assuming the targeted seam tests were sufficient.
...
- Cipher Gate:
  ...
  - Summary: Added an idle-key eviction sweep to RateLimitSeam's limiter so client keys that are never seen again don't accumulate in memory, plus carried-over fixes from PR #160 (SVG/base64 detection, draft-save-after-failed-load, timeout/abort classification) with a green `npm run verify`.
  - Risks: The sweep runs on every `checkAndConsume` call rather than on a throttled schedule; acceptable at current expected key cardinality per the Revisit criteria above.

## 2026-06-19 - Address CI/review findings on PR #171 itself

- Date: 2026-06-19
- Decision: Fix the two substantive, code-level findings that landed on PR #171 after it opened (a CodeQL-flagged ReDoS in the new SVG-detection regex, and a Gemini-flagged O(N)-per-request cost in RateLimitSeam's idle-key sweep); explicitly decline a third suggestion (input-length truncation as a ReDoS mitigation, and a separate mimeType-fallback suggestion) with reasoning posted on the originating threads.
- Context: `mcp__github__subscribe_pr_activity` on #171 surfaced a CodeQL high-severity alert on `SVG_TEXT_PATTERN` in `src/lib/core/image-generation-pipeline.ts` (nested `(?:<!--[\s\S]*?-->...)*` is exponentially backtrackable on adversarial input with many repeated `--><!--` runs) and a Gemini Code Assist review with three inline findings: (1) the same ReDoS, suggesting truncating decoded input to 2048 chars before testing; (2) `sweepIdleKeys` running on every `checkAndConsume` call costing O(N) per request; (3) `studio-state.svelte.ts`'s `imagePreviews` could render `data:undefined;base64,...` for drafts saved before `mimeType` existed. The "Rosentic - Conflict Detection" check also failed, but it failed identically on PR #166 before any of this session's changes (confirmed via `get_check_runs` on #166), so it predates this work and is out of scope.
- Alternatives: For the ReDoS, truncate the input to a fixed length before the regex test (Gemini's suggestion) instead of rewriting the pattern; rejected because it only shrinks the attack surface to "still quadratic/exponential, just bounded at 2048 chars" rather than removing the catastrophic-backtracking shape CodeQL flagged, and the deterministic-rewrite fix is the same cost to write and is correct for any input length. For the mimeType concern, add a `image.mimeType || 'image/' + image.format` fallback in `imagePreviews`; rejected because `GeneratedImageSchema` requires `mimeType: NonEmptyStringSchema` and both `DraftRecordSchema.safeParse`/`CreationRecordSchema.safeParse` (in `src/lib/adapters/creation-store.adapter.ts`) reject any stored record missing it before it ever reaches `this.images`, so the failure mode Gemini describes (a rendered image tag with a missing mimeType) cannot occur on this code path — a draft missing the field fails to load entirely instead, which is the already-fixed/already-tested behavior from the PR #160 carry-over fix.
- Consequences: `SVG_TEXT_PATTERN`'s comment-matching segment changed from `<!--[\s\S]*?-->` to `<!--(?:(?!-->)[\s\S])*-->`, which is a deterministic (single-parse-per-match) equivalent for any input that actually matches, so no fixture or behavior changed (`tests/unit/image-generation-pipeline.test.ts` still green). `RateLimitSeam`'s `createRateLimitSeam()` now tracks `lastSweepMs` in its closure and only calls `sweepIdleKeys` when `nowMs - lastSweepMs >= 60_000`; per-key correctness on the read path is unaffected since `pruneExpired` still runs against the specific request's own key on every call regardless of whether the map-wide sweep ran.
- Revisit criteria: If a future CodeQL/security scan flags another regex in this codebase with the same lazy-dot-star-before-terminator shape, apply the same `(?:(?!Y)[\s\S])*Y` rewrite directly rather than truncation. If RateLimitSeam ever needs the sweep to reclaim memory faster than once per 60s (e.g., demonstrated memory pressure under real traffic), shorten `SWEEP_INTERVAL_MS` rather than reverting to sweeping every call.
- Plan:
  - Goal: Close the two real defects surfaced by PR #171's own CI/review activity without adding unnecessary defensive code for unreachable scenarios.
  - Seams: RateLimitSeam (limiter behavior change only; contract unchanged). No seam contract changes for the image-generation-pipeline regex fix (pure helper, not seam-owned).
  - Files: `src/lib/core/image-generation-pipeline.ts`, `src/lib/seams/rate-limit-seam/limiter.ts`, `CHANGELOG.md`, `LESSONS_LEARNED.md`, `DECISIONS.md`.
  - Commands: `npx vitest run tests/unit/image-generation-pipeline.test.ts src/lib/seams/rate-limit-seam/test.ts tests/unit/rate-limit-guard.test.ts`, `npm run rewind -- --seam RateLimitSeam`, `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run verify`, `npm run cipher:gate`.
- Self-critique: Verified the deterministic-regex rewrite is semantically equivalent (not just "still passes the existing fixture") by reasoning through both forms character-by-character rather than only relying on the green test, since a ReDoS fix that quietly changes which inputs match would be worse than the original bug. Did not add a dedicated test asserting the sweep is throttled, since the throttle is a performance characteristic with no externally observable contract difference (per-key correctness already covered by existing tests); flagged this gap here instead of silently skipping it.

- Cipher Gate:
  - Date: 2026-06-19
  - Seams: RateLimitSeam
  - Evidence: docs/evidence/2026-06-19/rewind-RateLimitSeam.txt; docs/evidence/2026-06-19/targeted-tests.txt; docs/evidence/2026-06-19/check.txt; docs/evidence/2026-06-19/lint.txt; docs/evidence/2026-06-19/build.txt; docs/evidence/2026-06-19/test.txt; docs/evidence/2026-06-19/verify.txt; docs/evidence/2026-06-19/seam-ledger.md; docs/evidence/2026-06-19/proof-tape.md
  - Summary: Fixed a CodeQL-flagged ReDoS in the SVG-detection regex and throttled RateLimitSeam's idle-key sweep to once per 60s in response to PR #171's own CI/review findings, with a green `npm run verify`.
  - Risks: None new; the regex rewrite is a like-for-like deterministic equivalent and the sweep throttle only delays idle-key reclamation by up to 60s, which is acceptable per the original Assumption entry's per-process memory scope.

## 2026-06-19 - Address CodeRabbit's follow-up review round on PR #171

- Date: 2026-06-19
- Decision: Fix the two remaining small, valid findings from CodeRabbit's review on #171 (a stale route list in `rate-limit-guard.ts`'s header comment, and a weak fallback-bucket test that only proved one request passes); explicitly skip the rest as either already addressed by a different equivalent fix or out of scope.
- Context: CodeRabbit's review on #171 repeated the same ReDoS and sweep-cost findings Gemini had already raised (both already fixed earlier today via a deterministic-regex rewrite and a throttled sweep, respectively — CodeRabbit's suggested diffs are alternate, also-valid approaches to the same two problems, not new defects). It additionally flagged: `rate-limit-guard.ts`'s `Why` comment omits `/api/tools` even though that route is enforced in the policy map; `tests/unit/rate-limit-guard.test.ts`'s fallback-bucket test only asserts the *first* request through the shared `"unknown"` bucket succeeds, which wouldn't catch a regression that silently disabled enforcement for that bucket; and several "add a header comment / convert to .jsonc" nitpicks on `docs/evidence/**/*.json` files and one `.txt` evidence file.
- Alternatives: Apply CodeRabbit's count-based sweep throttle (`SWEEP_EVERY = 128` checks) instead of the existing time-based one (`SWEEP_INTERVAL_MS = 60_000`); rejected as redundant — both amortize the same O(N) cost to O(1) per request, and switching from one already-tested, already-shipped throttle to a different one with no behavioral advantage would just be net-new churn. Add header comments or `.jsonc` conversions to every flagged evidence JSON/TXT file; rejected because every other dated `docs/evidence/YYYY-MM-DD/*.json`/`*.txt` file in this repo (including ones already on `main`) is a raw, tool-generated dump with no header, so adding one only to the files CodeRabbit happened to flag would be inconsistent rather than a real fix, and retrofitting headers across the whole evidence history is out of scope for this PR.
- Consequences: `rate-limit-guard.ts`'s header comment now lists all six guarded routes. `tests/unit/rate-limit-guard.test.ts`'s fallback test now drives 30 requests through the shared bucket and asserts the 31st is rejected, proving the fallback path is still budget-enforced rather than accidentally unlimited.
- Revisit criteria: If a future evidence-file convention change adds headers to generated JSON/TXT dumps repo-wide, apply it uniformly rather than file-by-file in response to a bot nitpick.
- Plan:
  - Goal: Close the two genuinely actionable findings from CodeRabbit's #171 review without duplicating already-shipped fixes or chasing nitpicks against an established repo convention.
  - Seams: None (route-guard comment text and a test-only change; `RateLimitSeam`'s contract/limiter untouched).
  - Files: `src/lib/server/rate-limit-guard.ts`, `tests/unit/rate-limit-guard.test.ts`, `DECISIONS.md`.
  - Commands: `npx vitest run tests/unit/rate-limit-guard.test.ts src/lib/seams/rate-limit-seam/test.ts tests/unit/image-generation-pipeline.test.ts`, `npm run check`, `npm run lint`, `npm run build`, `npm run verify`, `npm run cipher:gate`.
- Self-critique: Checked the fallback test's shared bucket (`meechie-studio-text:unknown`) isn't touched by any other test in the file before tightening its assertion to a hard request count, since a shared keyed bucket touched by two tests would make the new assertion flaky depending on run order.
