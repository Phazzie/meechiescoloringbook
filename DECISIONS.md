...
- Assumption:
  ...
  - Validation: **The obvious plan does not work, and a reviewer had to point that out before this entry said so.** `probes/image-generation.probe.mjs` writes `fixtures/image-generation/sample.json` and `fault.json`, but nothing reads them: `grep -rn "fixtures/image-generation" src/ tests/ scripts/ contracts/` returns zero hits, while both suites — `src/lib/seams/image-generation-seam/test.ts` and `tests/contract/image-generation.test.ts` — import `src/lib/seams/image-generation-seam/fixtures.ts`. Of the fourteen directories under `fixtures/`, this is the only one with no consumer; the rest have one to four. So "run the probe, then re-run the rewind" would spend a billable live image call and produce a green suite that never touched the regenerated data — a validation that cannot fail for the reason it claims to test. The plan that actually validates, in order: (1) run `node probes/image-generation.probe.mjs` with xAI credentials to capture a live sample and a live fault; (2) update `src/lib/seams/image-generation-seam/fixtures.ts` — the file both suites import — from those captures, keeping its exported shapes; (3) only then run `npm run rewind -- --seam ImageGenerationSeam`, `npm run rewind -- --seam "ImageGenerationSeam (self-contained)"` if a canonical row is ever added, and `npm test -- tests/contract/image-generation.test.ts src/lib/seams/image-generation-seam/test.ts`, which now exercise the regenerated data. Step 2 is a code change to a seam folder and gets the full workflow; it is deliberately not folded into this documentation change. Wiring the mock to read the JSON captures instead is the other repair and is the better end state, but it changes what the seam's mock consumes, which is a larger decision than a waiver should take — recorded as follow-up 9. Until then the seam is covered by its existing contract suite, which passes on this head, and by the end-to-end suite, which stubs `/api/generate` and so exercises the caller rather than the provider.
