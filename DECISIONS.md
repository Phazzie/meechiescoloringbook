...
- Date: 2026-09-06
- Decision *(this statement is the final invariant, rewritten after review; the two corrections below record how it got here and are the more instructive half)*: Render the two studio-text fields nothing consumed — `qualityState` and `revisionNote` — through a new pure `src/lib/core/verdict-report.ts`, and take the fact of a standing having been *reported* as a **separate input** (`standingWasReported`) rather than as a new contract field. `StudioState` owns the provenance in `verdictSource: 'live' | 'stored' | 'derived' | 'none'`, and the two questions it governs are answered independently: **only `'live'` may be claimed as a standing** (a live studio response), while **only `'derived'` is withheld from storage** (words rebuilt from `intent`, so storing them adds nothing but the invented `'ready'`). `'stored'` text is somebody's real words and is written back, standing or no standing. There are exactly two writers, `setVerdict` (a verdict plus a required source) and `clearVerdict` (nothing), and `textOutput` is a getter, so no caller can produce an inconsistent pair. No file under `contracts/`, `probes/`, `fixtures/`, `src/lib/mocks/`, `src/lib/adapters/`, `src/lib/seams/` or `tests/contract/` changed.
