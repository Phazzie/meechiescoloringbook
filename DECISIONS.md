...
## 2026-06-16 - Eliminate dual seam layout for 5 seams (PromptAssembly, DriftDetection, MeechieVoice, MeechieTool, SpecValidation)
- Date: 2026-06-16
- Decision: Delete the 20 legacy flat-layout files (contract/mock/adapter/test) for PromptAssemblySeam, DriftDetectionSeam, MeechieVoiceSeam, MeechieToolSeam, and SpecValidationSeam, redirect every consumer import to the canonical self-contained `src/lib/seams/<seam-name>/` layout, and consolidate `docs/seams.md` to one row per seam.
- Context: These 5 seams had been partially migrated to the self-contained layout, but the legacy flat files were never deleted and ~30 production/test files still imported from the flat paths, creating dual sources of truth for shared types (`ColoringPageSpec`, `MeechieToolIdSchema`, `Violation`, `DriftDetectionOutput`, `SpecValidationOutput`, etc.). This is the same problem already attempted, separately and independently, by 4 prior open PRs (#151, #156, #159, #161) created on 2026-06-08, 2026-06-13, 2026-06-14, and 2026-06-15 — none of which were merged, so the next day's automated run kept re-discovering the same defect from a stale `main`. This PR performs the migration fresh against current `main` rather than rebasing one of the stale branches.
- Alternatives: Reuse/rebase one of PRs #151/#156/#159/#161 instead of redoing the work; rejected for this run because the harness instructs developing on a specific designated branch and because the 4 branches diverge from each other in scope (some also touch `contracts/CLAUDE.md`, fixture relocation, or unrelated quick-wins), so a clean redo against current `main` is less error-prone than reconciling four overlapping diffs. The duplication itself is flagged for the human operator to resolve by merging one and closing the rest.
- Consequences: `contracts/`, `src/lib/mocks/`, `src/lib/adapters/*.adapter.ts` (flat), and `tests/contract/` no longer contain anything for these 5 seams; all reads go through `src/lib/seams/<seam-name>/contract.ts` and `src/lib/adapters/<seam-name>/index.ts`. The legacy `fixtures/<seam>/*.json` directories are kept as-is because the canonical `fixtures.ts` modules still import JSON from them. `docs/seams.md` and `contracts/CLAUDE.md` updated to describe the post-migration state.
- Revisit criteria: Revisit if a future seam reintroduces a flat/self-contained split, or if the fixture JSON for these 5 seams is later relocated into the seam folders (at which point the `fixtures/<seam>/` directories can be deleted too).
- Plan:
  - Goal: Make the self-contained layout the sole layout for PromptAssemblySeam, DriftDetectionSeam, MeechieVoiceSeam, MeechieToolSeam, and SpecValidationSeam, with zero behavior change.
  - Seams: PromptAssemblySeam, DriftDetectionSeam, MeechieVoiceSeam, MeechieToolSeam, SpecValidationSeam.
  - Files: `docs/seams.md`, `contracts/CLAUDE.md`, `DECISIONS.md`, `LESSONS_LEARNED.md`, `plan.md`, deletions of `contracts/{prompt-assembly,drift-detection,meechie-voice,meechie-tool,spec-validation}.contract.ts`, `src/lib/mocks/{prompt-assembly,drift-detection,meechie-voice,meechie-tool,spec-validation}.mock.ts`, `src/lib/adapters/{prompt-assembly,drift-detection,meechie-voice,meechie-tool,spec-validation}.adapter.ts`, `tests/contract/{prompt-assembly,drift-detection,meechie-voice,meechie-tool,spec-validation}.test.ts`; import redirects across `src/lib/components/MeechieModePage.svelte`, `src/lib/components/MeechieTools.svelte`, `src/lib/components/meechie-mode-config.ts`, `src/lib/components/studio/StudioSettingsPanel.svelte`, `src/lib/components/studio/SystemTrace.svelte`, `src/lib/core/chat-interpretation-pipeline.ts`, `src/lib/core/image-generation-pipeline.ts`, `src/lib/core/meechie-studio.ts`, `src/lib/core/prompt-template.ts`, `src/lib/core/tools-pipeline.ts`, `src/lib/seams/safety-policy-seam/contract.ts`, `src/lib/utils/alignment-line.ts`, `src/routes/random/+page.svelte`, `src/routes/rate-his-excuse/+page.svelte`, `src/routes/who-fucked-up/+page.svelte`, `src/routes/studio-state.svelte.ts`, `tests/helpers/make-base-spec.ts`, `tests/unit/coloring-page-title.test.ts`, `tests/unit/drift-detection-helpers.test.ts`, `tests/unit/meechie-tool-adapter.test.ts`, `tests/unit/meechie-tool-adapter.responses.test.ts`, `tests/unit/meechie-tools-parity.test.ts`; relative-path redirects in surviving flat contracts `contracts/creation-store.contract.ts`, `contracts/image-generation.contract.ts`, `contracts/output-packaging.contract.ts`, `contracts/chat-interpretation.contract.ts`, `contracts/generate.contract.ts`.
  - Commands: `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run verify`, `npm run cipher:gate`.
- Self-critique: The main risk is silently deleting a file that production still depends on. Mitigated by grepping every repo-wide reference to each legacy path before deletion and diffing legacy vs. canonical file contents to confirm they were functionally identical mirrors, not divergent forks. A second risk is treating the legacy `fixtures/<seam>/` JSON directories as dead weight; verified via grep that `src/lib/seams/<seam-name>/fixtures.ts` still imports from them, so those directories were deliberately kept. The test count dropped from 519/62 files to 494/57 files, exactly matching the 25 deleted duplicate contract tests across 5 files, which is the strongest evidence of a clean 1:1 removal with no coverage loss.

- Cipher Gate:
  - Date: 2026-06-16
  - Seams: PromptAssemblySeam, DriftDetectionSeam, MeechieVoiceSeam, MeechieToolSeam, SpecValidationSeam
  - Evidence: docs/evidence/2026-06-16/check.txt; docs/evidence/2026-06-16/lint.txt; docs/evidence/2026-06-16/test.txt; docs/evidence/2026-06-16/build.txt; docs/evidence/2026-06-16/verify.txt
  - Summary: Deleted 20 legacy flat-layout files for 5 seams, redirected all consumer imports (31 files) to the canonical self-contained `src/lib/seams/<seam-name>/` layout, kept the legacy fixture JSON directories in place since the canonical fixtures still load from them, and consolidated `docs/seams.md`/`contracts/CLAUDE.md` to describe a single layout per seam.
  - Risks: Four prior open PRs (#151, #156, #159, #161) attempted this same fix independently and were never merged; that duplication is a process problem (the automated routine re-discovers this defect daily against a stale `main` instead of checking for or merging prior open PRs) flagged for the repo owner to resolve by merging one PR and closing the others.
