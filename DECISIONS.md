...
## 2026-06-22 - Add baseline HTTP security headers and CSP
- Date: 2026-06-22
- Decision: Add a new `src/hooks.server.ts` global `handle` hook that attaches `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, and `Strict-Transport-Security` to every response, and configure SvelteKit's built-in `kit.csp` in `svelte.config.js` with `mode: 'auto'` (nonce-mode on this app's fully dynamic routes) and an explicit per-directive allowlist.
- Context: A repo-wide audit for the hardest outstanding gaps found the app shipped with zero HTTP security headers and no Content-Security-Policy at all — there was no `hooks.server.ts` and no `csp` config anywhere. SvelteKit injects its own hydration bootstrap as an inline `<script>` tag, so a naive `script-src 'self'` would have broken hydration; `mode: 'auto'` resolves to nonce-mode here and SvelteKit auto-injects the matching nonce into that inline script. Three call sites (`rate-his-excuse/+page.svelte`, `StudioPreviewPanel.svelte`, `app.html`) use hand-written inline `style="..."` attributes that SvelteKit's nonce mechanism does not cover, so `style-src` keeps a documented `'unsafe-inline'` exception. `img-src` allows `https://placehold.co` because the wig catalog (`src/lib/data/wigs.json`) currently links placeholder images from that host (tracked separately as its own follow-up item, not fixed here).
- Alternatives: Hand-roll a custom nonce/hash scheme outside `kit.csp`; rejected because SvelteKit's built-in support already covers the hydration script correctly and a custom scheme would duplicate that logic with more room for drift. Ship `script-src 'self' 'unsafe-inline'` instead of nonce-mode; rejected because it would defeat the main point of the CSP for the one inline script that matters most (the hydration bootstrap) and is a materially weaker policy. Skip `Strict-Transport-Security` since Vercel terminates TLS; rejected because HSTS still instructs browsers to upgrade future requests and costs nothing to add.
- Consequences: Every response now carries baseline clickjacking/MIME-sniffing/referrer/permissions protections and a real CSP; no behavior change to any route or API response body/status. `style-src 'unsafe-inline'` remains a known, documented tradeoff until the three inline-style call sites are refactored to CSS classes.
- Revisit criteria: Revisit if the three inline `style="..."` attributes are moved to CSS classes (allows dropping `style-src 'unsafe-inline'`), if the wig catalog moves off `placehold.co` placeholder images (allows tightening `img-src`), or if a real browser/Playwright run becomes available in this sandbox to replace the curl-based hydration verification with an actual browser console/network check.
- Plan:
  - Goal: Close the "no HTTP security headers or CSP" gap identified as one of the hardest outstanding fixes, with zero regressions to existing routes/APIs.
  - Seams: none (this is global SvelteKit framework configuration and transport-layer middleware, not an SDD seam — no filesystem/network/process/clock/randomness I/O is introduced).
  - Files: `svelte.config.js`, `src/hooks.server.ts`, `tests/unit/hooks-server.test.ts`, `CHANGELOG.md`, `DECISIONS.md`, `docs/evidence/2026-06-22/*`.
  - Commands: `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run preview` (backgrounded) + `curl` header/nonce/route-sweep verification, `npm run verify`.
- Self-critique: The riskiest assumption is that nonce-mode CSP does not break hydration; this was verified directly by starting a production `npm run preview` server and confirming via `curl` that the nonce in the `Content-Security-Policy` response header matches the `nonce="..."` attribute on the actually-served inline hydration `<script>` tag, plus a full route sweep (all routes 200 with CSP present) and a confirmation that `POST /api/tools` still returns its normal 400 response under the new headers. Real-browser Playwright verification was attempted but blocked by sandbox network egress rules (`cdn.playwright.dev` not in the allowlist); the curl/nonce-matching approach is the documented mitigation rather than a false claim of browser-verified proof.

- Cipher Gate:
  - Date: 2026-06-22
  - Seams: none
  - Evidence: docs/evidence/2026-06-22/security-headers-check.txt; docs/evidence/2026-06-22/security-headers-lint.txt; docs/evidence/2026-06-22/security-headers-test.txt; docs/evidence/2026-06-22/security-headers-build.txt; docs/evidence/2026-06-22/security-headers-live-verification.txt
  - Summary: Added a global `hooks.server.ts` security-headers middleware and a `kit.csp` nonce-mode Content-Security-Policy; verified header presence, nonce-script matching, full route sweep, and unchanged API behavior against a real production preview server.
  - Risks: Real-browser Playwright verification is blocked in this sandbox (network egress denies the Playwright CDN), so browser-console-level CSP violation checks were not performed; `style-src` keeps a documented `'unsafe-inline'` exception for three pre-existing inline `style="..."` attributes until they are refactored to CSS classes.
