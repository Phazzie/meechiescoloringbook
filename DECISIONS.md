...
- Cipher Gate:
  ...
  - Summary: Added self-contained RateLimitSeam (sliding-window limiter with throttled idle-key sweep and a corrected `resetAtMs` formula shared by both the block and allow paths) and wired `enforceRateLimit` into all 6 metered AI-provider routes, closing the in-scope unaddressed review threads from PR #171 and PR #166; `npm run verify` passed clean (svelte-check 0 errors/0 warnings, 548 tests passed/1 skipped, lint/build green).
