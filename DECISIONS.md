...
- Revisit criteria: If the owner ever rules in lines that are not canon, re-introduce the distinction as a new contract change with its own red proof - do not restore `tier` as a comment-only marker.
...
- Plan:
  ...
  - Files: `contracts/meechie-quote.contract.ts`, `contracts/meechie-voice.contract.ts`, `src/lib/seams/meechie-voice-seam/contract.ts`, `src/lib/seams/meechie-voice-seam/voice-pack.ts`, `src/lib/seams/meechie-voice-seam/test.ts`, `fixtures/meechie-voice/sample.json`, `fixtures/meechie-voice/malformed-pack.json`, `DECISIONS.md`, and the regenerated `docs/evidence/2026-09-03/` artifacts including `drop-tier-red-proof.txt`.
