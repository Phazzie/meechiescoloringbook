...
## 2026-06-22 - Port RateLimitSeam from PR #171 and #166 to close unaddressed review threads

- Date: 2026-06-22
- Decision: Add a new self-contained `RateLimitSeam` (sliding-window limiter, idle-key sweeping, `resetAtMs` computed identically on the block and allow paths) plus `src/lib/server/rate-limit-guard.ts`, wire it into all 6 metered AI-provider routes (`generate`, `image-generation`, `chat-interpretation`, `meechie-studio-text`, `tools`, `wig-try-on`), and register the seam in `docs/seams.md` and `src/lib/seams/CLAUDE.md`.
- Context: Across the PRs opened in the last 5 days, PR #171 and PR #166 carried the most unaddressed review threads (11 and 9 respectively), both implementing the same rate-limiting feature as sibling branches off slightly different bases. Diffing each branch's tip against current `main` showed most of the substantive review feedback (sweep throttling, the `resetAtMs` block/allow asymmetry bug, idle-key eviction, missing `/api/tools` coverage, a weak fallback-bucket test, missing negative-value fixtures, a stale guard comment) had already been fixed within each PR's own commit history; the remainder was that neither branch's final state existed on `main` at all.
- Alternatives: Merge PR #171 or #166 directly; rejected because each also carries an out-of-scope, unrelated SVG/webp image-detection feature (inherited from prior PR #154/#157 baggage) that doesn't exist on `main` and isn't part of either PR's actual rate-limit contribution. Re-derive review fixes comment-by-comment against each PR's HEAD; rejected once diffing showed both branches already converged on the same corrected RateLimitSeam implementation, making a from-scratch re-fix redundant. Port everything in the raw 62-file diff; rejected as scope creep beyond the rate-limit feature the comments concern.
- Consequences: Six routes now enforce per-route sliding-window budgets via `enforceRateLimit`; a new pure, dependency-injected seam exists with full contract/mock/fixtures/test coverage and an idle-key sweep to bound memory; the SVG/webp ReDoS-regex and studio-state mimeType-fallback comments from both PRs remain unaddressed here because their underlying feature is out of scope.
- Revisit criteria: Revisit if PR #171/#166 are closed in favor of this port (the originals become redundant), if rate limits need to move from per-process memory to a shared store, or if the excluded SVG/webp image-detection feature is separately merged to `main` (its own ReDoS/mimeType review comments would then need addressing on their own terms).
- Plan:
  - Goal: Close the highest-unaddressed-review-comment PRs (#171, #166) by porting their converged, in-scope fix into a new PR against `main`.
  - Seams: RateLimitSeam.
  - Files: `src/lib/seams/rate-limit-seam/{contract.ts,validators.ts,limiter.ts,fixtures.ts,mock.ts,probe.ts,test.ts}`, `src/lib/server/rate-limit-guard.ts`, `tests/unit/rate-limit-guard.test.ts`, `src/routes/api/{generate,image-generation,chat-interpretation,meechie-studio-text,tools,wig-try-on}/+server.ts`, `docs/seams.md`, `src/lib/seams/CLAUDE.md`, `DECISIONS.md`, `docs/evidence/2026-06-22/*`.
  - Commands: `npm run check`, `npm test`, `npm run lint`, `npm run build`, `npm run verify`.
- Self-critique: The main risk is treating comments as "already addressed" based on reading each branch's tip rather than re-verifying against this repo's own test suite; that risk is covered by the full local `npm run check`/`test`/`lint`/`build`/`verify` run against the ported code, not just trusting the source branches. A second risk is under-scoping by excluding the SVG/webp-related comments; that exclusion is deliberate and called out explicitly in the PR description rather than silently dropped, since the underlying feature genuinely doesn't exist on `main`.

- Cipher Gate:
  - Date: 2026-06-22
  - Seams: RateLimitSeam
  - Evidence: docs/evidence/2026-06-22/chamber-lock.json; docs/evidence/2026-06-22/verify.txt; docs/evidence/2026-06-22/test.txt; docs/evidence/2026-06-22/seam-ledger.json; docs/evidence/2026-06-22/seam-ledger.md; docs/evidence/2026-06-22/clan-chain.json; docs/evidence/2026-06-22/clan-chain.md; docs/evidence/2026-06-22/proof-tape.json; docs/evidence/2026-06-22/proof-tape.md; docs/evidence/2026-06-22/shaolin-lint.json; docs/evidence/2026-06-22/assumption-alarm.json
  - Summary: Added self-contained RateLimitSeam (sliding-window limiter with throttled idle-key sweep and a corrected `resetAtMs` formula shared by both the block and allow paths) and wired `enforceRateLimit` into all 6 metered AI-provider routes, closing the in-scope unaddressed review threads from PR #171 and PR #166; `npm run verify` passed clean (svelte-check 0 errors/0 warnings, 544 tests passed/1 skipped, lint/build green).
  - Risks: The SVG/webp image-detection ReDoS-regex and studio-state mimeType-fallback review comments from both source PRs are intentionally not addressed here, since that feature does not exist on `main`; rate limiting remains per-process in-memory state, so it will not coordinate across multiple server instances/regions. `enforceRateLimit` runs before request-body validation on all 6 routes by design: rate limiting is meant to be the outermost guard against abusive call volume, and validating first would let an attacker send unlimited malformed requests without ever tripping the budget. The accepted tradeoff is that a legitimate client's own malformed request burns one slot of its budget even though no provider call was made; this is judged a minor, self-inflicted cost against a single client's own quota, not a cross-client abuse vector, so the ordering is kept as-is.
