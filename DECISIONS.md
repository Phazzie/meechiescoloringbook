...
- Date: 2026-09-06
- Decision: Render the two studio-text fields nothing consumed — `qualityState` and `revisionNote` — through a new pure `src/lib/core/verdict-report.ts`, and take the fact of a standing having been *reported* as a **separate input** (`standingWasReported`) rather than as a new contract field. `StudioState` owns that fact in `verdictStandingReported`, written only by `setVerdict`, and `textOutput` becomes a getter so the two cannot be set apart. `describingStudioText()` withholds studio text whose standing was never reported, so the invented value is not written into a record. No file under `contracts/`, `probes/`, `fixtures/`, `src/lib/mocks/`, `src/lib/adapters/`, `src/lib/seams/` or `tests/contract/` changed.
...
- Consequences: The card claims a standing only when one was reported, and a legacy record keeps storing no studio text rather than acquiring an invented approval. That exclusion is only safe because every field `buildStudioTextFromSpec` produces is derived from `intent` — title, items and `specOwnQuote` — so the round trip is lossless; a test asserts exactly that, and it is the test that fails if that function ever grows an input it cannot rebuild from the record. `textOutput` is read-only from outside, which narrows a previously fully-writable public field; `acceptVerdict` is the one public writer, and it is the operation the studio actually performs.
