...
- Date: 2026-09-06
- Decision: Add an optional `driftCheckFailure: { code, message }` to `GenerateResponseValueSchema` in `contracts/generate.contract.ts`. `runGeneratePipeline` populates it from `detectDrift`'s error when the seam returns `{ ok: false }`, leaving `violations` empty — because the seam genuinely found nothing, having graded nothing. `src/lib/core/quality-report.ts` reads the field and renders a `check-failed` finding. No seam contract, probe, fixture, mock or adapter changes.
