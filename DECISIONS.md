...
- Cipher Gate:
  ...
  - Summary: Fixed WigTryOnSeam's unused geminiBaseUrl config, README Gemini env-var drift, a redundant "out of 10" score label, and a mislabeled portrait download extension; full verify green (520 tests passed, 1 skipped).
  - Risks: The WigTryOnSeam base-URL fix changes real request routing when GEMINI_BASE_URL is overridden; covered by a new adapter test but not by a live probe against a real alternate endpoint.
