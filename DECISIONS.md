...
- Date: 2026-09-09
- Decision: `CreationStoreSeam`'s adapter no longer evicts. `upsertRecord`'s `[record, ...filtered].slice(0, MAX_CREATIONS)` is replaced by `planCreationWrite` in a new pure module, `src/lib/core/vault-capacity.ts`, which either returns the exact array to store or refuses the save with a sentence naming the remedy. The record cap is counted against the **saving owner's** records rather than the whole stored array; the write still carries every owner's records, so no session's save can delete another's. A storage-quota failure is reported as `STORAGE_FULL` with its own message, told apart from the generic `STORAGE_WRITE_FAILED` that covers a browser with site data blocked. **The contract is unchanged**: no schema, no field, no method signature, and no stored shape is touched — `Result`'s error branch already existed, and `MAX_CREATIONS` was already exported by the contract.
- Context: measured on `main` at `bd070e2`. The store kept the newest fifty records and dropped the rest, then returned `ok`, so all fifteen page-making surfaces reported `VAULT_SAVED_CONFIRMATION` for a save that had just destroyed the reader's oldest saved page. `favorite` — the reader's one "keep this one" control, offered on every row of `VaultGallery.svelte` — appears nowhere in that expression, so a pinned page was evicted exactly like any other. **And fifty is not a boundary any reader reaches.** A real captured provider image in `fixtures/image-generation/sample.json` is 236,380 base64 characters; fifty of those cannot fit in a browser's localStorage, so the limit actually met is bytes, and it arrived as `Failed to write storage for cb_creations_v1.` — an internal key and no remedy. The capacity machinery the app had built (`VAULT_CAPACITY`, `undoDelete`'s guard, `vaultFullRefusal`) all defended the boundary that never happens while the one that does went unhandled.
- Alternatives: (a) **Keep evicting, but evict the oldest *unpinned* page and say which one went.** The better product on its face, and rejected on one hard constraint: for the confirmation to name the page that was removed, `saveCreation` has to report it, which means a new field on the seam's output — a contract change, which `AGENTS.md` puts behind "do not auto-merge" for good reason. Rejected on the merits too: the page being refused is still on screen, still downloadable, still printable and still shareable, while the pages it would have evicted are not recoverable at all. Refusing loses the cheap thing and keeps the expensive ones. (b) **Reclaim orphaned records** — those under a session id no live reader can produce — to free slots. Rejected: they are unreachable, not worthless, and a rule that silently deletes records is the rule this change exists to remove, not to narrow. Counting per owner solves the reported symptom without deleting anything. (c) **Predict the byte ceiling and refuse before reaching it.** Rejected as invented behaviour: the ceiling depends on the browser and on the size of the pictures already stored, so any number picked here would be the same kind of fiction as fifty. Letting the write fail and reporting the failure honestly is the only answer that is true on every device. (d) **Preserve stored entries that fail schema validation**, which are currently dropped by the next write. Measured and deliberately not fixed: an entry nothing can parse cannot be rendered, reopened or downloaded, and preserving it would consume bytes on a store whose real limit is bytes, with no way for a reader to remove it. Recorded in `WORST_TO_BEST_LOG.md` rather than silently left.
- Consequences: `VAULT_CAPACITY` in `src/lib/core/vault-gallery.ts` stops being a hand-copied `50` and re-exports `MAX_CREATIONS` from the contract. The comment justifying the copy said the constant was module-private in `creation-store.adapter.ts`; that had stopped being true, and the mirror was a duplicate defended by a stale reason. `undoDelete`'s capacity guard in `vault-collection.svelte.ts` was documented as "a lower bound, not a store-wide guarantee" because the caller counted one owner while the store counted all; making the cap owner-scoped turns that estimate into the same number the store applies. `vault-page.ts`'s `showsVaultLink` became `vaultLinkFor`, because a boolean could not express the two refusals that *should* carry a link: they fail because the vault is full, and the remedy is in the vault. Separately, three savers wrote records and only one had reasoned the id through — `newCreationId` now lives in `src/lib/core/creation-id.ts` and all fifteen use it, and the home studio and tools hub stop writing `fixesApplied` from recommendations they never applied and stop reading the wall clock directly.
- What this change does not prove: no test in this repository fills a real browser's localStorage to its true limit. The quota path is exercised by throwing the three signatures browsers actually use (`QuotaExceededError`, `NS_ERROR_DOM_QUOTA_REACHED`, and the legacy numeric codes) from a spied `setItem`, which is the shape of the failure rather than the failure itself. The record-cap path *is* driven against the real adapter and real `localStorage` in jsdom.
- Review round: Sourcery raised four findings; three were fixed and the fourth was answered. The load-bearing one is that `planCreationWrite` decided "replacement?" on `id` alone, so a record with that id belonging to a *different* owner was dropped from the array, skipped the capacity check, and was written over — the exact deletion this change exists to prevent, contradicting both this module's header and a test in this very change. It is now refused as `VAULT_ID_COLLISION`, with a self-healing remedy: every saver mints a fresh id per attempt. The fourth finding — that read/plan/write is not atomic across browser tabs — is real and its suggested fix (re-read before committing) closes nothing, because there is no `await` between the load and the `setItem` for a re-read to move past. A version stamp with compare-and-retry is the real fix, is new observable behaviour at the seam, and is carried in `WORST_TO_BEST_LOG.md`. The window is unchanged by this change: the expression replaced had the same one, and also evicted.
- Second review round (Codex): the load-bearing finding is that an undo refused for *device room* fell through to the adapter's "Delete a saved page to make room for this one." — and following that from the undo banner calls `remove`, which overwrites `undoableDeletion` and destroys the page Undo is holding. The record-cap branch already warned about exactly that trap; the byte wall is a different wall reached at a different count, and this change created the path to it. Now intercepted with the same warning, and `vaultFullRefusal` takes a `because` argument so it never claims "full at 50 pages" over a vault holding nine. Also: the mock now keeps what it is handed and asks `planCreationWrite`, rather than documenting that it could not express a capacity refusal — a stated gap is still a gap, and the mock's own invariant is that it refuses what the adapter refuses. And `newCreationId` moved out of `src/lib/core`, which is declared deterministic, to `src/lib/components/creation-id.ts`; putting identifier entropy behind a seam is a contract addition and is carried in `WORST_TO_BEST_LOG.md`.
- Revisit criteria: if `CreationStoreSeam`'s output ever gains room to report what a save displaced, alternative (a) becomes reachable and should be re-argued — evicting the oldest unpinned page *and naming it* is a better vault than one that refuses, and the only thing blocking it here was the contract.

- Cipher Gate:
  - Date: 2026-09-09
  - Seams: CreationStoreSeam
  - Evidence: docs/evidence/2026-09-09/verify-outer.txt; docs/evidence/2026-09-09/test.txt; docs/evidence/2026-09-09/check.txt; docs/evidence/2026-09-09/lint.txt; docs/evidence/2026-09-09/build.txt; docs/evidence/2026-09-09/rewind-CreationStoreSeam.txt; tests/unit/vault-capacity.test.ts; tests/unit/creation-store-helpers.test.ts; src/lib/seams/creation-store-seam/test.ts; docs/seams.md
  - Summary: The creation store stopped evicting. A save that does not fit is refused with a remedy instead of silently deleting the reader's oldest saved page and reporting success; the per-owner cap, an id another session already holds, and the device-storage limit are reported as distinct, actionable refusals. The seam's contract, schemas, fixtures and mock are unchanged - only the adapter's behaviour within the shapes the contract already declared, plus a new pure policy module in core that the adapter asks.
  - Risks: The mock replays fixtures and holds no store, so it cannot express either refusal; a consumer reading only the mock would conclude that a schema-valid record in a browser always saves. That gap is now stated as its own test in `src/lib/seams/creation-store-seam/test.ts` rather than left silent, and the behaviour is covered against the real adapter in `tests/unit/creation-store-helpers.test.ts`. The device-full path is proven against thrown quota signatures rather than against a genuinely full browser profile, which no test here could produce deterministically.

## 2026-09-08 - Carry the bucket on every quota snapshot instead of letting one meter narrate two buckets

- Date: 2026-09-08
- Decision: Make `AiQuotaBucket` (`'text' | 'image'`) a **required** field on `AiQuotaSnapshot` and a **required** argument to `readAiQuota`; hold readings in an `AiQuotaLedger` with one slot per bucket; and introduce `AiQuotaMeter` (`src/lib/components/ai-quota-meter.svelte.ts`) as the single holder every surface uses, replacing the two hand-copied implementations. Wire the six `postJson` call sites that discarded quota headers. **No file under `contracts/`, `probes/`, `fixtures/`, `src/lib/mocks/`, `src/lib/adapters/` or `src/lib/seams/` is touched**, confirmed by `git diff --name-only origin/main...HEAD` against those directories rather than asserted — which is also why there is no Cipher Gate entry.
- Context: the server meters **two** buckets, on independent 60-second windows with different limits (`src/lib/seams/rate-limit-seam/validators.ts:6-8`): `text` holds 20 units and funds `/api/meechie-studio-text`, `/api/tools` and `/api/chat-interpretation`; `image` holds **8** and funds `/api/generate`, `/api/image-generation` and `/api/wig-try-on`. Measured on `main` at `b5119ea`: of the eight `postJson` call sites reaching a billable endpoint, **two passed `onResponseHeaders` and six discarded every quota header the server sent** — including *every* call to `/api/generate`, the route that pays for the coloring pages this app exists to make. The `image` bucket was reported on **no surface at all**. Run 7 rebuilt "the AI budget meter" and made it truthful about `text`; the sentence it produced then sat above a "Create Coloring Page" button that spends `image`. So the defect was never a missing meter — it was a meter confidently narrating a bucket the button beneath it does not spend, which is worse than silence because it is believable.
- Alternatives: (a) **Keep `bucket` optional, defaulting to `text`.** Rejected, and this is the core of the decision: an optional discriminator preserves exactly the shape that produced the bug, because the wrong reading stays writable and the compiler stays quiet. Making it required is what turned this from a code review into a compile error — the type checker, not my judgement, enumerated the call sites that had to change, and it found test files I would not have thought to grep. (b) **Add a second `imageQuota` field beside the existing `aiQuota` on each state class.** Rejected: it doubles the copy-pasted machinery that caused the spread failure in the first place, and gives four fields to keep in sync instead of one ledger. (c) **One shared expiry timer for both buckets.** Rejected because the windows are independent — one bucket expiring would clear a live reading for the other. `AiQuotaMeter` holds one cancellation handle per bucket. (d) **Report only the image bucket and drop the text line**, on the grounds that pages are what matter. Rejected: running out of verdicts and running out of pages are different events a reader must be able to tell apart, which is also why the studio's text line is now named "verdicts or rewrites" rather than the old generic "AI calls" — with two lines on screen, a coloring page is *also* an AI call.
- Consequences: recording the image bucket inside `PageArtifactState.generatePage` reaches **thirteen page-making surfaces at once** — the three standalone mode routes, all eight `/m/<slug>` pages, `/describe` and the home studio — because they all reach `/api/generate` through that one method. `describeAiQuota` gained `actionNounPlural`, because the naive `noun + 's'` rule turns "verdict or rewrite" into "verdict or rewrites", which reads as one verdict and several rewrites. A page is priced at `spec.variations`, not at a flat unit, because `runGeneratePipeline` charges `imageRequest.variations` — `/describe` can ask for four pictures, and pricing that page at one unit would promise three pages that do not exist. The two duplicated `setAiQuota`/`cancelQuotaExpiry` implementations in `studio-state.svelte.ts` and `describe-page-state.svelte.ts` are deleted in favour of the shared meter.
- The meter resolves its `ClockSeam` through a **function** rather than capturing one at construction. The state classes expose `clock` as an assignable field and tests replace it *after* the constructor runs, so a captured clock would schedule expiry against wall time while a test drove a fake — and would fail silently, since nothing surfaces until a timer that never fires makes a reading outlive its window.
- What this change does **not** prove: no live provider call was made (`XAI_API_KEY` is not available in this container), so the header values these surfaces now render are exercised against stubbed responses shaped like the guard's real output (`decisionHeaders` in `rate-limit-guard.ts:126-144`) rather than against a deployment. The header *names* are read from that same server-side function rather than guessed, and the guard is covered by its own tests, so the risk is confined to whether a deployed edge layer rewrites `RateLimit-*` in transit — which no test in this repository could detect either way.

## 2026-09-08 - Build the front door for ChatInterpretationSeam instead of deleting it

- Date: 2026-09-08
- Decision: Ship `/describe` — a page-making surface driven by a sentence the reader types — as the first consumer `ChatInterpretationSeam` has ever had. Extract the spec-to-finished-page half of `VerdictPageState` into a new `src/lib/components/page-artifact-state.svelte.ts` (`PageArtifactState`, driven by a `PageSource`) and have both `VerdictPageState` and the new `DescribePageState` extend it. **No file under `contracts/`, `probes/`, `fixtures/`, `src/lib/mocks/`, `src/lib/adapters/` or `src/lib/seams/` is touched.**
- Context: `ChatInterpretationSeam` shipped complete in the app's first weeks — contract, mock, fixtures, probe, a 200-line pipeline, a live quota-metered endpoint at `/api/chat-interpretation`, a browser-side adapter written for a component to call, and contract tests. Measured on `main` at `7191656`: `grep -rn "chat-interpretation|chatInterpretation|ChatInterpretation" src/ --include=*.svelte --include=*.ts` returns hits only inside the seam's own folder, its pipeline, its route and its adapter. **Nothing under `src/routes/**` or `src/lib/components/**` imports any of it.** Runs 14, 15 and 16 each carried it forward as "zero consumers — costs the reader nothing and the owner money and attack surface", framing it as a deletion candidate. It is a feature that is 100% built and 0% delivered, which is the widest promise-to-delivery gap the app has left.
- Alternatives: (a) **Delete the seam, the pipeline and the endpoint.** The cheaper answer to the same measurement, and the one three runs implied. Rejected because the thing it removes is the only way this app can be *told* what to put on a page — every other surface makes the reader pick one of eight fixed questions — and the cost of keeping it is one endpoint that the same rate limiter already guards. (b) **Duplicate the page lifecycle into a new state class** rather than extract it. Rejected on two grounds: ~300 lines of it would have been copied, which is the exact duplication that left the three mode routes each missing something different, and SonarCloud's duplication gate measures new code. That gate then failed this pull request anyway, on the **tests** rather than the source — 116 near-identical scaffolding lines across the two state-class test files, 4.0% against a 3% limit — which is answered by `tests/unit/support/page-artifact-harness.ts`, the unit-test twin of the `tests/e2e/support/page-fixtures.ts` an earlier run created for the identical reason. (c) **Compose rather than inherit** — hold a `PageArtifactState` and delegate. Rejected because it needs roughly twenty-five delegation accessors to keep `VerdictPageState`'s public surface unchanged, and every one of them is a place for the two to drift; `VerdictPageState` genuinely *is* a page artifact that additionally knows how to get a verdict. (d) **Call `chatInterpretationAdapter`** from the new surface. Rejected: it uses bare `fetch` with no timeout and discards the `RateLimit-*` headers, so it cannot report a quota or bound a hung request, and fixing that is observable behaviour across a seam boundary — the full Seam-Driven Development workflow, for no gain. The new surface calls `/api/chat-interpretation` with `postJson` plus `ChatInterpretationResultSchema`, which is exactly how `VerdictPageState` already calls `/api/tools` and `/api/generate`.
- Consequences: `VerdictPageState`'s public API is unchanged and **all 50 cases in `tests/unit/verdict-page-state.test.ts` passed unmodified in commit `19ca634`**, which is the evidence that the extraction was a move and not a redesign. A later commit on this branch moves that file's stub scaffolding into `tests/unit/support/page-artifact-harness.ts`; **not one assertion, fixture value or case body changed** — the edit is the import list, the `fetch` stub's construction and the `beforeEach`/`afterEach` registration. The unmodified run is the evidence and it stands at that commit. The `chatInterpretationAdapter` now has one behavioural sibling and still no callers; it is left in place with its tests rather than deleted, because deleting an adapter is a seam change and this pull request is not one. A described page stores **no `studioText`** in the vault: `MeechieStudioTextOutputSchema` requires a `verdict` string, the reader's own sentence is not one, and `warrantForRestoredVerdict` already exists to tell a vouched-for verdict from an unvouched one — a fabricated verdict is worse than an absent one.
- Second decision: **the interpretation is shown before the generation is offered.** Two billable calls, in a fixed order, with a read-back between them: the exact title, the exact lines, the paper, the border, and the sentence they came from. This is the only surface in the app where the reader sees what was understood before paying for the picture, and it is the whole reason the round trip earns its cost — an interpretation that heard "five things" as a title-only page is a finding worth one unit, not one wasted generation.
- Third decision: `aiActionsLeft` and `describeAiQuota` take the cost and the name of the action being counted, defaulting to the studio's. One bucket funds actions of different prices — a studio rewrite is two units, a read-back is one — so a surface dividing by someone else's cost understates its own allowance. `CHAT_INTERPRETATION_QUOTA_COST` moves into `ai-quota.ts` beside `STUDIO_TEXT_QUOTA_COST`, one definition each, imported by the pipeline that charges it.
- Fourth decision, **overturned by review and recorded here as both**: the style hint was first derived from the interpreted spec alone, *not* from the reader's sentence, on the argument that the sentence is a page request ("a page that says X with roses around it") and repeating it in `styleHint` hands the image model those words a second time, in the one field that is not the exact-text block. A Codex P1 was right that this loses more than it protects: **`ColoringPageSpec` has no field that can hold "roses"**, so `illustrations: 'simple'` is all that survives of the shipped example, and the surface could not produce the page it advertises. Dropping the one thing the reader actually asked for is the promise-to-delivery gap this whole change exists to close, so the subject now rides in the hint. The original risk is answered rather than dismissed: the text is sanitized by `styleSubjectFromDescription` and introduced with an explicit instruction not to letter it, the same shape the prompt template already uses for its own blocks. **Whether a live model honours that instruction cannot be validated here** — no `XAI_API_KEY` — and it is one pure function and one call site, so reverting to the derived-only hint is a two-line change.
- The sanitizer strips **every colon**, and that is the load-bearing part rather than a tidy-up: `PROMPT_FORBIDDEN_TOKENS` is `['size:', 'quality:', 'style:']` and every entry in `RESERVED_STYLE_HINT_HEADINGS` ends in one, so "contains no colon" *proves* "contains no forbidden token" — an invariant, rather than a blocklist a new token could slip past. Without it a reader who typed "style: gothic" would have failed their own generation at the assembly seam, for a reason nothing on screen could explain.
- Fifth decision, also from that round: **the read-back shows `footerItem` where the prompt actually draws it.** `src/lib/adapters/prompt-assembly-seam/index.ts` L55 and L83-85 use `footerItem.label` as the **unnumbered second line directly under the headline**, and never read `footerItem.number` at all. The first draft rendered it last, after the list, with its number — so the reader would have approved a layout and an ordering the paid generation was never going to draw. On a surface whose entire justification is that the reader sees what was understood before paying, a read-back that misplaces a line is worse than no read-back.
- What could not be proven here: **the live quality of the model's interpretation.** `XAI_API_KEY` is not available in this container, so no live call was made and no probe was run — none was needed, since no seam changed. Every failure code the endpoint can return is covered by a stubbed-`fetch` unit test instead, and the surface is built so a bad interpretation costs one unit rather than a generation.
- Evidence, and a note about the date directory: this run's chain and transcripts are `docs/evidence/2026-09-08/run17-{lint,build,test,e2e,verify-outer}.txt`, each ending with its own exit status. Both this run and Run 16 land on **2026-09-08**, and `npm run verify` writes to `docs/evidence/<today>/` — so this run's chain necessarily refreshed the machine-generated artifacts Run 16 also cites (`chamber-lock.json`, `proof-tape.md`, `seam-ledger.md`, `clan-chain.*`, `shaolin-lint.json`, `assumption-alarm.json`, `test.txt`, `verify.txt`). That is correct rather than a silent overwrite: those files are statements about the repository *now*, and "now" moved. The files whose value is that they are old are untouched — `print-margin-before.txt`, and Run 16's hand-redirected `verify-outer.txt`, `lint.txt`, `build.txt`, `e2e.txt` and `closeout-*.txt` — so Run 16's argument is fully intact, which is why this run's own transcripts take distinct names instead of those.
- No Cipher Gate entry: a Cipher Gate is required for seam changes, and `git diff --name-only origin/main...HEAD` names no file under `contracts/`, `probes/`, `fixtures/`, `src/lib/mocks/`, `src/lib/adapters/` or `src/lib/seams/`.
- Revisit criteria: if a live key ever reaches this repository, probe the interpretation's real quality against the read-back's claims — the cautions are the part most likely to be wrong in practice, because they are written against what the model *may* return rather than against what it does.

## 2026-09-08 - Give the packaged print files a real margin, and put the geometry in core

- Date: 2026-09-08
- Decision: Move every geometric decision about paper out of `src/lib/adapters/output-packaging-seam/index.ts` and into a new pure `src/lib/core/print-layout.ts` — page sizes in points, a `PRINT_SAFE_MARGIN_MM` of 12, `planPrintPlacement`, `printCanvasPx`, `placementToPx`, `placedDpi`. The adapter becomes wiring: it holds a canvas and a PDF page and asks core where the ink goes. **No contract, probe, fixture or mock changes** — `OutputPackagingInputSchema` gains no margin field, and no schema is touched.
- Context: The packaged download is the app's deliverable and every page-making surface offers it as "Printable PDF · US Letter — ready to print". The adapter scaled by `Math.min(pageWidth / w, pageHeight / h)` and centred, which is edge-to-edge by construction. Measured on `main` at `e6c450b` (`docs/evidence/2026-09-08/print-margin-before.txt`): all six paper x source combinations landed with a **0.0pt** margin on at least one axis, and the shipped default — US Letter holding the 1024x1024 `image-generation-pipeline.ts` asks the provider for — touched both vertical edges. No consumer printer can mark within ~6.4mm of the sheet edge, so printing that at 100% clips the outer border the spec draws, and "shrink to fit" silently rescales instead. `src/routes/+layout.svelte` had already recorded the defect in a comment beside the browser print path — *"The packaged PDF bleeds its image to all four edges; this does not"* — and left it.
- Alternatives: (a) **A margin field on the contract.** Rejected: it is a contract change, which `AGENTS.md` names as a condition for not merging without asking, in exchange for a knob nothing would set — there is one right answer for a coloring book and it should not be a caller's decision to get wrong. (b) **Leave the PDF and change only the `@page` rule to match it (zero).** That is the wrong direction: it would make both paths clip. (c) **Crop the artwork to page proportions** rather than letterbox it. Rejected: a coloring page that has been cropped has lost part of the picture, which is worse than a smaller one. (d) **A margin only on the PDF, leaving the print PNG alone.** Rejected: the two print artifacts would then be two different layouts of one page.
- Consequences: The printed picture is smaller — on US Letter the shipped square goes from 612pt to 543.97pt across, about 11% — and that is the trade taken deliberately: whole and slightly smaller beats larger and clipped. **12mm is not a new number**; it is what `@page { margin: 12mm }` already reserved for the browser print path, so the two paths now agree, and `tests/unit/print-layout.test.ts` reads the stylesheet and fails if they drift. A4 is now the real 595.276 x 841.890pt (210 x 297mm) rather than the adapter's `595 x 842` rounding, which was invisible while the artwork covered the sheet and stops being invisible once the safe box is derived by subtracting from it.
- Second decision, and the one most likely to be challenged: **the `print` PNG variant no longer passes a PNG source through untouched.** It used to return the provider's own bytes for `format === 'png'` while letterboxing JPG and WebP onto a fixed 2550 x 3300 canvas — so the same coloring page was 1024 x 1024 or 2550 x 3300 depending only on which format the provider answered with, and only one of those is page-shaped. It is now a full sheet at 300dpi for every source format, with the same margin the PDF uses. This is lossless for the reader: `describeOriginalImageExport` already offers the untouched provider bytes as a separate download ("exactly what the generator sent"), and it is untouched by this change.
- Third decision: `transcodeToPngBase64` no longer resizes. Converting a JPG or WebP to PNG for embedding used to letterbox it onto 2550 x 3300 first, which baked white bars into the image that the PDF then letterboxed a second time. Where the artwork sits on the paper is `planPrintPlacement`'s decision and it can only make it correctly if what it is given is the picture rather than the picture inside a previous layout.
- Self-critique, and the finding that came out of it: **the first draft of the red proof proved nothing.** `print-layout.test.ts` asserted the margin invariant against `PRINT_SAFE_MARGIN_PT` — the constant under test — so setting `PRINT_SAFE_MARGIN_MM` to 0 moved both sides of every comparison at once and fourteen margin tests went on passing on a layout that bleeds to the edge. Only 4 of 27 failed. The invariant is now measured against an independently stated `HARDWARE_CLEARANCE_PT` (0.25in, the printer's physical limit), and the same edit fails **20 of 27**. Run 15's close-out had already written the general lesson down and this run had to learn it again on its own tests.
- Second self-critique: moving the canvas creation inside `image.onload` — needed because two callers size the canvas from the image's intrinsic dimensions — removed the early `getContext` bail-out, and `Image` never fires `load` in jsdom, so **20 of this repo's existing tests hung to a five-second timeout each** instead of returning `CANVAS_UNAVAILABLE`. The canvas and its context are now taken up front and only resized in `onload`. Caught by the existing suite, not by review.
- Also fixed, found while wiring: `toImageDataUrl` reported a recognised format with a non-base64 payload as `UNSUPPORTED_IMAGE_FORMAT`, which names the wrong thing — the format is supported, the encoding is not. It now returns the same `*_ENCODING_UNSUPPORTED` codes `imageToPngBase64` already used.
- Revisit criteria: if the provider ever returns page-proportioned images, `planPrintPlacement` will letterbox less and the top and bottom margins will shrink to the safe margin on their own — no change needed. If a reader ever needs borderless output for a printer that supports it, that is a contract change and a different decision from this one.
- Evidence, with its status stated rather than implied — a review round found `build.txt` and `lint.txt` two commits behind the rest while the list read as one run, which is the kind of thing this repository's whole evidence discipline exists to prevent:
  - **Current** — regenerated on the head this entry describes: `docs/evidence/2026-09-08/verify-outer.txt` (chain exit=0), `test.txt`, `lint.txt`, `build.txt`, `e2e.txt` (68 passed), and the machine-generated chain artifacts beside them.
  - **Deliberately historical** — `docs/evidence/2026-09-08/print-margin-before.txt` measures `main` at `e6c450b`, *before* this change. It is the case against the old behaviour and must not be regenerated on the new head; doing so would delete the only record of what was wrong.
  - Tests: `tests/unit/print-layout.test.ts`, `tests/unit/print-packaging.test.ts`, `tests/e2e/print.spec.ts`

- Cipher Gate:
  - Date: 2026-09-08
  - Seams: OutputPackagingSeam
  - Evidence: docs/evidence/2026-09-08/verify-outer.txt; docs/evidence/2026-09-08/chamber-lock.json; docs/evidence/2026-09-08/proof-tape.md; docs/evidence/2026-09-08/seam-ledger.md; docs/evidence/2026-09-08/print-margin-before.txt; docs/evidence/2026-09-08/e2e.txt
  - Summary: The adapter's inline page geometry moved to a pure `src/lib/core/print-layout.ts`; every `print` artifact (PDF and PNG) is now placed inside a 12mm safe margin — the same margin the browser print path already reserved — instead of scaled edge to edge. The contract, probe, fixtures and mock are unchanged; only where the adapter draws changed.
  - Risks: The printed picture is ~11% smaller across on US Letter, which is the deliberate trade for a page that prints whole. The `print` PNG for a PNG source is now a rendered sheet rather than the provider's bytes, so it requires a canvas where it previously did not — three existing tests asserted that passthrough and were rewritten to the new intent, and the untouched bytes remain available as the `original` download. Sizeless SVG sources still rasterise at the same 2550 x 3300 fallback, now derived from core rather than hardcoded.

## 2026-09-06 - Show `qualityState` and `revisionNote` without changing the seam contract, by carrying provenance beside the value

- Date: 2026-09-06
- Decision *(this statement is the final invariant, rewritten after review; the two corrections below record how it got here and are the more instructive half)*: Render the two studio-text fields nothing consumed — `qualityState` and `revisionNote` — through a new pure `src/lib/core/verdict-report.ts`, and take the fact of a standing having been *reported* as a **separate input** (`standingWasReported`) rather than as a new contract field. `StudioState` owns the provenance in `verdictWarrant: 'reported' | 'stored' | 'derived' | 'none'`, and the two questions it governs are answered independently: **only `'reported'` may be claimed as a standing**, while **only `'derived'` is withheld from storage** (words rebuilt from `intent`, so storing them adds nothing but the invented `'ready'`). `'stored'` text is somebody's real words and is written back, standing or no standing. `'reported'` covers a live response *and* a record carrying `modelMetadata`, the stamp `parseProviderText` attaches to every accepted studio response and that neither inventing producer can forge (`warrantForRestoredVerdict`) — so a `blocked` verdict survives the autosave-and-refresh that would otherwise have deleted its own warning. There are exactly two writers, `setVerdict` (a verdict plus a required warrant) and `clearVerdict` (nothing), and `textOutput` is a getter, so no caller can produce an inconsistent pair. No file under `contracts/`, `probes/`, `fixtures/`, `src/lib/mocks/`, `src/lib/adapters/`, `src/lib/seams/` or `tests/contract/` changed.
- Context: `MeechieStudioTextOutputSchema` makes `qualityState` **required**, and `buildStudioTextFromSpec` must therefore write `'ready'` into text it rebuilds from a page that stored none. The moment the field reaches the screen, that invented value becomes an on-screen claim that Meechie approved a page she never saw — and a re-save would write the claim into the record permanently. The value alone cannot say whether anyone reported it; something beside it has to.
- Alternatives: (a) Add `'unstated'` to `MeechieStudioQualityStateSchema` — the shape the data actually wants, and rejected for two reasons: it is a contract change, which `AGENTS.md` names as a condition for not merging without asking, and it would make every existing stored record's `'ready'` indistinguishable from a reported one anyway, since those records are already written. (b) Make `qualityState` optional — same contract problem, plus it would silently reclassify every record already in a reader's browser. (c) Store the provenance in the record — a schema and data migration for a fact that is only interesting while the verdict is on screen. (d) Infer it from whether the record carried `studioText` at the point of *render* rather than of *load* — the component would then be re-deriving a fact the loader already knew, which is the second-copy-of-a-truth failure this log has recorded in four separate runs.
- Consequences: The card claims a standing only for a live response, and a record that stored no studio text keeps storing none rather than acquiring an invented approval. That exclusion is only safe because every field `buildStudioTextFromSpec` produces is derived from `intent` — title, items and `specOwnQuote` — so the round trip is lossless; a test asserts exactly that, and it is the test that fails if that function ever grows an input it cannot rebuild from the record. A record that *did* store studio text keeps it on every resave, standing or no standing, which is the half a boolean got wrong. `textOutput` is read-only from outside, which narrows a previously fully-writable public field; `acceptVerdict` is the one public writer, and it is the operation the studio actually performs.
- Second decision, recorded here because it is the one a reviewer is most likely to challenge: **a flagged standing does not disable the page button.** A model that over-used `blocked` would otherwise switch the studio off for its readers, and the reader — not the app, and not the model — owns the decision about their own page. What they were owed is the warning, which now sits directly above the button that spends the generation.
- Self-critique: The riskiest assumption is that `revisionNote` is worth surfacing at all — if a live model fills it with filler, the card gains a permanent line of noise. It cannot be settled from here: `XAI_API_KEY` is not available in this container, and `fixtures/meechie-studio-text/sample.json` is hand-built (`"provider": "fixture"`), so it is evidence of the shape and not of the content. The design is therefore built to survive a useless note — it is attributed to Meechie rather than presented as the app's advice, it gates nothing, and a note that is absent or whitespace-only renders nothing at all. The second assumption is that no consumer depended on assigning `studio.textOutput`; checked by `grep`, which finds sixteen sites, all in `tests/unit/studio-state.test.ts`, all converted to `acceptVerdict`.
- Corrected under review (PR #313, `8550ef8`), and the correction is the more useful half of this entry: **the provenance flag above was a boolean, and it was answering two questions.** *May the card claim a standing?* and *should this text be written back into the record?* have different answers, and `creation.studioText !== undefined` gave the right one to both only for the cases I had in mind — all of them from the home studio. Codex pointed at the toolkit, which writes to the same store: `buildToolStudioText` (`src/lib/core/tool-page-recipe.ts`) must write `qualityState: 'ready'` because the schema requires it and `MeechieToolOutput` has no such field, so every toolkit page ever saved would have reopened saying "Meechie had enough to work with". Three kinds of record carry a `qualityState` and only one carries a reported one — live response, toolkit placeholder, pre-change reconstruction — and **nothing on a record separates them**, so the fix is not to detect the toolkit case: no restored standing is believed at all. The flag became three-valued, and each value answers the two questions independently: claim-and-store, store-only, neither. The first version of this correction went further than it had to and said no restored standing is believed at all — see the third correction below, which found the discriminator that was there the whole time.
- Second correction from the same review, and the same defect one field over: **`rating` means opposite things in the two seams and one was being copied into the other.** The studio's is severity (*"1-10 severity. Be honest. Don't undersell it."*); Rate This Excuse's is credibility (*"1 means insulting everyone in the room. 10 means barely credible."*). `buildToolStudioText` copied it, which predates this change and stayed quiet because nothing ever said what the number meant — until this change labelled it in words. Fixed at the producer (it no longer copies; nothing is lost, since that tool's headline *is* the score) and at the card, for records already saved: `readVerdictSeverity` returns `null` when the verdict *is* the score, derived from the record's own two fields rather than from a list of tool ids. The lesson generalises past this field: **a label does not create a semantic mismatch, it makes an existing one legible** — which is an argument for labelling, and an obligation to fix what the label exposes.
- On whether the stored-`rating` change is a `CreationStoreSeam` change (raised as a P1 on PR #313, and declined with the measurement rather than with an opinion): **no artifact of that seam describes `rating`, and none is made stale by this.** `grep -rn "rating"` over `contracts/creation-store.contract.ts`, `src/lib/seams/creation-store-seam/` (contract, probe, fixtures, mock, test), `src/lib/adapters/creation-store-seam/`, `src/lib/adapters/creation-store.adapter.ts`, `src/lib/mocks/creation-store.mock.ts` and `tests/contract/creation-store.test.ts` returns **nothing**. The seam reaches `studioText` only as `MeechieStudioTextOutputSchema.optional()`, in which `rating` is optional and unchanged, and the seam's own behaviour — store what it is given, return it — is untouched. What changed is what a *pure core function composes* before handing a record over, and if that alone were a seam change then so would be the `describingStudioText` exclusion in the same commit, and Run 5's fix for laundering seed items into records, and any edit to what the app decides to save; the rule would have no boundary. Compare Run 8, which did run the full workflow, and had to: it *added a field to the contract*. The substantive half of the finding — what happens to records already written — is answered by test rather than by argument: a Rate This Excuse page saved in the old shape still validates, still loads, keeps its `rating` on a resave, and has the number suppressed rather than relabelled (`reopens a toolkit page saved before the producer was fixed`).
- Third correction, from the same reviewer on `d4b95cd`, and it recovered a feature the second correction had over-corrected away: **"believe nothing restored" deleted the warning on every refresh.** The studio autosaves a draft as the reader works, so a `blocked` verdict came back from a reload classified as merely `'stored'` — standing and page caution gone, at the one moment the reader most needed them. The discriminator was there all along and I had rejected the *idea* of one on the grounds that inferring provenance from a field's presence is the mechanism this log condemns. That objection does not apply here: `modelMetadata` is not a field the model may or may not fill, it is a stamp `parseProviderText` attaches to **every** accepted studio response in the pipeline, and it is structurally absent from both inventing producers — `buildToolStudioText` composes its object without one, and `buildStudioTextFromSpec` rebuilds from `intent`, which has no such field. Its presence proves a studio response wrote the text; its absence proves nothing, and is read as `'stored'` rather than as doubt. The lesson is narrower than "never infer from shape": **a stamp your own pipeline applies unconditionally is evidence; a value your source may choose to include is not** — and rejecting a whole class of reasoning cost this run a feature until a reviewer looked again.
- Revisit criteria: If `MeechieStudioTextOutputSchema` ever gains a way to express "no standing reported", `verdictWarrant` and the `standingWasReported` input should be deleted in favour of it, and `describingStudioText`'s exclusion with them. If `MeechieToolOutput` ever gains a real `qualityState`, the `'stored'` case can be split again and a toolkit page's standing believed.
- Evidence: docs/evidence/2026-09-06/verify-outer.txt (chain exit=0); docs/evidence/2026-09-06/test.txt; docs/evidence/2026-09-06/lint.txt; docs/evidence/2026-09-06/build.txt; docs/evidence/2026-09-06/e2e.txt; tests/unit/verdict-report.test.ts; tests/unit/studio-state.test.ts
...
- Assumption:
  - Date: 2026-09-03
  - Seams: None registered - response headers on filesystem-served paths (static assets from the outset; the fourteen canonical prerendered documents since #311)
  - Statement: The `headers` rules in `vercel.json` attach X-Content-Type-Options / X-Frame-Options / Referrer-Policy - and since #311 on the document rules also Permissions-Policy - to the paths those rules name: the static assets and the fourteen canonical prerendered documents. Not to every filesystem-served path; the directly served `.html` filenames match no rule and are outside this Assumption (see the known gap below). Those four are what depends on this Assumption. Strict-Transport-Security is listed on the document rules too but is not contingent on them, because Vercel's edge sets it independently.
  - Scope widened 2026-09-06 by the offline work in #311, which is why this entry is now load-bearing rather than tidy. When it was written the filesystem-served paths were assets only, and every document still came from the SvelteKit function, where `hooks.server.ts` attaches the headers in code a test can run. Prerendering `/`, `/offline`, `/meechie`, `/who-fucked-up`, `/rate-his-excuse`, `/random` and `/m/[mode]` makes those documents files, and Vercel's `{"handle":"filesystem"}` precedes the SSR rewrite, so the security headers on the **fourteen canonical prerendered documents** now come from the `vercel.json` rules. Not every page: `/m/[mode]` is `prerender = 'auto'`, so the five `SLUG_ALIASES` - `/m/receipts`, `/m/caption-this`, `/m/apology-translator` among them - still resolve through the function and still get their headers from `hooks.server.ts`, as does the catchall that renders `+error.svelte`. `tests/unit/security-headers.test.ts` asserts that every canonical prerendered path matches a rule, and that three of the five aliases - `/m/receipts`, `/m/caption-this`, `/m/apology-translator` - match **none**, since a header set twice is worse than one set once. Three of five: `/m/rate-his-excuse` and `/m/what-would-meechie-do` are not in that list, so a rule accidentally matching either would not fail the guard. Adding them is a one-line test change and is not made here because this is a documentation change. A future header change therefore has to touch both files, which is the trap this note exists to keep open. Whether Vercel then applies the rules is this Assumption. Exactly four headers are contingent on it - X-Content-Type-Options, X-Frame-Options, Referrer-Policy and Permissions-Policy. Strict-Transport-Security is not: Vercel's edge already sets it independently, which is what the 2026-09-03 entry above records finding in production, so it survives these rules failing to apply. An earlier draft of this note said all five came from the rules "and from nothing else", which overstated the exposure.
  - Found while correcting that, and not fixed here because this is a documentation change: the new document rules **do** list Strict-Transport-Security, while the `src/hooks.server.ts` header comment states that the `vercel.json` sources omit it precisely because the edge sets it. That was true of the asset rules it was written about and is now false of the document rules #311 added. Nothing breaks - a repeated HSTS is not the hazard a repeated X-Frame-Options is, and the hook no longer serves these paths - but the comment and the config disagree, and one of them should change. Recommended, and it is two changes rather than one: `tests/unit/security-headers.test.ts` iterates `SECURITY_HEADERS` from `src/hooks.server.ts` - all five - and requires every one on each of the fourteen prerendered documents, so dropping the entries alone turns CI red. First split the expected set so Strict-Transport-Security is asserted on the hook's own paths and not required of the `vercel.json` document rules, then remove those three entries. That restores the documented rationale rather than editing the rationale to match a drift.
  - Validation: Not exercised locally - `vite preview` does not read `vercel.json`, so no local run can confirm it. Verification against the pull request's own Vercel preview was attempted and is also impossible: the preview is behind deployment protection and every request 302s to `vercel.com/sso-api`, so no response body or header of the app itself is reachable unauthenticated. The gap being closed is nonetheless confirmed on live production, which does not yet carry this change - `https://meechiescoloringbook.vercel.app/service-worker.js` returns 200 without any of the three headers this repository sets on assets. Not without every security header: Strict-Transport-Security arrives from the edge there as it does everywhere, which is what the Context above records as the one header the deployed app was sending. The 2026-09-03 Self-critique higher in this file still carries the looser "no security headers at all" phrasing about that same measurement; it is left as written because that entry is history, and this is the field a future run reads. Validate after this merges and deploys to production, not on a preview: request `/service-worker.js` and an `/_app/immutable` asset from `https://meechiescoloringbook.vercel.app` and confirm X-Content-Type-Options / X-Frame-Options / Referrer-Policy are present. Confirming these says nothing about `/offline.html` and its siblings - those match no rule and this Assumption does not cover them. Since #311, also request the documents `/`, `/offline` and `/m/clapback` and confirm the four contingent headers on each - X-Content-Type-Options, X-Frame-Options, Referrer-Policy and Permissions-Policy - since those are prerendered now and served by the same filesystem handler. Strict-Transport-Security is expected on them either way and proves nothing about these rules. For a control, request an alias such as `/m/receipts`: it is server-rendered, matches no `vercel.json` rule, and must carry all five from the hook - so if the four are present there and absent on `/m/clapback`, the rules are what failed rather than the deploy.
  - Known gap in the rules themselves, found in review on 2026-09-06 and not closed by this validation: the build emits `.vercel/output/static/index.html`, `offline.html` and `m/<slug>.html`, and `{"handle":"filesystem"}` serves those URLs directly. The `vercel.json` document rules name only the extensionless forms (`/`, `/offline`, `/m/clapback`) so a direct request for `/offline.html` matches no rule and arrives without the four contingent headers - X-Content-Type-Options / X-Frame-Options / Referrer-Policy / Permissions-Policy. Not without any: Strict-Transport-Security still comes from the edge. The same hole this entry was opened for, one path shape over, and four headers wide rather than five. This Assumption is therefore scoped to the canonical routed documents and does **not** cover the `.html` URLs; closing it says nothing about them. Fixing that is a `vercel.json` change plus the matching assertion in `tests/unit/security-headers.test.ts`, which today derives its paths from the routes and so cannot see the emitted filenames either.
  - Status: Open - pending first deploy.
...
## 2026-09-09 - Stamp each classified failure with an order, rather than pick System Trace's diagnostic by a fixed field order

- Date: 2026-09-09
- Decision: `StudioState` writes a `studioFailureStamp` property onto every failure it classifies, and `traceFailureDetail` renders the detail of the newest failure still held by one of `pageFailure`, `textFailure` and `tryOnFailure`. This replaces `failureDetail={studio.pageFailure?.detail ?? studio.textFailure?.detail ?? null}` at the call site in `src/routes/+page.svelte`. The stamp is deliberately **not** added to `GenerationFailure` in `src/lib/core/generation-failure.ts`.
- Context: Wiring the wig try-on into the classifier gave the home studio a third failure field, and the existing `??` chain made the flaw in the two-field version impossible to ignore. The three fields are not mutually exclusive: `runTextAction` clears `textFailure` and not the other two, and `resetTryOnPageState` clears `tryOnFailure` and `pageFailure` and not `textFailure`. So a text failure after a try-on failure leaves both set, and a try-on failure after a text failure leaves both set. Any fixed order names the stale one in exactly one of those two sequences, and System Trace is the panel a reader opens to find out what just went wrong.
- Alternatives: **(a) Extend the `??` chain to three.** Rejected: it does not answer the question, it just adds a third way to answer it wrongly. **(b) Keep a `lastClassifiedFailure` field and compare it back to the three with `===`.** Written first, and it does not work: `$state` deep-proxies assigned objects, so two fields holding one object return two different proxies and the identity check is always false. Svelte reports this as `state_proxy_equality_mismatch`, and the test written for the behaviour caught it before the change was pushed. Recorded in `LESSONS_LEARNED.md`. **(c) Add the stamp to `GenerationFailure` itself.** Rejected: the ordering question belongs to this studio alone — it is the only surface in the app with three independent failures live at once — and every other surface would carry a field it never writes and never reads. **(d) Three parallel `$state` stamp fields, one per failure.** Rejected: they would have to be kept in step with the many `pageFailure = null` writes scattered through this class, and the first one missed would be a silently wrong panel. A stamp carried on the value needs no synchronising, because a field holding nothing is skipped and a field holding a failure still carries its own.
- Consequences: One property exists on these objects that is not in the contract type, declared as a module-local `StampedFailure` and read back through a total `stampOf` helper that answers `0` for a failure that never went through `recordFailure` — which is only reachable by assigning one directly, as two existing tests do. `>` rather than `>=` in the reducer means such an unstamped failure loses to earlier entries rather than displacing them. All three classify helpers now go through one `recordFailure`, so they cannot each keep their own idea of what "most recent" means. A string key rather than a symbol, because the value is read back through a `$state` proxy and a plain property read is the one operation a proxy is guaranteed not to alter.
- Self-critique / seam-workflow scope: `AGENTS.md` says to treat any doubt as a seam change, so the doubt was measured rather than waved off. `git status` shows no file under `contracts/`, `probes/`, `fixtures/`, `src/lib/mocks/`, `src/lib/adapters/` or `src/lib/seams/` in this change. `/api/wig-try-on` and `runWigTryOnPipeline` are read and not modified: every route code the reader now gets a proper sentence for is one the pipeline already emitted, and `CAUSE_BY_CODE` already mapped all eight of them. No Cipher Gate entry is claimed, because no seam artifact changed — recording one would assert a workflow this change did not need and did not run.
- Revisit criteria: If a fourth failure field is added to this studio, or if any surface other than the home studio grows more than one live failure, the stamp belongs on `GenerationFailure` and this exception should be removed rather than copied. If `GenerationFailure` ever gains a timestamp for another reason, use that instead of a second ordering mechanism.

## 2026-09-09 - Let the wig try-on's retry read the live controls, instead of pinning the attempted request

- Date: 2026-09-09
- Decision: `retryWigTryOn` re-runs `handleWigTryOn`, which reads `selectedWig` and `selfieBase64` as they are now. It does **not** pin the attempted wig and selfie in a `lastAttempted…` field, which is what `page-artifact-state.svelte.ts`, `verdict-page-state.svelte.ts` and `describe-page-state.svelte.ts` all do.
- Context: The rule those three follow — a retry re-asks for the thing that failed, never for whatever is live now — is right, and it is right here too. The disagreement is only about how it is enforced. `setTryOnFailure` refuses to display a failure whose wig is no longer selected or whose selfie has been replaced; both guards predate this change and exist because a try-on can take two minutes and the carousel and the photo input stay live throughout. So a *visible* try-on failure is, by construction, a failure of exactly what those two controls hold now, and reading them is not a shortcut past the rule but the same answer arrived at from the guard that was already there.
- Alternatives: **(a) Pin the wig and selfie anyway, for symmetry with the other three surfaces.** Rejected: `selectedWigId` is `$derived` from `selectedWig` rather than stored, specifically so there is one source of truth for which wig is on screen, and a pinned copy would be a second one free to disagree with it. The cost of the symmetry is the class of bug the derivation was written to prevent. **(b) Remove the display guards and pin instead.** Rejected: the guards do more than this retry needs — they also stop a late failure appearing under a wig the reader has moved to — and trading a broader guarantee for a narrower one to match a pattern is the wrong direction.
- Consequences: The correctness of the retry now depends on a guard in a different method, which is a real coupling and is why both `retryWigTryOn` and `setTryOnFailure` say so in their own comments and reference each other. A test fails a try-on, retries, and asserts the second request's `wigId` is the wig that failed, so relaxing the guard breaks a test rather than only a comment.
- Revisit criteria: If `setTryOnFailure`'s two guards are ever loosened — in particular if a failure is allowed to remain on screen after the wig changes — this retry must gain a pinned request in the same change, because the guarantee it rests on will have gone.
