...
- Date: 2026-07-09
- Decision: Add a new self-contained `RateLimitSeam` (fixed-window counter, pure algorithm over an injected store + clock) and wire it into `/api/generate`, `/api/image-generation`, `/api/chat-interpretation`, `/api/meechie-studio-text`, `/api/wig-try-on`, and `/api/tools` via a shared `enforceRateLimit` guard keyed by `${routeName}:${clientAddress}`. Excess requests receive `429 RATE_LIMIT_EXCEEDED` with a `Retry-After` header.
- Context: A repo-wide audit found none of the provider-backed routes had any authentication or request budget. `/api/generate`, `/api/image-generation`, `/api/chat-interpretation`, `/api/meechie-studio-text`, and `/api/wig-try-on` are public, unauthenticated, and call metered xAI/Gemini APIs directly from server code with real API keys. `/api/tools` was initially assumed to be deterministic/template-only and left out of this change, but automated PR review on #226 (Codex) correctly caught that the canonical self-contained `MeechieToolSeam` adapter (`src/lib/adapters/meechie-tool-seam/index.ts`) also calls `provider.createChatCompletion` against the real xAI provider — it is billed identically to the other text routes, so it was added to the guarded set in the same PR. Any caller — a script, a bot, a competitor — could hit these endpoints without limit and drive unbounded provider billing or exhaust the endpoint. No `hooks.server.ts` and no prior rate limiting existed anywhere in the app.
...
- Cipher Gate:
  ...
  - Summary: Added RateLimitSeam (contract/validators/fixtures/mock/probe/test + real in-memory adapter) and wired a shared `enforceRateLimit` guard into all six provider-billed API routes (including `/api/tools`, added after review feedback) with per-route request budgets.
...
- Follow-up (2026-07-09, same day, addressing automated PR review feedback on #226): The in-memory `RateLimitStore` had no eviction, so a long-lived instance seeing many distinct client keys (or an attacker spraying spoofed IPs) could grow it without bound. `evaluateRateLimit` in `src/lib/core/rate-limit.ts` now caps the store at `MAX_RATE_LIMIT_STORE_ENTRIES` (5000): when a brand-new key needs tracking at capacity, it evicts an already-expired entry if one exists, falling back to FIFO eviction of the oldest-inserted key otherwise. Also wired the previously-unused `validateRateLimitCheckValue` into both the adapter and mock so seam output is validated against the contract like the input already was, and added `resolveClientAddress` in `src/lib/server/rate-limit-guard.ts` so a throwing `getClientAddress()` (some proxy/adapter configurations) falls back to a shared `'unknown'` bucket instead of crashing the request with a 500. Evidence refreshed under `docs/evidence/2026-07-09/`.
