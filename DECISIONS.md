...
## 2026-06-23 - Port RateLimitSeam plus SVG/WebP image-detection from PR #171 and #180 to close unaddressed review threads

- Date: 2026-06-23
- Decision: Add a new self-contained `RateLimitSeam` (sliding-window limiter, throttled idle-key sweep with backward-clock-drift guard, `resetAtMs` computed from the minimum live hit on both the block and allow paths) plus `src/lib/server/rate-limit-guard.ts`, wire it into all 6 metered AI-provider routes (`generate`, `image-generation`, `chat-interpretation`, `meechie-studio-text`, `tools`, `wig-try-on`); add the seam's Zod schemas to `contract.ts` and a scenario-aware `mock.ts` that loads fixtures instead of aliasing the production limiter; and separately port `image-generation-pipeline.ts`'s SVG/WebP image-format detection (length-capped, non-backtracking regex) and `studio-state.svelte.ts`'s mimeType-fallback plus draft-load-guard fix.
- Context: Among PRs opened in the last 5 days, PR #171 and PR #180 carried the most unaddressed review threads. Both implement the same rate-limiting feature as sibling branches, and PR #171 also carries an SVG/WebP image-format-detection feature that PR #180 deliberately excluded as "out of scope" because it didn't exist on `main`. Diffing each branch's tip against current `main` showed nearly all substantive feedback (sweep throttling, the `resetAtMs` ordering bug under clock rollback, missing Zod schemas, the mock bypassing fixtures, idle-key eviction, missing `/api/tools` coverage, a weak fallback-bucket test, a `console.warn` process-I/O violation, stale evidence, ReDoS in the SVG sniff regex, a missing mimeType fallback for old saved drafts, and a `draftLoaded` flag that could get stuck `false` forever on a rejected init promise) had already been fixed within each PR's own commit history; what was missing was that neither branch's converged state existed on `main`.
- Alternatives: Merge PR #171 or #180 directly; rejected because each still carries its own unrelated unresolved nitpicks (CodeRabbit `.jsonc`-for-evidence suggestions) and neither branch's tip has both fixes converged together. Keep PR #180's exclusion of the SVG/WebP feature; rejected here because that feature's own ReDoS and mimeType-fallback review comments are squarely "unaddressed review comments on a PR opened in the last 5 days" per this task's scope, and `contracts/image-generation.contract.ts` already requires `mimeType`/`format` for non-PNG/JPEG images, so detecting WebP/SVG is a real current gap rather than scope creep. Re-derive every fix from scratch against each PR's HEAD; rejected once diffing showed both branches had already converged on corrected implementations.
- Consequences: Six routes now enforce per-route sliding-window budgets via `enforceRateLimit`; RateLimitSeam has full contract/Zod-schema/mock/fixtures/test coverage with a fault scenario for consumer testing; image-generation responses correctly tag WebP and BOM/XML-prologue SVG payloads instead of defaulting them to PNG; and saved drafts predating the `mimeType` field render correctly instead of producing `data:undefined;base64,...`.
- Revisit criteria: Revisit if PR #171/#180 are closed in favor of this port, if rate limits need to move from per-process memory to a shared store, or if a future review surfaces a case the `draftLoaded`/`initialized` guard in `studio-state.svelte.ts` doesn't cover.
- Plan:
  - Goal: Close the two PRs with the most unaddressed review comments by porting their converged, genuinely-still-valid fixes into one PR against `main`.
  - Seams: RateLimitSeam.
  - Files: `src/lib/seams/rate-limit-seam/{contract.ts,validators.ts,limiter.ts,fixtures.ts,mock.ts,probe.ts,test.ts}`, `src/lib/server/rate-limit-guard.ts`, `tests/unit/rate-limit-guard.test.ts`, `src/routes/api/{generate,image-generation,chat-interpretation,meechie-studio-text,tools,wig-try-on}/+server.ts`, `src/lib/core/image-generation-pipeline.ts`, `tests/unit/image-generation-pipeline.test.ts`, `src/routes/studio-state.svelte.ts`, `docs/seams.md`, `src/lib/seams/CLAUDE.md`, `DECISIONS.md`, `docs/evidence/2026-06-23/*`.
  - Commands: `npm run check`, `npm test`, `npm run lint`, `npm run build`, `npm run rewind -- --seam RateLimitSeam`, `npm run verify`.
- Self-critique: The main risk is treating a thread as "already addressed" from reading a branch's tip rather than re-verifying against this repo's own suite; covered by running the full local check/test/lint/build/verify chain against the ported code. A second risk is overriding PR #180's own explicit scope exclusion of the SVG/WebP feature; that override is deliberate and recorded here rather than silent, on the grounds that the feature's review comments are themselves in-scope unaddressed comments on a PR opened in the last 5 days, and the contract already requires the fields this code now populates.

- Cipher Gate:
  - Date: 2026-06-23
  - Seams: RateLimitSeam
  - Evidence: docs/evidence/2026-06-23/chamber-lock.json; docs/evidence/2026-06-23/verify.txt; docs/evidence/2026-06-23/test.txt; docs/evidence/2026-06-23/seam-ledger.json; docs/evidence/2026-06-23/seam-ledger.md; docs/evidence/2026-06-23/clan-chain.json; docs/evidence/2026-06-23/clan-chain.md; docs/evidence/2026-06-23/proof-tape.json; docs/evidence/2026-06-23/proof-tape.md; docs/evidence/2026-06-23/shaolin-lint.json; docs/evidence/2026-06-23/assumption-alarm.json; docs/evidence/2026-06-23/rewind-RateLimitSeam.txt
  - Summary: Added self-contained RateLimitSeam (Zod-schema contract, scenario-aware mock, throttled idle-key sweep, clock-rollback-safe `resetAtMs`) wired into all 6 metered AI-provider routes, plus SVG/WebP image-format detection and a saved-draft mimeType fallback, closing the in-scope unaddressed review threads from PR #171 and PR #180; `npm run verify` passed clean (svelte-check 0 errors/0 warnings, 553 tests passed/1 skipped, lint/build green).
  - Risks: Rate limiting remains per-process in-memory state, so it will not coordinate across multiple server instances/regions. `enforceRateLimit` runs before request-body validation on all 6 routes by design, so a client's own malformed request still burns one slot of its own budget even though no provider call was made; this is judged a minor, self-inflicted cost against a single client's own quota rather than a cross-client abuse vector. CodeRabbit's `.jsonc`-for-evidence-files nitpicks on both source PRs are intentionally left unaddressed as low-value churn on auto-generated, dated evidence snapshots that aren't part of this feature.
