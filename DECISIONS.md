...
## 2026-09-03 - Drop the `tier` field from the Meechie quote contract

- Date: 2026-09-03
- Decision: Remove `tier` from `MeechieQuoteSchema`, along with `MeechieQuoteTierSchema`, the `MeechieQuoteTier` type, and its re-export from both voice contracts. A quote is now `{ id, text }`, still `.strict()`. The ten canon lines, their ids and their wording are untouched, and `meechieVoicePack.version` stays `v4`.
- Context: `tier` was `'canon' | 'approved'` - provenance for humans, never behaviour. Both values fed the prompt identically, in the same order, and no code ever read the field. PR #227 introduced it when the list held 5 canon and 15 approved lines. `bc5c674` then cut the ten fabricated `approved` lines, and from that moment every surviving line was canon: a field with one possible value that nothing reads. That is the same dead-metadata shape (`category`, `rawness`, `thirdPersonUsage`, `modeFit`, `defaultMode`, `coloringPageReady`, `visualMotifs`) PR #227 existed to remove, and leaving one behind invites the next reader to attach meaning to it. Originally proposed in PR #230, which is closed as superseded - its other changes all landed by other routes, this one did not.
- Alternatives: Keep `tier` for provenance; rejected because provenance is a comment, not a field, and the voice pack's header now carries it in prose. Make `tier` optional; rejected as the worst of both - the field survives in the type, readers still infer meaning from it, and `.strict()` stops guarding its return. Bump `version` to `v5` as PR #230 did; rejected because that PR bumped it for a change that also cut the line list, whereas here `tone.samples` still derives from `MEECHIE_LINES.map((line) => line.text)` and the ten strings are byte-identical - the system prompt the provider sees does not change, so a version bump would signal a voice change that did not happen.
- Consequences: Any voice pack still carrying `tier` is now a validation error rather than a silently ignored field, because the schema stays `.strict()`. That is the intended failure mode and it is guarded two ways: an inline case (`'the retired tier field'`) and the third bad quote in `fixtures/meechie-voice/malformed-pack.json`, which was previously "a tier that does not exist" and is now "a line still carrying the retired tier field" - the same fixture, repointed at the removal it should guard. If a second class of line is ever ruled in, the distinction goes back into the contract deliberately rather than by reviving a label.
- Collapsed mirror (forced by this change): `contracts/meechie-voice.contract.ts` and `src/lib/seams/meechie-voice-seam/contract.ts` were byte-identical 128-line files. Removing `tier` required the same one-line edit in both, and SonarCloud read that as 6.1% duplicated new code against a 3% limit - a real failure of this pull request, not scan noise. PR #230 predicted exactly this ("while the two contract layouts stay byte-identical, *any* future change touching both trips that gate") and deferred the fix. Deferring again was not available: the gate is red until the duplication goes. The flat file is now an 8-line `export * from` the canonical self-contained contract, which is the same collapse `d1b5520` already shipped for SpecValidationSeam - an in-repository precedent rather than a new pattern. Both layouts still verify (`rewind MeechieVoiceSeam` 4 passing, `rewind "MeechieVoiceSeam (self-contained)"` 17 passing) and the legacy import path keeps working for `src/lib/adapters/meechie-voice.adapter.ts`, `src/lib/mocks/meechie-voice.mock.ts` and `tests/contract/meechie-voice.test.ts`. No import cycle: the seam contract imports only `contracts/meechie-quote.contract` and `contracts/shared.contract`, never the flat voice contract.
- Revisit criteria: If the owner ever rules in lines that are not canon, re-introduce the distinction as a new contract change with its own red proof - do not restore `tier` as a comment-only marker. If the remaining mirrored contract pairs are ever collapsed, do it the same way and in their own change.
- Self-critique: The riskiest claim is "nothing read it", since a missed reader would be a silent behaviour change in the model prompt. It was checked rather than assumed - a repository-wide search for `tier`/`Tier` across `.ts`, `.svelte` and `.json` returns only this contract, the pack, the two fixtures, the seam test, and unrelated matches (`rateLimitTier`, `make_prettier`, `prettier`, and one prompt string containing the word "tiers"). `tone.samples` reads `.text` only. The second risk is that removing a field looks done when the suite is green, while a green suite proves only that the field is gone, not that anything stops its return; the red proof exists for exactly that, and RED 1 confirms the guard fails when `tier` is re-admitted as optional. Not run: a live provider probe. None is warranted - the assembled system prompt is byte-identical before and after, since it is built from the unchanged line text, so there is no provider-visible change to probe.
- Evidence: docs/evidence/2026-09-03/drop-tier-red-proof.txt; docs/evidence/2026-09-03/verify.txt; docs/evidence/2026-09-03/test.txt; docs/evidence/2026-09-03/rewind-MeechieVoiceSeam(self-contained).txt
- Merge posture: This is a contract change, which `AGENTS.md` lists as an explicit exclusion from the merge-on-green rule ("The PR carries a schema, contract, or data migration"). It is therefore **not** auto-merged regardless of CI, and waits for the owner.
- Plan:
  - Goal: Remove the one-valued, unread `tier` field from the Meechie quote contract without changing the voice.
  - Seams: MeechieVoiceSeam (self-contained).
  - Files: `contracts/meechie-quote.contract.ts`, `contracts/meechie-voice.contract.ts` (collapsed to a re-export), `src/lib/seams/meechie-voice-seam/contract.ts`, `src/lib/seams/meechie-voice-seam/voice-pack.ts`, `src/lib/seams/meechie-voice-seam/test.ts`, `fixtures/meechie-voice/sample.json`, `fixtures/meechie-voice/malformed-pack.json`, `DECISIONS.md`, and the regenerated `docs/evidence/2026-09-03/` artifacts including `drop-tier-red-proof.txt`.
  - Commands: `npm run check`, `npm run lint`, `npm test`, `npm run rewind -- --seam "MeechieVoiceSeam (self-contained)"`, `npm run verify`.

- Cipher Gate:
  - Date: 2026-09-03
  - Seams: MeechieVoiceSeam (self-contained)
  - Evidence: docs/evidence/2026-09-03/drop-tier-red-proof.txt; docs/evidence/2026-09-03/verify.txt; docs/evidence/2026-09-03/test.txt; docs/evidence/2026-09-03/rewind-MeechieVoiceSeam(self-contained).txt
  - Summary: Removed the `tier` field, its enum schema and its exported type from the Meechie quote contract, the voice pack's ten lines and both voice fixtures, leaving `{ id, text }` under `.strict()`. Every line was canon after the 2026-08-25 cut, so the field held one value and no code read it. The ten lines, their wording and `version: 'v4'` are unchanged, so the assembled system prompt is byte-identical. Guarded by an inline rejection case and by the malformed-pack fault fixture, both red-proved.
  - Risks: A voice pack persisted or authored elsewhere that still carries `tier` now fails validation instead of being ignored - intended, but it is a hard failure rather than a soft one. `MeechieQuoteTier` is removed from the public type re-export of both voice contracts, so any external consumer importing that type breaks at compile time; no in-repository consumer does. Collapsing the mirrored flat contract into an `export *` means the contracts layer now imports from `src/lib` for this seam, as it already does for SpecValidationSeam; a reviewer who considers that dependency direction wrong should change both seams together rather than only this one.
