...
- Revisit criteria: If `MeechieStudioTextOutputSchema` ever gains a way to express "no standing reported", `verdictStandingReported` and the `standingWasReported` input should be deleted in favour of it, and `describingStudioText`'s exclusion with them.
