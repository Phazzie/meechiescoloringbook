...
## 2026-09-07 - Store the draft's mode on the contract, and resolve it against the catalogue in core rather than in the schema

- Date: 2026-09-07
- Decision: Add an optional `modeId: NonEmptyStringSchema.optional()` to `DraftRecordSchema` in `src/lib/seams/creation-store-seam/contract.ts`, and put every decision about what a stored id *means* in a new pure `src/lib/core/draft-restore.ts`. `restoreDraftMode(storedModeId, modes)` returns the mode the studio will open on plus a three-valued `DraftModeProvenance` — `recorded` (the draft names a live mode), `unrecorded` (the draft names none, which every draft written before this change does), `retired` (the draft names a mode this build no longer has). `describeDraftRestore` turns that into the sentence the reader is shown, with `caution: string | null` as the single thing the component branches on. `StudioState` writes `modeId` on every autosave and reads it once during `init()`, before it assigns the evidence.
- Context: The studio's autosaved draft stored `intent`, `chatMessage`, `studioText` and `styleSelection` and restored all four. It stored no mode. `activeModeId` initialises to `studioModes[0].id` and nothing moved it, so a reader who picked any of the other seven modes, typed evidence and refreshed got their words back under **Who Fucked Up?** — with that mode's heading, help line and placeholder — and the next Generate Verdict posted `toolId: 'red_flag_or_run'`. A billable provider call answering a question the reader never asked, with nothing on screen saying the question had changed.
- Alternatives: (a) **Make `modeId` required.** Rejected outright: every draft already in a reader's browser was written without it, and a required field turns each of those into an unparseable record — the studio would not degrade to a default mode, it would throw the reader's whole page away. (b) **Type it as an enum of the live mode ids.** Tempting, and it would let the schema catch a typo. Rejected because the catalogue is application data that has already changed twice: a stored draft would become invalid the moment a mode was renamed or retired, converting a recoverable "your question is gone, here is the default" into the same total loss as (a). Resolving an id against a catalogue is a *decision*, not a validation, and it belongs where decisions are tested. (c) **Derive the mode from the stored `studioText`.** There is nothing on `MeechieStudioTextOutput` that names a mode, and a draft often has no verdict at all. (d) **Keep the fallback but say nothing**, i.e. persist the mode and stop. Rejected because it leaves the pre-change drafts silently wrong forever, which is the exact defect one cohort smaller. (e) **Drop the restored verdict whenever the mode cannot be recognised** — this was the first design, and it is recorded here because it was wrong in an instructive way; see below.
- Consequences: A refresh returns the reader to their own question. The three provenance values keep two facts apart that a boolean would have merged — "this draft is old" and "your mode was removed" are different sentences, and only the second is unrecoverable. The notice is the first thing the studio has ever said about restoring a draft at all: before this, a reader came back to text they had not typed in this sitting with no explanation of where it came from. `NonEmptyStringSchema` rather than `z.string()` is load-bearing: a blank id would round-trip as a mode matching nothing and be indistinguishable, at the consumer, from a draft that recorded none — one is corruption and the other is age, and they get different sentences.
- Self-critique: **The first design dropped the restored verdict on any unrecognised mode**, reasoning that a verdict is an answer and an answer detached from its question is meaningless. The reasoning is sound and the decision was still wrong, for a reason that only shows up when you count who it lands on: *every* draft written before this change is `unrecorded`, so shipping it would have deleted the page of every reader upgrading into this build, exactly once, silently — a total loss inflicted on everyone, to avoid a mislabelling that affected only readers who had switched modes. The restore now keeps everything it kept before and adds only the reporting. The lesson generalises: **when a fix's cost falls on a different population than its benefit, count both populations before preferring the more principled behaviour.** The second-riskiest assumption is that the optional field cannot hurt an existing reader; checked rather than asserted, in both directions — a draft with `modeId` deleted still parses (a test), and a draft written by this build parses in the previous one because zod objects ignore unknown keys by default.
- On the merge: `AGENTS.md`'s "Do not auto-merge when ... the PR carries a schema, contract, or data migration" applies to this change, and this run is a scheduled one with no human watching. The pull request is therefore driven to green and to a clean review and then **held**, with the condition named on the pull request rather than quietly worked around. The additive, both-directions-compatible shape argued above is a reason a human can approve it quickly; it is not a reason for an unattended run to decide the rule does not mean this.
- Revisit criteria: If the mode catalogue is ever frozen — no renames, no retirements — the enum in alternative (b) becomes affordable and `retired` loses its only producer. If a future run finds readers cannot tell the `unrecorded` and `retired` sentences apart in practice, collapsing them is a two-line change and `tests/unit/draft-restore.test.ts` names exactly which two.
- Evidence: docs/evidence/2026-09-07/verify-outer.txt (chain exit=0); docs/evidence/2026-09-07/test.txt; docs/evidence/2026-09-07/lint.txt; docs/evidence/2026-09-07/build.txt; docs/evidence/2026-09-07/e2e.txt; docs/evidence/2026-09-07/rewind-CreationStoreSeam.txt; docs/evidence/2026-09-07/rewind-CreationStoreSeam(self-contained).txt; src/lib/core/draft-restore.ts; tests/unit/draft-restore.test.ts; tests/unit/studio-state.test.ts; src/lib/seams/creation-store-seam/test.ts

- Cipher Gate:
  - Date: 2026-09-07
  - Seams: CreationStoreSeam (self-contained) — contract, fixtures, mock inputs and contract tests changed; CreationStoreSeam (legacy flat) re-exports it unchanged; ClockSeam — consulted, not modified
  - Evidence: plan.md; docs/evidence/2026-09-07/verify-outer.txt; docs/evidence/2026-09-07/test.txt; docs/evidence/2026-09-07/lint.txt; docs/evidence/2026-09-07/build.txt; docs/evidence/2026-09-07/e2e.txt; docs/evidence/2026-09-07/chamber-lock.json; docs/evidence/2026-09-07/shaolin-lint.json; docs/evidence/2026-09-07/rewind-CreationStoreSeam.txt; docs/evidence/2026-09-07/rewind-CreationStoreSeam(self-contained).txt; fixtures/creation-store/sample.json; fixtures/creation-store/fault.json; src/lib/seams/creation-store-seam/test.ts; tests/unit/draft-restore.test.ts
  - Summary: Added an optional `modeId` to `DraftRecordSchema` so an autosaved draft records the question its evidence was typed under, re-captured the sample fixture's outputs from a real browser through `probes/browser-seams.probe.mjs`, and moved every judgement about a stored id — live, absent, or retired — into pure `src/lib/core/draft-restore.ts`, which also produces the sentence the studio now shows about what it restored. The draft's `updatedAtISO` moved from an unseamed `new Date()` to `ClockSeam`.
  - Risks: The field is optional, so a draft can still arrive with no mode and the studio still opens on the default for it — the mitigation is that it now says so rather than substituting silently, and both non-recorded cases are covered by contract, unit and end-to-end tests. A blank `modeId` is refused by `NonEmptyStringSchema` (red proof: `rejected.draftWithBlankModeId` in `fixtures/creation-store/fault.json`), so corruption cannot masquerade as age. This pull request carries a contract change and is therefore held rather than auto-merged, per `AGENTS.md`.
