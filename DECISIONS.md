...
## 2026-06-25 - Lazy-load pdf-lib in OutputPackagingSeam instead of eager top-level import
- Date: 2026-06-25
- Decision: Replace the static `import { PDFDocument } from 'pdf-lib'` at the top of `src/lib/adapters/output-packaging.adapter.ts` with a dynamic `const { PDFDocument } = await import('pdf-lib')` inside the only branch that actually constructs a PDF (the non-PNG `outputFormat` path of `package()`).
- Context: `pdf-lib` is a ~177KB-gzip dependency. Because `output-packaging.adapter.ts` was imported eagerly by `src/routes/studio-state.svelte.ts` (which `+page.svelte` and several `Studio*.svelte` components import at module top), every visitor to the main builder routes downloaded and parsed the full pdf-lib bundle on initial page load, even if they only ever export PNGs or never click download at all. Repo-wide audit for this session's "10 hardest fixes" list found this as the only genuinely uncovered, low-risk, high-value fix: the dual seam-layout duplication, RateLimitSeam gap, zod v3->v4 bump, dependency CVEs, missing CSP, and corrupted `.env.example` are all already the subject of open, unmerged PRs (see PR body / issue #175 backlog note), and the MeechieTools/voice-pack/meechie-tool.adapter triple-duplication is structurally the hardest problem but too risky to touch without dedicated isolation tests for the 11-tool dispatch logic.
- Alternatives: Leave pdf-lib eager; rejected because it taxes every page visitor for a feature only used by a minority who export prints. Replace pdf-lib with a smaller library; rejected as a much larger, riskier change with no clear smaller-and-equally-correct replacement, and out of scope for "fix the bundling problem" rather than "replace the PDF engine." Lazy-load the whole adapter module from the UI call site instead; rejected because the contract (`OutputPackagingSeam.package`) is already `Promise`-returning and async, so pushing the laziness into the adapter itself keeps the seam boundary and call sites unchanged.
- Consequences: `OutputPackagingSeam`'s contract signature, mock, and fixtures are unchanged — this is a pure implementation-detail optimization inside the adapter. Confirmed via build manifest inspection (see evidence) that pdf-lib now lives in its own chunk (`C28niuCj.js`, 434,897 bytes / 180,916 bytes gzip) that is only fetched the first time a user actually requests a non-PNG (PDF) export; the adapter's own code shrank to an 8,461-byte/3,041-byte-gzip chunk that the route bundles import statically instead of the old 437,122-byte/181,170-byte-gzip combined chunk. First-load weight for the affected routes drops by ~177KB gzip for every visitor who never exports a PDF.
- Revisit criteria: Revisit if `OutputPackagingSeam.package()` needs to run outside a browser (e.g. a server-side PDF export route), since dynamic `import()` of an ESM-only package assumes a module-capable runtime; also revisit if pdf-lib itself shrinks enough that the code-split no longer matters.
- Plan:
  - Goal: Move pdf-lib off the eager client bundle path without changing `OutputPackagingSeam` behavior.
  - Seams: OutputPackagingSeam.
  - Files: `DECISIONS.md`, `src/lib/adapters/output-packaging.adapter.ts`, `docs/evidence/2026-06-25/*`.
  - Commands: `npm run check`, `npm run lint`, `npx vitest run tests/contract/output-packaging.test.ts tests/unit/output-packaging-helpers.test.ts`, `npm test`, `rm -rf .svelte-kit/output && npm run build` (run both before and after the change via `git stash`/`git stash pop` to compare manifest chunking), `npm run verify`, `npm run cipher:gate`, `git diff --check`.
- Self-critique: The main risk is claiming a bundle-size win without proving the dynamic import actually code-splits rather than just moving an import statement that Rollup re-inlines anyway. Verified this directly by building both before and after the change and grepping the output chunks for the literal `PDFDocument` class plus inspecting `.vite/manifest.json`'s `dynamicImports` field — pdf-lib measurably moved to its own chunk that route bundles no longer statically import. The full unit/contract suite (519/520, 1 pre-existing skip) and `npm run check`/`lint` are unchanged before and after, confirming no behavior regression in either the PNG-passthrough or real `pdf-lib` PDF-generation paths.

- Cipher Gate:
  - Date: 2026-06-25
  - Seams: OutputPackagingSeam
  - Evidence: docs/evidence/2026-06-25/pdf-lib-lazy-load-bundle-diff.txt; docs/evidence/2026-06-25/pdf-lib-lazy-load-build-after.txt; docs/evidence/2026-06-25/pdf-lib-lazy-load-focused-tests.txt; docs/evidence/2026-06-25/pdf-lib-lazy-load-check.txt; docs/evidence/2026-06-25/pdf-lib-lazy-load-lint.txt; docs/evidence/2026-06-25/pdf-lib-lazy-load-diff.txt; docs/evidence/2026-06-25/pdf-lib-lazy-load-diff-check.txt; docs/evidence/2026-06-25/verify.txt; docs/evidence/2026-06-25/test.txt
  - Summary: Replaced OutputPackagingSeam's eager top-level `pdf-lib` import with a dynamic `import()` inside the PDF-generation branch, confirmed via before/after build manifest inspection that pdf-lib now ships in its own on-demand chunk (~177KB gzip) instead of every route's eager bundle, with zero contract or test changes.
  - Risks: This is a client-bundling optimization only; if a future change needs pdf-lib synchronously at module init (e.g. a top-level constant built from a pdf-lib export), the dynamic import would need to move back up or be awaited earlier.
