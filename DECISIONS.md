...
- Cipher Gate:
  ...
  - Summary: Reconciled every reviewed PR #228 repair on the concurrent head: one-attempt provider chat with shared identifier redaction, schema-defaulted image size, injection-safe exact-text prompt boundaries, strict nonblank/single-line title and label validation, a single canonical SpecValidation implementation behind legacy compatibility exports, regenerated prompt/drift/image fixtures, and truthful live/replay evidence, while removing unrelated permission and merge-authority changes.
