...
## 2026-06-19 - Add RateLimitSeam to gate billable /api/* POST routes
- Date: 2026-06-19
- Decision: Add a new self-contained `RateLimitSeam` (in-memory, clock-injected, fixed-window) and wire it into a new `src/hooks.server.ts` `Handle` hook so every `POST /api/*` request is gated by client address before reaching a route handler, instead of touching all six existing route handlers individually.
- Context: A repo-wide audit found six `/api/*` POST routes (`generate`, `image-generation`, `chat-interpretation`, `tools`, `wig-try-on`, `meechie-studio-text`) with zero authentication or request-volume control in front of provider calls billed per-request to xAI and Gemini, and no body-size limit on `parseRequestBody`. A single unauthenticated client could trigger unbounded billable calls or a basic flood, with no operator-facing guardrail beyond provider-account-level safeguards (if any). This was selected as the most consequential item on a ranked list of ten hard repo problems because it is the only one that is both immediately exploitable on the live Vercel deployment and fully fixable without a blocked external dependency (unlike probe-blocked seams) or a high-blast-radius refactor (unlike the legacy/self-contained seam consolidation).
- Alternatives: Add ad hoc rate-limit checks inside each of the six route handlers; rejected because it duplicates logic six times and is easy to forget on the next new route. Use a third-party rate-limit service (Upstash/Vercel KV); rejected for this pass because no such datastore is currently provisioned, and an in-memory limiter is a strict improvement over the current zero protection while remaining fully testable without new infrastructure. Block on request body size in the same change; deferred as a separate, narrower follow-up since it is an unrelated guardrail.
- Consequences: Every `POST /api/*` request now consumes a slot in a fixed window keyed by `event.getClientAddress()`; once a client exceeds `RATE_LIMIT_MAX_REQUESTS` (default 20) within `RATE_LIMIT_WINDOW_MS` (default 60000), further requests in that window receive `429` with a `Retry-After` header until the window rolls over. The limiter state is in-memory per server instance, so it resets on cold start and is not shared across concurrent serverless instances — documented below as a known limitation rather than silently ignored. Also fixed `.env.example`, discovered while adding the new env vars: the committed file was base64-encoded text instead of plain text (`base64 -d` recovered the original content byte-for-byte); restored it and appended `RATE_LIMIT_MAX_REQUESTS`/`RATE_LIMIT_WINDOW_MS` documentation.
- Revisit criteria: Revisit if the app moves to a shared key-value store (Upstash/Vercel KV) for cross-instance rate limiting, if abuse patterns show client-address keying is insufficient (e.g. shared NAT/proxy IPs), or if request body-size limiting is tackled as its own workpack.
- Plan:
  - Goal: Implement the highest-priority item from a 10-item hardest-problems audit: add abuse protection to billable AI endpoints.
  - Seams: RateLimitSeam (new).
  - Files: `src/lib/seams/rate-limit-seam/{contract,validators,fixtures,limiter,mock,probe,test}.ts`, `src/lib/adapters/rate-limit-seam/index.ts`, `src/hooks.server.ts`, `tests/unit/rate-limit-seam-adapter.test.ts`, `tests/unit/hooks-server-rate-limit.test.ts`, `.env.example`, `docs/seams.md`, `src/lib/seams/CLAUDE.md`, `CHANGELOG.md`, `plan.md`, `DECISIONS.md`, `docs/evidence/2026-06-19/*`.
  - Commands: `npx vitest run src/lib/seams/rate-limit-seam/test.ts tests/unit/rate-limit-seam-adapter.test.ts tests/unit/hooks-server-rate-limit.test.ts`, `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run verify`, `npm run rewind -- --seam RateLimitSeam`, `git diff --check`.
- Self-critique: The riskiest assumption is that `event.getClientAddress()` is a reliable abuse-protection key on Vercel; it can be shared by clients behind the same NAT/proxy, so this is a volume guardrail, not per-user auth. The in-memory store also does not survive serverless cold starts or share state across concurrent instances, which is called out explicitly as a known limitation rather than hidden. The fault fixture (empty key) proves `checkAndConsume` rejects invalid input instead of silently allowing it, and the hooks test proves both the 429 short-circuit and the per-key isolation (one exhausted client does not block another).
