...
- Assumption:
  ...
  - Validation: **BLOCKED, not merely deferred - and it took three review rounds to establish that, each finding the previous plan unexecutable.** No validation plan for this waiver can be written in documentation alone, because the seam's fixture-refresh path is broken in three independent places. (1) The probe's output has no consumer: `probes/image-generation.probe.mjs:58` writes `fixtures/image-generation/{sample,fault}.json`, and `grep -rn "fixtures/image-generation" src/ tests/ scripts/ contracts/` returns zero hits, while both suites import `src/lib/seams/image-generation-seam/fixtures.ts`; of the fourteen directories under `fixtures/`, this is the only one nothing reads. (2) No captured success can reach the mock: `fixtures.ts` exports a *request* and a fault and nothing else, `mock.ts:10` imports only `imageGenerationFaultFixture` and its `sample` scenario synthesises an SVG via `buildSvg` rather than replaying a provider response, and `tests/contract/image-generation.test.ts:32` uses its own hard-coded `xaiSampleResponse` - so a transcribed success has nowhere to land, and a maintainer could pay for the live call, update the request fixture and get a green rewind that exercised no real provider output. (3) The probe cannot capture a live fault: it throws on any non-2xx response (`:138-148`), so a real failure aborts the run instead of being recorded, and the `fault.json` it writes afterwards on the success path is a hard-coded `PROMPT_MISSING_REQUIRED_PHRASES` (`:224`), a different code from the canonical `IMAGE_HTTP_ERROR` fixture (`fixtures.ts:15`), so it could not be transcribed in the fixture's shape even if something read it. Together: running the probe today spends a billable live image call and produces two files nothing loads, one of which is not a real fault, so any validation built on it would be green for reasons unrelated to what it claims to test. This waiver therefore rests on the diff-unchanged argument in its Status field, with validation genuinely unavailable rather than scheduled. Making it available is three code changes, each in a seam folder and each owed the full workflow rather than a line in a waiver: give `fixtures.ts` a captured-success export that `mock.ts`'s `sample` scenario loads (or have the mock read the JSON captures); give the probe a bounded fault capture recording a real non-2xx response in the canonical shape instead of throwing; then point the contract test at the shared fixture instead of its private `xaiSampleResponse`. Recorded as follow-up 9. Until those land, a future run must not record this Assumption as validated on the strength of a green rewind - the rewind is green now, on fixtures nobody refreshed. **The plan for later validation, which the completion checklist requires and an earlier draft of this field wrongly declared impossible to write.** It is conditional on those three repairs, not absent: what could not be written was a plan that runs *today*. Once they land, in order: (1) `node probes/image-generation.probe.mjs` with XAI credentials, which by then records a live success and a real non-2xx fault in the canonical `IMAGE_HTTP_ERROR` shape; (2) refresh `src/lib/seams/image-generation-seam/fixtures.ts` from those captures, keeping its exported shapes, and confirm `mock.ts`'s `sample` scenario now loads the captured success rather than `buildSvg`; (3) red proof before green, as the checklist requires - point the fault scenario at the refreshed fault fixture and watch `src/lib/seams/image-generation-seam/test.ts` fail on it, capturing that output as `docs/evidence/<date>/image-generation-fault-red-proof.txt` before the fixture is accepted; (4) `npm test -- tests/contract/image-generation.test.ts src/lib/seams/image-generation-seam/test.ts --pool=forks --maxWorkers=1`; (5) `npm run rewind -- --seam ImageGenerationSeam` -> `docs/evidence/<date>/rewind-ImageGenerationSeam.txt`; (6) `npm run verify`, then `npm run cipher:gate` - the three repairs are a seam change and owe a Cipher Gate entry of their own; (7) update the `ImageGenerationSeam` row's last-probe date in `docs/seams.md`, then mark this Assumption Resolved with the date and the evidence paths above. Step 3 is the one a hurried run will skip and is the only step that proves the refreshed fault fixture can fail; a green suite after a refresh means nothing without it.
  - Status: Waived for PR #295 and its close-out. Scoped deliberately: the earlier waiver dated 2026-05-14 covers "this review-comment repair" only, and citing it for a different change would be borrowing an authorization that was never given. PR #295 adds no new call shape, model id or provider request to this seam - it adds a **sixth** screen that can reach an endpoint **five** others already reached - so the fixtures' representativeness is unchanged by the diff. The five at base `210b301`, enumerated so this count is checkable rather than asserted: the home studio (`studio-state.svelte.ts:668`), `/meechie` (`MeechieTools.svelte:283`), and `/who-fucked-up`, `/rate-his-excuse`, `/random` (all three through `verdict-page-state.svelte.ts:480`). This is the **third** wrong exposure count in this close-out, and the reason it recurs is worth naming: two different endpoints have two different consumer sets, and the counts are not interchangeable. `/api/tools` already had `/m/<slug>` among its five consumers before the rebuild (see the 2026-08-24 ruling below); `/api/generate` did not, so for *this* seam `/m/<slug>` is genuinely new and genuinely the sixth. A future audit reading either number must check which endpoint it belongs to. `assumption-alarm.json` continues to list ImageGenerationSeam under blockedSeams, which is correct and is why this entry exists.
