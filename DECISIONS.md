...
## 2026-07-02 - PR #202/#204 review-comment audit: no live main bugs found, seam-inventory drift fixed

- Date: 2026-07-02
- Decision: Do not port PR #202's or PR #204's feature branches onto `main` as part of a "review comment fix" task. Instead, fix the one confirmed, independent doc-drift bug on `main` (`src/lib/seams/CLAUDE.md` missing 8 of 15 self-contained seam folders) and record this audit so a future, deliberately-scoped task can decide whether/how to land the underlying feature work.
- Context: Asked to address the 23 unresolved review threads on PR #202 and the 19 unresolved review threads on PR #204 (both based on `main` at `29109f0`, neither merged). Cross-referencing every thread against current `main` showed that PR #202 and PR #204 are not incremental fixes to existing `main` behavior — they are two links in a stacked chain (PR #198 → PR #202 → PR #204, confirmed by PR #204's own description: "Both branches were merged into this working branch") that together introduce brand-new, never-merged infrastructure: a rate limiter and `rate-limit-seam`/`rate-limit-config-seam` pair, a circuit breaker added to `http-resilience.ts`, abort-signal threading into `/api/chat-interpretation`, `/api/tools`, and the image/wig routes, image/Gemini config preflight-before-rate-limit ordering, a `precomputedWig` id-match guard in `wig-try-on-pipeline.ts`, legacy-string clamping in `creation-store.adapter.ts`, and a shared `MAX_FREE_TEXT_LENGTH` constant. PR #204's diff is +15,822/-342 across 132 files. None of this exists on `main` today (verified by grep: zero matches for `circuit|breaker|isOpen|recordFailure` under `src/`; `src/lib/server/rate-limiter.ts` does not exist; `src/lib/seams/rate-limit-*` do not exist; `/api/chat-interpretation` and `/api/tools` routes never read `request.signal`; `wig-try-on-pipeline.ts` has no `precomputedWig` parameter; `creation-store.adapter.ts` has no clamping logic; `docs/evidence/2026-06-29/` and `docs/top-10-hardest-fixes.md` do not exist on `main`). The single thread that references code actually present on `main` (`contracts/meechie-studio-text.contract.ts` `pageItems` input schema vs. output schema shape) was checked against its only caller, `src/routes/studio-state.svelte.ts:292`, which already maps the output's `{number,label}` objects down to plain label strings before sending them back as `currentText.pageItems` — so the input schema already matches what is actually sent and there is no reachable bug, just an intentional type asymmetry between echoed-context input and generated output. Separately (not from either PR's review threads, but matching the general "seam inventory drift" issue category called out for this audit), `src/lib/seams/CLAUDE.md`'s "Current seams" snapshot table was stale independent of anything in PR #202/#204: it listed 7 of the 15 folders actually present under `src/lib/seams/`, missing `drift-detection-seam/`, `meechie-tool-seam/`, `meechie-voice-seam/`, `prompt-assembly-seam/`, `spec-validation-seam/`, `wig-catalog-seam/`, `wig-try-on-seam/`, and `cache-seam/`.
- Alternatives: Cherry-pick or merge PR #204's branch (`claude/keen-hypatia-fo7yip`) onto `main` and apply the reviewer-suggested fixes on top; rejected because that is a unilateral decision to land ~15,800 lines of unreviewed, billing-relevant production code (rate limiting and provider-call resilience) under the framing of "fix review comments," which this task did not authorize and which deserves its own deliberately-scoped Seam-Driven Development pass (contract → probe → fixtures → mock → adapter → test) rather than being smuggled in as a side effect. Fabricate new, narrower resilience code directly on `main` under the same banner; rejected for the same reason — it would be new feature work, not a bug fix, and the repo already has a much more complete attempt at this work sitting in the PR #198/#202/#204 chain that a human should explicitly choose to land, revise, or discard. Do nothing and report only; rejected because the seam-inventory drift was a real, independent, low-risk bug worth fixing regardless of the PR #202/#204 finding.
- Consequences: `src/lib/seams/CLAUDE.md` now lists all 15 current self-contained seam folders, matching `docs/seams.md`. No other `main` code changed. The 42 combined PR #202/#204 review threads remain open on those PRs (as instructed, this work does not attempt to resolve threads on PR #202/#204 themselves); the new PR opened from this branch documents, thread-by-thread, why each was skipped as not-applicable-to-`main`, superseded-by-reply, or (for the one doc fix) implemented.
- Revisit criteria: Revisit if a human decides to land the PR #198/#202/#204 rate-limiter/circuit-breaker/abort-threading work — at that point the specific reviewer-suggested fixes cited across both PRs (breaker cooldown reset, abort-before-breaker-check ordering, deferred breaker success recording, Retry-After cap, config-preflight-before-rate-limit ordering, `precomputedWig` id guard, shared `MAX_FREE_TEXT_LENGTH`, `rate-limit-config-seam` type/mock dedup) should be applied as part of that landing, not skipped.
- Plan:
  - Goal: Audit all unresolved PR #202 (23 threads) and PR #204 (19 threads) review comments against current `main`, fix any real currently-unaddressed issues, and document findings.
  - Seams: None (no seam contract, adapter, mock, or observable seam behavior changed; `src/lib/seams/CLAUDE.md` is a non-authoritative documentation snapshot explicitly marked as such in its own text).
  - Files: `DECISIONS.md`, `LESSONS_LEARNED.md`, `src/lib/seams/CLAUDE.md`.
