...
- Fourth decision, and the one a reviewer is most likely to challenge: **the style hint is derived from the interpreted spec, not from the reader's sentence.** The sentence is a *page* request ("a page that says X with roses around it") and putting it in `styleHint` hands the image model the words a second time, in the one field that is not the exact-text block — the surest way to get them drawn twice. Interpreting "with roses" into `illustrations` and `decorations` is what the interpretation call already did; `styleHintForSpec` reads its answer.
