...
- Assumption:
  ...
  - Scope widened 2026-09-06 by the offline work in #311, which is why this entry is now load-bearing rather than tidy. When it was written the filesystem-served paths were assets only, and every document still came from the SvelteKit function, where `hooks.server.ts` attaches the headers in code a test can run. Prerendering `/`, `/offline`, `/meechie`, `/who-fucked-up`, `/rate-his-excuse`, `/random` and `/m/[mode]` makes those documents files, and Vercel's `{"handle":"filesystem"}` precedes the SSR rewrite, so all five security headers on every page of this app now come from the `vercel.json` rules and from nothing else. `tests/unit/security-headers.test.ts` proves the rules *cover* every prerendered path, deriving the list from the routes' own `prerender` flags; whether Vercel then applies them is this Assumption.
