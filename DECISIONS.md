...
- Cipher Gate:
  ...
  - Risks: The SVG/webp image-detection ReDoS-regex and studio-state mimeType-fallback review comments from both source PRs are intentionally not addressed here, since that feature does not exist on `main`; rate limiting remains per-process in-memory state, so it will not coordinate across multiple server instances/regions.
