...
## 2026-07-05 - Four quick-win fixes: rating display, safety keyword parity, wig config status, settings toggle label

- Date: 2026-07-05
- Decision: Fix four small, isolated defects found during a routine sweep: (1) `rate-his-excuse` page rendered `result.headline` (already `"N/10"`) next to a static `"out of 10"` label, producing `"7/10 out of 10"` — now renders the raw `result.rating` number; (2) `SafetyPolicySeam`'s `disallowedKeywords` locally appended `'suicide'` and `'extremist'` to `SYSTEM_CONSTANTS.DISALLOWED_KEYWORDS`, so `/api/tools` and `/api/meechie-studio-text` (which call the shared `findDisallowedKeywords` helper against the bare constant) never blocked those two words — moved both into `SYSTEM_CONSTANTS.DISALLOWED_KEYWORDS` so all three call sites share the same list; (3) `wig-try-on-pipeline.ts` mapped `WIG_TRY_ON_CONFIG_ERROR` (missing `GEMINI_API_KEY`) to a generic `502`, indistinguishable from a real upstream failure, while the sibling `image-generation-pipeline.ts` already maps its analogous config-error code to `503` — added the same `503` mapping via a `statusByCode` table; (4) `StudioSettingsPanel.svelte`'s `<details>` toggle always showed the static label `"Open"`, even once expanded — added a scoped `when-closed`/`when-open` span pair toggled by the `details[open]` CSS selector so the label reads `"Close"` once expanded.
- Context: Found via a codebase-wide quick-win sweep (lint/check/test all green beforehand). None of the four had existing test coverage asserting the old (buggy) behavior, so no test was locked to the defect.
- Alternatives: For (2), could have kept the two words only in the safety-policy-seam list and duplicated them into `findDisallowedKeywords`'s callers instead; rejected because that keeps three seam-adjacent call sites in sync by convention rather than by a single source of truth. For (3), could have added a broader error-code taxonomy; rejected as out of scope for a quick win — matched the existing `image-generation-pipeline.ts` pattern instead. For (4), could have driven the label from Svelte state (`$state`/`bind:open`) instead of CSS; rejected as more moving parts than a presentational toggle needs.
- Consequences: Rating display and settings-panel label are now correct without changing markup structure other tests key off. Content-safety keyword coverage is now consistent across `/api/generate`, `/api/tools`, and `/api/meechie-studio-text`. Wig try-on config errors are now distinguishable from network/provider failures via HTTP status.
- Revisit criteria: Revisit if `SafetyPolicySeam` grows a need for per-endpoint keyword lists (currently all three endpoints intentionally share one list); revisit wig-try-on status mapping if the contract adds more granular error codes.
- Plan:
  - Goal: Land four small, independently-safe fixes in one PR.
  - Seams: SafetyPolicySeam (keyword list only; no contract/mock/fixture changes).
  - Files: `src/routes/rate-his-excuse/+page.svelte`, `src/lib/core/constants.ts`, `src/lib/seams/safety-policy-seam/policy.ts`, `src/lib/core/wig-try-on-pipeline.ts`, `src/lib/components/studio/StudioSettingsPanel.svelte`, `docs/evidence/2026-07-05/*`.
  - Commands: `npm run lint`, `npm run check`, `npm run test`, `npm run verify`.
- Self-critique: The main risk is that (2) makes moderation strictly more permissive-to-block (additive keywords only), so it cannot regress any passing test; confirmed via full `npm run verify` and `npm run test` (519 passed, 1 skipped) with no assertions on the old narrower keyword list. (3) and (4) have no existing tests asserting the old status code or label text, checked via targeted greps before editing.

- Cipher Gate:
  - Date: 2026-07-05
  - Seams: SafetyPolicySeam
  - Evidence: docs/evidence/2026-07-05/quick-wins-lint.txt; docs/evidence/2026-07-05/quick-wins-check.txt; docs/evidence/2026-07-05/quick-wins-test.txt; docs/evidence/2026-07-05/quick-wins-verify.txt
  - Summary: Fixed a duplicated "N/10 out of 10" rating display, unified the disallowed-keyword list so `/api/tools` and `/api/meechie-studio-text` block the same words as `/api/generate`, mapped the wig try-on config-error code to HTTP 503 instead of a generic 502, and made the settings-panel `<details>` toggle label reflect open/closed state.
  - Risks: None of the four fixes had prior test coverage pinning the old (buggy) behavior; full `npm run verify` and `npm run test` pass with no new fixtures added since none of the four changes altered a seam contract shape.
  - Follow-up (PR #218 review): lowercased the keyword side of the `hasDisallowedContent` comparison in `safety-policy-seam/policy.ts` per a Gemini Code Assist suggestion (defensive against a future mixed-case keyword); added a `ratingScore ?? '—'` fallback in `rate-his-excuse/+page.svelte` per Copilot/CodeRabbit findings (`rating` is typed optional on `MeechieToolOutputSchema` even though the `rate_excuse` adapter path guarantees it at runtime — the fallback guards the type-level gap without changing observed behavior).
