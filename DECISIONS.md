...
## 2026-09-07 - Fix the safety-keyword parity gap carried forward from PR #218 / issue #327

- Date: 2026-09-07
- Decision: Move `'suicide'` and `'extremist'` into `SYSTEM_CONSTANTS.DISALLOWED_KEYWORDS` (`src/lib/core/constants.ts`) and delete the local `disallowedKeywords` copy in `src/lib/seams/safety-policy-seam/policy.ts`, so `hasDisallowedContent` scans the same shared list that `findDisallowedKeywords` (used by `/api/tools` and `/api/meechie-studio-text`) already reads. Also lowercase the keyword side of the substring comparison, not just the text side, so a future keyword added in mixed case cannot silently fail to match.
- Context: PR #218 (2026-07-05) originally fixed four quick wins, including this one. That PR sat open for two months while `main` moved 361 commits; a later session re-verified the PR against current `main`, found three of its four items already independently fixed (rating display, wig-try-on config-error status, settings-panel toggle label — the last with its own code comment referencing the exact bug), closed PR #218, and filed issue #327 to carry forward the one item still live: `SafetyPolicySeam` had its own local `[...SYSTEM_CONSTANTS.DISALLOWED_KEYWORDS, 'suicide', 'extremist']`, so the two words were enforced only on whichever endpoint `SafetyPolicySeam` guards, while `/api/tools` and `/api/meechie-studio-text` — which call `findDisallowedKeywords` directly against the bare constant — did not block either word at all. Re-verified directly before fixing: `git show origin/main:src/lib/core/constants.ts` and `git show origin/main:src/lib/seams/safety-policy-seam/policy.ts` on `main` at `7fbb57d` (current head at the time of this entry) both still showed the split list, confirming issue #327's premise still held.
- Alternatives: Keep the split and instead duplicate `'suicide'`/`'extremist'` into `findDisallowedKeywords`'s own call sites; rejected because that's the same bug shape one level down — a second place that must be kept in sync by convention instead of by there being only one list. Add a shared helper that both `constants.ts` and `policy.ts` import from; rejected as unnecessary indirection once the keywords live in one array both already import.
- Consequences: `/api/tools`, `/api/meechie-studio-text`, and whatever `SafetyPolicySeam` guards now all enforce the same four-keyword list from one source. No contract, fixture, or mock changed — `DISALLOWED_KEYWORDS` was already `SYSTEM_CONSTANTS`, a plain array, and neither `tests/unit/constants.test.ts` nor any other test pins its length or exact contents, only that it is non-empty and lowercase.
- Revisit criteria: If any endpoint ever needs a keyword list narrower or broader than the shared one (e.g. a stricter policy for a specific mode), that is the point at which per-endpoint lists become legitimate and should be modeled explicitly rather than by silently drifting apart again.

- Cipher Gate:
  - Date: 2026-09-07
  - Seams: SafetyPolicySeam
  - Evidence: docs/evidence/2026-09-07/verify.txt; docs/evidence/2026-09-07/test.txt; docs/evidence/2026-09-07/chamber-lock.json; docs/evidence/2026-09-07/seam-ledger.json; docs/evidence/2026-09-07/clan-chain.json; docs/evidence/2026-09-07/proof-tape.json; docs/evidence/2026-09-07/shaolin-lint.json; docs/evidence/2026-09-07/assumption-alarm.json
  - Summary: Closed the content-safety gap from issue #327 by making `SYSTEM_CONSTANTS.DISALLOWED_KEYWORDS` the single source of truth for `suicide`/`extremist` blocking across `/api/tools`, `/api/meechie-studio-text`, and `SafetyPolicySeam`, and hardened the comparison to be case-insensitive on both sides.
  - Risks: None identified — this only widens what two of three endpoints already block, and `npm run verify` (1636 tests) passes with no fixture or contract changes.
