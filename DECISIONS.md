...
- Plan:
  ...
  - Files — every path the change touches, with what happens to each. Checkable against the commit with `git diff --name-status <base>..HEAD`; an earlier version of this list omitted four of them and marked none, which made the scope gate uncheckable in the direction that matters (understating it).
    ...
    - The whole list above is meant to equal `git diff --name-status <base>..HEAD`, and that is the check: any path in one and not the other is a defect in this entry. `lint.txt` is deliberately absent — eslint prints nothing on success, so the file does not change and does not appear in the diff.
