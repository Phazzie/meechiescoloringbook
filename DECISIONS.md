...
## 2026-06-22 - Fix dependency vulnerabilities and add a permanent audit gate

- Date: 2026-06-22
- Decision: Run a non-breaking `npm audit fix` to resolve the 4 known advisories present on `main` (high: `undici`, `vite`; moderate: `js-yaml`, `tar`), then add `scripts/audit-gate.mjs` (wired into `npm run verify` as `audit:gate`, immediately after `chamber:lock`) so any future high/critical advisory fails verify instead of sitting unnoticed.
- Context: This was chosen as the highest-value fix to perform in this session out of a 10-item difficulty review (see PR body / `plan.md` for the full list). Several harder items on that list (a correct serverless-safe `RateLimitSeam`, finishing the legacy/self-contained seam-layout consolidation) already have multiple divergent, unmerged attempts open against this repo (tracked in GitHub Issue #175: 26+ open PRs across the `claude/keen-hypatia-*`, `claude/trusting-volta-*`, and `claude/sweet-mendel-*` branch families, 0 merges in 14+ days as of this session). Adding a 6th/7th competing implementation for any of those would worsen the exact problem the repo owner already flagged; the dependency-vulnerability gap, by contrast, had no safe fix in flight (the one open PR touching it, a dependabot bump, proposes a breaking `vite` 7→8 major version jump rather than the safe in-range fix) and was fully fixable and verifiable without touching any seam.
- Alternatives: Accept the dependabot `vite` 7→8 major bump instead; rejected because it is a breaking change far outside the scope of a security fix and is itself sitting unmerged. Fix the advisories without adding a gate; rejected because `npm audit` was already silently failing (4 unresolved advisories, 2 high) with nothing in `verify` to catch it, so the same drift would recur. Add the gate at a stricter threshold (fail on moderate too); rejected because low/moderate transitive dev-dependency churn is frequent and would make the gate noisy without much safety benefit; high/critical is the conventional `--audit-level` threshold.
- Consequences: `package-lock.json` now resolves `js-yaml@4.2.0`, `tar@7.5.16`, `undici@7.28.0`, `vite@7.3.5` (all within existing `package.json` semver ranges; zero `package.json` changes). `npm audit` reports 0 vulnerabilities. `npm run verify` now runs `npm run audit:gate` and will fail the build the next time a high/critical advisory appears, with evidence written to `docs/evidence/YYYY-MM-DD/audit-gate.json`.
- Revisit criteria: Revisit the severity threshold if `high`-severity false positives in dev-only tooling start blocking verify frequently; revisit the gate's placement in the chain if it becomes a speed bottleneck (currently a single fast `npm audit --json` call).
- Plan:
  - Goal: Close the currently-unenforced dependency-vulnerability gap safely, without adding to the open competing-PR backlog on harder/larger items.
  - Seams: None (no seam, contract, fixture, mock, adapter, or contract test touched; `scripts/` is tooling, consistent with `chamber-lock.mjs`/`shaolin-lint.mjs`).
  - Files: `package.json`, `package-lock.json`, `scripts/audit-gate.mjs`, `docs/evidence/2026-06-22/*`, `DECISIONS.md`, `LESSONS_LEARNED.md`, `CHANGELOG.md`, `plan.md`.
  - Commands: `npm audit`, `npm audit fix`, `node scripts/audit-gate.mjs` (red-proofed against the pre-fix lockfile, then green-proofed against the fixed lockfile), `npm run verify`, `npm run lint`, `npm test`, `npm run check`.
- Self-critique: The main risk is that `npm audit fix` silently introduced a regression in a dev-only tool (vite, vitest's transitive deps); this is mitigated by `npm run verify` and `npm run lint` both passing green afterward with zero `package.json` range changes (only in-range lockfile resolutions moved). A second risk is the gate itself being a false sense of security since it only runs locally/in CI rather than continuously; this is the same tradeoff every other `verify` gate in this repo already accepts. A third, larger risk is scope: the actual hardest item on the day's list (the runaway PR backlog) is a process problem outside what a single autonomous code change can fix, so this entry deliberately documents why a smaller, safe, unduplicated fix was chosen instead of attempting (and likely failing to land) a bigger one.
