...
## 2026-07-02 - Add RateLimitSeam and gate paid-provider API routes
- Date: 2026-07-02
- Decision: Add a new `RateLimitSeam` (in-memory fixed-window limiter, dependency-injected pattern) and wire it into every `+server.ts` route that calls a paid external AI provider (`/api/generate`, `/api/image-generation`, `/api/chat-interpretation`, `/api/meechie-studio-text`, `/api/tools`, `/api/wig-try-on`) via a shared `src/lib/server/rate-limiter.ts` singleton and `src/lib/server/enforce-rate-limit.ts` helper. Requests over 20 per 10-minute window per `${routeName}:${clientAddress}` key now receive `429` with a `Retry-After` header instead of reaching the pipeline.
- Context: An audit of this repo's hardest outstanding gaps (see PR description / `docs/todo-from-cleanup.md` history) found that all six API routes were fully anonymous and completely unbounded: any caller on the internet could trigger unlimited calls to metered xAI and Gemini APIs, creating unbounded billing exposure and a trivial DoS vector. No rate limiting, auth, or abuse protection existed anywhere in the codebase (confirmed via repo-wide search) before this change.
- Alternatives: Add a KV/Redis-backed distributed limiter; rejected for this change because it requires new paid infrastructure and credentials that are out of scope for a code-only fix, and the seam boundary here is designed so a KV-backed adapter can be swapped in later without changing the contract or call sites. Add authentication instead; rejected as a much larger scope change (session/user model) than the immediate cost/abuse exposure warrants. Do nothing and rely on provider-side quotas; rejected because provider-side quotas protect the provider's account, not this app's budget or availability per caller.
- Consequences: Each paid-provider route now rejects bursty/abusive traffic per client address within a warm server instance. This is a best-effort, per-instance mitigation, not a perfect cross-instance guarantee — a caller that hits multiple cold-started serverless instances can still exceed the nominal budget until instances converge. That limitation is intentional and documented in `docs/seams.md` and the seam's own comments so a future KV-backed adapter swap is an easy, low-risk follow-up.
- Revisit criteria: Revisit if abuse is observed in production logs despite this limiter (add a KV/Redis-backed adapter behind the same `RateLimitSeam` contract), or if per-route budgets need to differ (currently all six routes share one `MAX_REQUESTS`/`WINDOW_MS` pair in `SYSTEM_CONSTANTS.RATE_LIMIT`).
- Plan:
  - Goal: Close the highest-severity unaddressed gap found in a repo-wide "hardest fixes" audit: unbounded anonymous access to paid AI provider routes.
  - Seams: RateLimitSeam (new).
  - Files: `src/lib/seams/rate-limit-seam/{contract,limiter,mock,fixtures,probe,validators,test}.ts`, `src/lib/server/rate-limiter.ts`, `src/lib/server/enforce-rate-limit.ts`, `src/routes/api/{generate,image-generation,chat-interpretation,meechie-studio-text,tools,wig-try-on}/+server.ts`, `src/lib/core/constants.ts`, `docs/seams.md`, `tests/unit/enforce-rate-limit.test.ts`, `tests/unit/api-{generate,image-generation,chat-interpretation,meechie-studio-text-endpoint,tools,wig-try-on}.test.ts` (added `getClientAddress` to test event builders), `DECISIONS.md`, `LESSONS_LEARNED.md`.
  - Commands: `npm run rewind -- --seam RateLimitSeam`, `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run verify`.
- Self-critique: The main risk is a false sense of security — an in-memory limiter only bounds abuse within one warm process, not across a fleet of serverless instances, so it does not fully prevent cost overrun under sustained distributed abuse. This is called out explicitly in `docs/seams.md`, this entry, and the seam's own file comments rather than left implicit. The other risk was breaking the six existing route-level tests, which constructed `RequestHandler` events without `getClientAddress`; all six were updated to supply it and the full suite (527 tests) still passes.
...
- Cipher Gate:
  ...
  - Summary: Added RateLimitSeam (in-memory fixed-window limiter) and gated all six paid-provider API routes behind it, closing the top item from a repo-wide hardest-fixes audit: fully anonymous, unbounded access to metered xAI/Gemini calls.
  - Risks: In-memory state is per warm serverless instance only, not a distributed guarantee; a future KV/Redis-backed adapter behind the same contract is the documented follow-up if production abuse is observed.

## 2026-07-02 - Bound RateLimitSeam memory growth and normalize client keys (PR #211 review follow-up)
- Date: 2026-07-02
- Decision: Address review findings on PR #211 (Gemini Code Assist, Sourcery, Copilot) against the just-added `RateLimitSeam`: (1) the singleton `windows` Map never evicted entries, so distinct keys accumulated forever in a warm instance; (2) even opportunistic expired-entry eviction cannot bound memory against a flood of always-fresh distinct keys (e.g. an attacker varying source IPs), since none of those entries are ever "expired" mid-attack; (3) `enforceRateLimit` assumed `getClientAddress()` always returns a non-empty string, which could silently fragment or collide rate-limit buckets on an unexpected empty value; (4) `resetAtMs` in the validator schema allowed negative values, which cannot occur in practice and should be caught as contract drift.
- Context: Three independent automated reviewers converged on the same root issue (unbounded Map growth) within minutes of the PR opening, and Copilot additionally identified that expired-only eviction is insufficient under adversarial high-cardinality traffic. All four points were small, mechanical, and directly verifiable, so they were fixed in the same PR rather than deferred.
- Alternatives: Defer the hard-cap fix as a follow-up ticket since expired-entry eviction alone addresses the common case; rejected because the reviewed threat model for this seam is explicitly abuse/DoS resistance, and an eviction strategy that only helps under non-adversarial conditions doesn't meet that bar. Use a true LRU cache library; rejected as unnecessary complexity for a bounded in-memory map with infrequent (threshold-gated) sweeps.
- Consequences: `RateLimitSeam` now exposes `size()` for observability/testing, evicts expired entries once tracked keys exceed 1,000, and additionally evicts the oldest entries once tracked keys exceed a 5,000 hard cap regardless of expiry — bounding worst-case memory even under a sustained flood of distinct keys. `enforceRateLimit` normalizes an empty/whitespace client address to an explicit `'unknown'` bucket instead of an accidental falsy-key collision. The `resetAtMs` validator now requires a non-negative number.
- Revisit criteria: Same as the original entry — revisit if a KV/Redis-backed adapter becomes necessary; additionally revisit the 5,000-key hard cap if legitimate traffic ever has more than 5,000 concurrently active distinct client keys per warm instance (unlikely at this app's scale).
- Plan:
  - Goal: Resolve automated-reviewer findings on PR #211 before merge.
  - Seams: RateLimitSeam.
  - Files: `src/lib/seams/rate-limit-seam/{contract,limiter,validators,test}.ts`, `src/lib/server/enforce-rate-limit.ts`, `tests/unit/enforce-rate-limit.test.ts`, `DECISIONS.md`, `docs/evidence/2026-07-02/*` (refreshed).
  - Commands: `npm run rewind -- --seam RateLimitSeam`, `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run verify`.
- Self-critique: The main risk is that oldest-first hard-cap eviction could let an attacker force-evict a legitimate client's active window during a flood, temporarily resetting that client's count. This is an accepted trade-off of a best-effort, no-new-infrastructure limiter — the alternative (unbounded memory growth) is strictly worse, and the behavior is proven via a dedicated test (`caps memory growth under a flood of always-fresh distinct keys`) rather than asserted.
