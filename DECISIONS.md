...
## 2026-06-18 - Centralize timeout/abort error classification

- Decision: Make `isTimeoutError`/`isTimeoutMessage` in `src/lib/core/http-resilience.ts` the single source of truth for "is this error a timeout," and have `generate-pipeline.ts` and `meechie-studio-text-pipeline.ts` call it instead of each maintaining their own regex.
- Context: A scheduled autonomous audit of the repo (picking the single hardest fix from a list of ten candidate upgrades) found three independent, subtly different timeout-detection regexes deciding whether an outbound provider failure should surface as HTTP 502 (generic) or 504 (timeout): `http-resilience.ts` used `/timed out/i`, `generate-pipeline.ts`'s `imageExceptionResponse` used `/timeout/i`, and `meechie-studio-text-pipeline.ts`'s `isProviderTimeout` used `/\b(timeout|timed out)\b/i`. These can disagree on the same input — e.g. a plain `Error` with message "request timed out" and no explicit `.name` was a timeout to `http-resilience.ts` but a generic failure to `generate-pipeline.ts`. `provider-adapter.adapter.ts` already correctly used the shared `isAbortError`/`isTimeoutError` helpers, so it was the reference implementation; the other two pipelines were the outliers.
- Alternatives: Redo the dual-seam-layout consolidation that 5 other open PRs (#151, #156, #159, #161, #164) already attempt; rejected because doing a 6th independent fix for an already-fixed problem would add to, not reduce, the open-PR backlog. Introduce a new typed `PROVIDER_TIMEOUT` error code threaded through `ProviderAdapterSeam`'s contract; rejected for this pass because it touches a seam contract and would require the full Contract → Probe → Fixtures → Mock → Test → Adapter cycle for a fix that core-logic deduplication already resolves without a contract change. Leave the three regexes as-is; rejected because the inconsistency is demonstrable and silently changes which HTTP status callers see for the same underlying failure.
- Consequences: `generate-pipeline.ts` and `meechie-studio-text-pipeline.ts` no longer own their own timeout regex; both defer to `http-resilience.ts`. The canonical pattern was initially shipped as `/\b(timeout|timed out)\b/i`; two independent automated PR reviewers (Gemini Code Assist and Codex) caught before merge that this was *not* actually a strict superset of the old `/timeout/i` used by `generate-pipeline.ts` — `\b` treats `_` as a word character, so snake_case provider error codes like `request_timeout` or `connect_timeout` would regress from a 504 timeout back to a generic 502, the opposite of this change's intent. The pattern was corrected to `/timeout|timed out/i` (no word boundaries) to restore the true superset property, with regression tests added for snake_case inputs. The same review pass also flagged that `isTimeoutError`'s message extraction (`error instanceof Error ? error.message : String(error)`) would stringify a plain non-Error object that has a `message` property to `"[object Object]"` instead of reading the message; `errorMessage` was made defensive to read `.message` off any object that has one, with tests covering both a timeout and non-timeout plain-object case. One test fixture in `provider-adapter-helpers.test.ts` used the literal message "Network timeout" to mean "a generic, non-timeout failure"; that fixture was renamed to "ECONNREFUSED" since the old assertion was relying on a regex gap rather than expressing real intent. Added direct unit tests for `isTimeoutMessage`/`isTimeoutError` (previously untested), a regression test in `tests/unit/api-generate.test.ts` proving the previously-missed case is now classified as 504, and a new test in `provider-adapter-helpers.test.ts` proving a named `TimeoutError` is still rewritten to the fixed user-facing message.
- Revisit criteria: Revisit if seam contracts move toward typed error codes (e.g. a dedicated `PROVIDER_TIMEOUT` code) instead of message-text sniffing, which would let downstream pipelines stop parsing English messages entirely.
- Plan:
  - Goal: Remove the only demonstrable cross-module inconsistency among the repo's timeout-classification logic without touching any seam contract.
  - Seams: None (core pipeline logic only — no contract, mock, adapter, or fixture changes).
  - Files: `src/lib/core/http-resilience.ts`, `src/lib/core/generate-pipeline.ts`, `src/lib/core/meechie-studio-text-pipeline.ts`, `tests/unit/http-resilience.test.ts`, `tests/unit/api-generate.test.ts`, `tests/unit/provider-adapter-helpers.test.ts`, `CHANGELOG.md`, `LESSONS_LEARNED.md`, `DECISIONS.md`.
  - Commands: `npm run check`, `npm run lint`, `npm run test:unit`, `npm run verify`.
- Self-critique: The main risk is silently widening or narrowing which errors get treated as timeouts, which changes user-facing status codes. The first draft of this fix asserted the new canonical regex was a strict superset of all three prior patterns without actually checking the `\b`-on-underscore edge case — that claim was false, and the regression (snake_case provider codes silently downgraded from 504 to 502) would have shipped if two independent automated reviewers (Gemini Code Assist, Codex) had not both caught it on the same PR before merge. Lesson: "superset" claims about a regex change need an explicit counter-example check (try the new pattern against every input the old ones matched), not just "it looks broader." The one test that broke before this correction (`provider-adapter-helpers.test.ts`) was unrelated — it was asserting on a fixture string that coincidentally dodged the old narrower regex rather than on real intent, so fixing the fixture was correct rather than a sign the change is unsafe. This was not run through `npm run cipher:gate` since it touches no seam contract/adapter/mock/fixture path (confirmed by reading `scripts/cipher-gate.mjs`'s `watchPaths`, none of which include `src/lib/core` or `tests/unit`); `npm run check`, `npm run lint`, `npm run test:unit`, and the full `npm run verify` chain all passed clean after the correction, with evidence under `docs/evidence/2026-06-18/`.
