...
- Plan:
  ...
  - Seams: MeechieVoiceSeam, MeechieToolSeam, MeechieStudioTextSeam.
  - Files (every path touched, source and record alike): `src/lib/seams/meechie-voice-seam/voice-pack.ts`, `contracts/meechie-quote.contract.ts`, `src/lib/seams/meechie-voice-seam/contract.ts`, `contracts/meechie-voice.contract.ts`, `src/lib/seams/meechie-voice-seam/test.ts`, `src/lib/core/meechie-studio.ts`, `tests/unit/meechie-studio.test.ts`, `fixtures/meechie-voice/sample.json`, `fixtures/meechie-voice/malformed-pack.json`, `DECISIONS.md`, `docs/evidence/2026-08-25/canon-ten-check.txt`, `docs/evidence/2026-08-25/canon-ten-lint.txt`, `docs/evidence/2026-08-25/canon-ten-full-tests.txt`, `docs/evidence/2026-08-25/canon-ten-red-proof.txt`, `docs/evidence/2026-08-25/canon-ten-verify-ci.txt`, `docs/evidence/2026-08-25/rewind-MeechieVoiceSeam.txt`, `docs/evidence/2026-08-25/rewind-MeechieVoiceSeam(self-contained).txt`, `docs/evidence/2026-08-25/rewind-MeechieToolSeam.txt`, `docs/evidence/2026-08-25/rewind-MeechieToolSeam(self-contained).txt`, `src/routes/studio-state.svelte.ts`, `tests/integration/meechie-tool-seam.test.ts`, `tests/integration/image-generation-seam.test.ts`, `tests/setup/env-dynamic-private.ts`, `vitest.integration.config.ts`, `docs/evidence/2026-08-25/canon-ten-provider-probe.txt`, `docs/evidence/2026-08-25/canon-ten-draft-migration-proof.txt`, `tests/integration/meechie-studio-text-seam.test.ts`, `docs/evidence/2026-08-25/rewind-PromptAssemblySeam.txt`, `docs/evidence/2026-08-25/canon-ten-integration.txt`, `HANDOFF-CANON.md`.
  - Commands: `npm run verify`, `npm test`, `npm run rewind -- --seam "MeechieVoiceSeam (self-contained)"`, `npm run rewind -- --seam "MeechieToolSeam (self-contained)"`, `npm run rewind -- --seam MeechieVoiceSeam`, `npm run rewind -- --seam MeechieToolSeam`, `npm run check`, `npm run lint`, `FEATURE_INTEGRATION_TESTS=true npm run test:integration`.
...
- Draft migration: The studio decided whether a saved draft held real generated wording by comparing its page title against the CURRENT placeholder. Changing the placeholder therefore made every older untouched draft look generated, so the studio restored retired wording as though the user had produced it and allowed exporting it. PR #227 had the same latent flaw when it moved the placeholder from IN THIS ECONOMY to I DON'T ACT. `PLACEHOLDER_PAGE_TITLES` now lists every placeholder title ever shipped and `isPlaceholderPageTitle()` tests membership, so the check survives future placeholder changes. Red-proved by narrowing it back to the single-sentinel form and watching the historical-title cases fail. See docs/evidence/2026-08-25/canon-ten-draft-migration-proof.txt.
- Provider probe: The voice pack is the system prompt for every Meechie tool call, so halving it changes live provider behaviour that mocks cannot observe. Probed against live xAI across random_meechie, red_flag_or_run, and rate_excuse: all contract-valid, none echoing a canon line verbatim. Running it required fixing the integration harness, which could not resolve `$env/dynamic/private` and so failed at import for every integration test on main. See docs/evidence/2026-08-25/canon-ten-provider-probe.txt.
- Studio-text probe: The voice pack also builds `VOICE_EXAMPLES` in `src/lib/core/meechie-studio-text-pipeline.ts`, so `/api/meechie-studio-text` — the primary generate flow on the main page — receives a changed prompt too. An earlier revision probed only MeechieToolSeam and named only two seams here, which was no evidence for the primary flow. `tests/integration/meechie-studio-text-seam.test.ts` now probes `meechieStudioTextAdapter` live: contract-valid output and no verbatim canon echo.
- Draft classification: an earlier revision of this fix matched placeholders on page title alone, which traded one error for its mirror — a draft holding genuine generated output that happened to be titled `I DON'T ACT` would be classified as untouched and the user's saved preview would silently disappear. `PLACEHOLDER_SHAPES` now records title plus item labels for every placeholder ever shipped and `isUntouchedPlaceholder()` matches the whole shape, so real generated output would have to reproduce a retired placeholder exactly to be misread. Red-proved by narrowing it back to title-only.
- Probe timeouts: `provider-adapter.adapter.ts` allows 3 attempts at a 60s chat timeout plus backoff, so one call can legitimately exceed three minutes. An earlier revision ran three sequential tool calls under a single 120s budget, which would have turned valid retry behaviour into a false integration failure. Each call now has its own case at a timeout derived from that budget.
- Cipher Gate:
  - Date: 2026-08-25
  - Seams: MeechieVoiceSeam, MeechieToolSeam, MeechieStudioTextSeam
  - Evidence: docs/evidence/2026-08-25/canon-ten-provider-probe.txt; docs/evidence/2026-08-25/canon-ten-draft-migration-proof.txt; docs/evidence/2026-08-25/canon-ten-verify-ci.txt; docs/evidence/2026-08-25/canon-ten-red-proof.txt; docs/evidence/2026-08-25/canon-ten-full-tests.txt; docs/evidence/2026-08-25/canon-ten-check.txt; docs/evidence/2026-08-25/canon-ten-lint.txt; docs/evidence/2026-08-25/rewind-MeechieVoiceSeam(self-contained).txt; docs/evidence/2026-08-25/rewind-MeechieToolSeam(self-contained).txt; docs/evidence/2026-08-25/rewind-MeechieVoiceSeam.txt; docs/evidence/2026-08-25/rewind-MeechieToolSeam.txt
  ...
  - Risks: `npm run verify` and `npm test` are green on CI — both verify jobs pass on head 1765ebd along with all 10 check runs, and the SonarCloud quality gate passes with 0 new issues and 0.0% duplication on new code. They are NOT green in the sandboxed dev container, where `tests/unit/api-generate.test.ts` times out at the 5000ms default. Measured rather than assumed: 6.79s total with 5.83s of test time against roughly 2.8s of user CPU, so it is waiting on I/O rather than computing. That endpoint does HTTP with retry and backoff and this container proxies outbound traffic. It fails identically on a clean checkout of the base commit, so it predates this change. Not patched here: raising that one test's timeout would widen this change into a file it does not otherwise touch and would alter a shared gate to accommodate one environment's network latency. See docs/evidence/2026-08-25/canon-ten-verify-ci.txt for the command output and the full reasoning. Full suite 532 passing locally. Seam rewinds green: 17 (voice self-contained), 6 (tool self-contained), 4 and 5 (legacy rows).
