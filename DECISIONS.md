...
- Self-critique: The main risk is silently widening which errors get treated as timeouts, which changes user-facing status codes. Because the new canonical regex is a strict superset of all three prior patterns, no previously-classified case flips from timeout to non-timeout — only the reverse, and that reverse case is exactly the bug being fixed. The one test that broke (`provider-adapter-helpers.test.ts`) confirmed this: it was asserting on a fixture string that coincidentally dodged the old narrower regex rather than on real intent, so fixing the fixture was correct rather than a sign the change is unsafe. This was not run through `npm run cipher:gate` since it touches no seam contract/adapter/mock/fixture path (confirmed by reading `scripts/cipher-gate.mjs`'s `watchPaths`, none of which include `src/lib/core` or `tests/unit`); `npm run check`, `npm run lint`, `npm run test:unit` (530 passed, 1 skipped), and the full `npm run verify` chain all passed clean, with evidence under `docs/evidence/2026-06-18/`.
