...
## 2026-07-03 - Four quick-win fixes: Gemini base URL, README drift, redundant score label, portrait file extension
- Date: 2026-07-03
- Decision: Fix four small, verified bugs found during a codebase sweep: (1) `WigTryOnSeam`'s adapter ignored `AppConfigSeam`'s already-validated `geminiBaseUrl` field and always called the hardcoded production Gemini host, so `GEMINI_BASE_URL` had no effect; (2) `README.md`'s Tech Stack and Environment Variables tables never mentioned Gemini/`GEMINI_API_KEY`/`GEMINI_BASE_URL` even though the wig try-on feature requires them and `.env.example`/`docs/seams.md` both document them; (3) `/rate-his-excuse` rendered a hardcoded "out of 10" label next to `result.headline`, which the `MeechieToolSeam` adapter already formats as `"N/10"`, producing visible text like "8/10 out of 10"; (4) `WigTryOnStudio.svelte`'s "Save Portrait" download link hardcoded a `.png` filename even though `portraitMimeType` comes directly from Gemini and can be `image/jpeg` or `image/webp` (the same value is already handled as a three-format union elsewhere in `studio-state.svelte.ts`).
- Context: Found via a targeted read-only sweep of core pipelines, routes, components, and adapters looking for small, low-risk, verifiable defects (not style nitpicks). Each was confirmed by reading the actual producing code (config validator, tool adapter, mime-type handling) before fixing.
- Alternatives: For (1), could have left `geminiBaseUrl` unused and instead removed it from the contract; rejected because the field is already part of `AppConfigSeam`'s validated contract and other consumers may rely on it being configurable. For (3), could have stripped the `/10` suffix in the adapter's `headline` formatting instead of removing the page's label; rejected as higher blast radius since `headline` is reused verbatim elsewhere. For (4), could have required the caller to pass an explicit extension; rejected because the mime type is already available on the data URL and parsing it locally matches the existing pattern in `studio-state.svelte.ts`.
- Consequences: `GEMINI_BASE_URL` overrides now take effect for wig try-on requests (default behavior unchanged); README documents the Gemini env vars; the excuse-rating page no longer shows duplicated "N/10 out of 10" text; downloaded try-on portraits now get a correct `.png`/`.jpg`/`.webp` extension matching their actual image data.
- Revisit criteria: Revisit if `WigTryOnSeam` adds a dedicated contract test suite for base-URL overrides beyond the one adapter test added here, or if the rate-his-excuse scoring UI is redesigned.
- Plan:
  - Goal: Fix four small, independently-verified bugs without altering any seam contracts or adding/removing seams.
  - Seams: WigTryOnSeam (adapter-only fix using an existing AppConfigSeam field; no contract/mock/fixture change).
  - Files: `src/lib/adapters/wig-try-on-seam/index.ts`, `src/lib/seams/wig-try-on-seam/test.ts`, `README.md`, `src/routes/rate-his-excuse/+page.svelte`, `src/lib/components/studio/WigTryOnStudio.svelte`, `DECISIONS.md`, `docs/evidence/2026-07-03/*`.
  - Commands: `npm run lint`, `npm run check`, `npm run test`, `npm run verify`.
- Self-critique: The main risk is the `WigTryOnSeam` adapter change silently altering default behavior; mitigated because the default `geminiBaseUrl` value is unchanged (`https://generativelanguage.googleapis.com`) and a new contract test proves the override path works. The README and page-copy changes are docs/UI-only with no logic risk. The download-extension fix only changes a client-side filename, not the downloaded bytes.
- Follow-up (same day, PR #213 review): Automated PR reviewers (Gemini Code Assist, GitHub Copilot, Codex) flagged three real edge cases in the initial `WigTryOnSeam` fix, all addressed before merge: (a) a trailing slash on a configured `geminiBaseUrl` would have produced a double-slash endpoint URL, fixed by stripping a trailing slash before building the endpoint; (b) an explicitly blank `GEMINI_BASE_URL` (present but empty) bypasses `AppConfigSeam`'s `??`-based default and reaches the adapter as `''`, which would have built a relative `/v1beta/...` URL and thrown a confusing network error instead of a clear config error — fixed with the same empty-check pattern already used for `geminiApiKey`; (c) the portrait download extension regex was case-sensitive and would silently fall back to `.png` for uppercase mime types (e.g. `image/JPEG`) — fixed by adding the `/i` flag and lowercasing the matched extension. Two new adapter tests cover (a) and (b). A related Sourcery suggestion to extract a shared mime-type-parsing utility between `WigTryOnStudio.svelte` and `studio-state.svelte.ts`'s `parseTryOnPortraitImage` was deliberately not applied in this PR: that method also drives `OutputPackagingSeam` image conversion and merging its stricter format contract with the download-only extension helper is a small design decision better suited to its own change, not bundled into this quick-win fix set.

- Cipher Gate:
  - Date: 2026-07-03
  - Seams: WigTryOnSeam
  - Evidence: docs/evidence/2026-07-03/verify.txt; docs/evidence/2026-07-03/test.txt; docs/evidence/2026-07-03/chamber-lock.json; docs/evidence/2026-07-03/shaolin-lint.json; docs/evidence/2026-07-03/seam-ledger.json; docs/evidence/2026-07-03/clan-chain.json; docs/evidence/2026-07-03/proof-tape.json
  - Summary: Fixed WigTryOnSeam's unused geminiBaseUrl config (plus trailing-slash normalization and an empty-base-URL config-error guard added during review), README Gemini env-var drift, a redundant "out of 10" score label, and a mislabeled/case-sensitive portrait download extension; full verify green (522 tests passed, 1 skipped).
  - Risks: The WigTryOnSeam base-URL fix changes real request routing when GEMINI_BASE_URL is overridden; covered by adapter tests but not by a live probe against a real alternate endpoint.
