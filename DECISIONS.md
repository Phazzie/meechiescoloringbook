...
## 2026-07-06 - Add RateLimitSeam to guard all six AI-provider-backed API routes

- Date: 2026-07-06
- Decision: Add a new self-contained `RateLimitSeam` (pure sliding-window counter, in-memory adapter) and wire it into every API route that calls a paid external AI provider (`/api/generate`, `/api/image-generation`, `/api/chat-interpretation`, `/api/meechie-studio-text`, `/api/tools`, `/api/wig-try-on`), returning HTTP 429 with a `Retry-After` header once a client (keyed by `getClientAddress()`) exceeds its per-route limit.
- Context: This was identified as the single hardest unresolved gap in a full-repo audit (see the ranked list in the corresponding PR description). Every route reads its own server-side `XAI_API_KEY`/Gemini key from env (confirmed via `src/lib/adapters/image-provider-config-seam/index.ts`) and none of the six routes had any throttling, auth, or request-size limit (confirmed via `grep -rn "rate.?limit" src/` returning nothing before this change). An unauthenticated caller could loop requests against `/api/generate` or `/api/wig-try-on` and run up the app owner's xAI/Gemini bill with no server-side ceiling. This is harder than it looks in this stack because Vercel functions are stateless/ephemeral, so a fully correct distributed limiter needs external shared storage (Redis/Upstash/Vercel KV) that is not part of the current dependency set and would require new paid infrastructure and credentials this session does not have.
- Alternatives: Add a distributed limiter backed by a new Redis/KV dependency; rejected for this pass because it requires provisioning external infrastructure and credentials outside this session's scope, and would violate "Adapters may depend on third-party libraries only behind seams" without first proving the infra choice. Do nothing until abuse is observed; rejected because the cost exposure is already live in production with zero mitigation. Add an API-key/auth gate instead; rejected as a larger, separate product decision (the app currently has no user accounts) and orthogonal to bounding raw request volume. The chosen in-memory sliding-window approach is a real, immediate mitigation (bounds bursts within a warm instance) while being honest about its limitation (does not share counters across cold starts or concurrent instances).
- Consequences: All six API routes now reject excess calls per client with `429` + `{ ok: false, error: { code: 'RATE_LIMIT_EXCEEDED', message } }` and a `Retry-After` header; limits are 5/min for the three expensive image/multi-image routes (`generate`, `image-generation`, `wig-try-on`) and 10-20/min for the cheaper text/tool routes (`chat-interpretation`, `meechie-studio-text`, `tools`). Existing endpoint tests were updated to stub `getClientAddress` with a unique value per call (module-scoped counter) so shared in-memory state does not cross-contaminate unrelated test cases within the same file; dedicated 429 tests were added for `/api/chat-interpretation` and `/api/image-generation` using a fixed client address repeated past the limit.
- Revisit criteria: Revisit if traffic/cost data shows abuse surviving cold starts (i.e., attackers rotating across instances), at which point move to a shared external store (Redis/Upstash/Vercel KV) behind the same `RateLimitSeam` contract — the contract's `checkAndConsume(key, now)` shape does not need to change, only the adapter. Also revisit if the app adds user accounts/API keys, since per-account limits would likely replace or augment per-IP limits.
- Plan:
  - Goal: Close the no-rate-limiting gap on all six paid-AI-provider-backed API routes with a new pure `RateLimitSeam`.
  - Seams: RateLimitSeam (new).
  - Files: `docs/seams.md`, `DECISIONS.md`, `LESSONS_LEARNED.md`, `CHANGELOG.md`, `src/lib/seams/rate-limit-seam/contract.ts`, `src/lib/seams/rate-limit-seam/window.ts`, `src/lib/seams/rate-limit-seam/fixtures.ts`, `src/lib/seams/rate-limit-seam/mock.ts`, `src/lib/seams/rate-limit-seam/probe.ts`, `src/lib/seams/rate-limit-seam/test.ts`, `src/lib/adapters/rate-limit-seam/index.ts`, `src/lib/server/rate-limit-response.ts`, `src/routes/api/generate/+server.ts`, `src/routes/api/image-generation/+server.ts`, `src/routes/api/chat-interpretation/+server.ts`, `src/routes/api/meechie-studio-text/+server.ts`, `src/routes/api/tools/+server.ts`, `src/routes/api/wig-try-on/+server.ts`, `tests/unit/api-chat-interpretation.test.ts`, `tests/unit/api-generate.test.ts`, `tests/unit/api-image-generation.test.ts`, `tests/unit/api-meechie-studio-text-endpoint.test.ts`, `tests/unit/api-tools.test.ts`, `tests/unit/api-wig-try-on.test.ts`, `docs/evidence/2026-07-06/*`.
  - Commands: `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run rewind -- --seam RateLimitSeam`, `npm run verify`, `npm audit`.
- Self-critique: The riskiest assumption is that per-instance in-memory counters meaningfully reduce abuse risk given Vercel's serverless model; they do bound rapid-fire loops within a warm instance (the common abuse pattern from a single script/browser tab) but do not prevent a determined attacker from spreading requests across many cold starts or IPs. This is stated plainly above rather than claimed as a complete fix. A second risk is that six existing endpoint test files needed `getClientAddress` stubs added or the whole suite would fail with `TypeError: getClientAddress is not a function`; this was caught by running `npm test` locally (not assumed) and fixed by giving each test a unique per-call client address so pre-existing assertions are unaffected by the new shared rate-limit state, with dedicated tests added to prove the 429 path itself. `npm run verify` is green with fresh evidence under `docs/evidence/2026-07-06/`.

- Cipher Gate:
  - Date: 2026-07-06
  - Seams: RateLimitSeam
  - Evidence: docs/evidence/2026-07-06/rate-limit-check.txt; docs/evidence/2026-07-06/rate-limit-lint.txt; docs/evidence/2026-07-06/rate-limit-build.txt; docs/evidence/2026-07-06/rewind-RateLimitSeam.txt; docs/evidence/2026-07-06/test.txt; docs/evidence/2026-07-06/verify.txt; docs/evidence/2026-07-06/chamber-lock.json; docs/evidence/2026-07-06/shaolin-lint.json; docs/evidence/2026-07-06/assumption-alarm.json; docs/evidence/2026-07-06/seam-ledger.json; docs/evidence/2026-07-06/seam-ledger.md; docs/evidence/2026-07-06/clan-chain.json; docs/evidence/2026-07-06/clan-chain.md; docs/evidence/2026-07-06/proof-tape.json; docs/evidence/2026-07-06/proof-tape.md; docs/evidence/2026-07-06/npm-audit.txt
  - Summary: Added a pure, contract-tested RateLimitSeam and wired it into all six paid-AI-provider-backed API routes, returning 429 + Retry-After once a client exceeds its per-route sliding-window limit; check/lint/test/build/rewind/verify are all green.
  - Risks: In-memory per-instance counters do not share state across cold starts or concurrent serverless instances, so this mitigates but does not eliminate abuse risk; a distributed store is the documented follow-up if traffic data shows it is needed.

## 2026-07-06 - Fix RateLimitSeam review findings (NaN retryAfterMs, unbounded Map, unguarded getClientAddress)

- Date: 2026-07-06
- Decision: Apply three automated-review findings from PR #222 (Gemini Code Assist and GitHub Copilot both independently flagged the same two core issues) to `RateLimitSeam`: (1) guard `evaluate()` against `NaN` retryAfterMs when `rule.limit <= 0` by falling back to `now`/computing the true minimum timestamp instead of assuming `kept[0]` is sorted-oldest; (2) bound the in-memory adapter's `Map` with an opportunistic prune sweep once tracked keys exceed 10,000; (3) add a `safeClientAddress()` helper so a throwing `getClientAddress()` falls back to a shared `'unknown'` bucket instead of crashing the route with a non-JSON 500.
- Context: Both Gemini Code Assist and Copilot review bots independently reported the NaN and unbounded-Map issues on the same lines; a human read of `window.ts`/`index.ts` confirmed both were real (an empty `kept` array with `rule.limit <= 0` does produce `kept[0] === undefined`, and the adapter's `Map` never evicted a key once its window fully elapsed). Gemini additionally flagged that every route calls `getClientAddress()` unguarded; SvelteKit's own docs note this can throw on platforms/proxies that don't expose a client address, and none of the six routes wrapped it, so a throw would surface as a generic 500 instead of the route's normal JSON contract. Copilot additionally noted `kept[0]` assumes insertion-sorted timestamps.
- Alternatives: Ignore the zero/negative-limit case since no current call site passes one; rejected because the contract type does not forbid it and a latent NaN-producing bug in a shared seam is cheap to close now versus expensive to debug later in production. Cap the Map with a hard LRU eviction library; rejected as unnecessary complexity for a documented stop-gap in-memory limiter — an opportunistic sweep triggered only when the cap is reached is simpler and sufficient. Let `getClientAddress()` throw and rely on SvelteKit's generic error page; rejected because it breaks the API's JSON-error contract that every other failure path in these routes honors.
- Consequences: `evaluate()` now returns a finite `retryAfterMs` for any `rule.limit` value and computes the retry delay from the true minimum timestamp rather than assuming order; the adapter's `Map` self-prunes once it holds ≥10,000 keys and a new key arrives, evicting any key whose entire timestamp history has fallen outside its window; all six routes resolve the rate-limit key via `safeClientAddress(getClientAddress)` (new `src/lib/server/client-address.ts`) instead of calling `getClientAddress()` directly.
- Revisit criteria: Revisit the prune-threshold constant (`MAX_TRACKED_KEYS = 10_000`) if real traffic data shows the sweep itself becomes a hot path, or if the adapter moves to a distributed store per the original Cipher Gate's follow-up criterion (an external store would have its own TTL/eviction and would not need this in-process sweep).
- Plan:
  - Goal: Close three automated-review findings (NaN retryAfterMs, unbounded Map growth, unguarded getClientAddress) on the RateLimitSeam PR without changing its public contract.
  - Seams: RateLimitSeam.
  - Files: `src/lib/seams/rate-limit-seam/window.ts`, `src/lib/seams/rate-limit-seam/test.ts`, `src/lib/adapters/rate-limit-seam/index.ts`, `src/lib/server/client-address.ts`, `tests/unit/client-address.test.ts`, `src/routes/api/generate/+server.ts`, `src/routes/api/image-generation/+server.ts`, `src/routes/api/chat-interpretation/+server.ts`, `src/routes/api/meechie-studio-text/+server.ts`, `src/routes/api/tools/+server.ts`, `src/routes/api/wig-try-on/+server.ts`, `DECISIONS.md`, `docs/evidence/2026-07-06/rate-limit-review-fixes-*`.
  - Commands: `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run rewind -- --seam RateLimitSeam`, `npm run verify`.
- Self-critique: The riskiest assumption is that the black-box `checkAndConsume` contract can adequately prove the Map-pruning fix without exposing internal store size for direct inspection; the added test exercises the exact code path (10,001 keys, crossing the prune threshold) and asserts correct allow/block behavior afterward, which catches a crash or corrupted-state regression even though it cannot assert the internal `Map.size` shrank. A second risk is that `safeClientAddress` groups all address-resolution failures into one `'unknown'` bucket, meaning distinct clients whose address genuinely can't be resolved would rate-limit each other; this is an acceptable, documented degradation given how rarely `getClientAddress()` is expected to throw on `adapter-vercel`. `npm run check/lint/test/build/rewind/verify` are all green after the fix.
- Cipher Gate:
  - Date: 2026-07-06
  - Seams: RateLimitSeam
  - Evidence: docs/evidence/2026-07-06/rate-limit-review-fixes-check.txt; docs/evidence/2026-07-06/rate-limit-review-fixes-lint.txt; docs/evidence/2026-07-06/rate-limit-review-fixes-build.txt; docs/evidence/2026-07-06/rate-limit-review-fixes-test.txt; docs/evidence/2026-07-06/rate-limit-review-fixes-verify.txt; docs/evidence/2026-07-06/rate-limit-review-fixes-rewind.txt
  - Summary: Fixed the NaN-retryAfterMs edge case, bounded the in-memory rate-limit Map with an opportunistic prune sweep, and guarded every route's `getClientAddress()` call behind a safe fallback, all in response to independent Gemini Code Assist and Copilot review findings on PR #222.
  - Risks: None new; this closes gaps in the mitigation added earlier the same day without changing its documented in-memory/per-instance limitation.

## 2026-07-06 - Address CodeRabbit review findings on PR #222

- Date: 2026-07-06
- Decision: Apply the still-valid, small findings from CodeRabbit's review of PR #222 — fix a route-count typo in this file's own Cipher Gate summary, strengthen both existing 429 boundary tests to assert requests under the limit are not rejected, extract the `nextClientAddress` test helper (duplicated across all six `tests/unit/api-*.test.ts` files) into `tests/helpers/next-client-address.ts`, and update every rate-limited route's top-of-file info-flow comment to mention the new rate-limit gate. Skip the remaining CodeRabbit findings with reasons: the NaN/unbounded-Map findings were already fixed in the prior commit responding to Gemini/Copilot (CodeRabbit's review ran against the pre-fix commit); the suggested `withRateLimit`/`hooks.server.ts` middleware refactor is a heavy-lift architectural change that would touch all six routes' structure, explicitly marked a "nitpick" by the tool, and not requested by the user — left as-is pending an explicit decision to take it on; the `mock.ts` precedent question, the `rate-limit-response.ts` third-party-dependency question, and the two "confirm rate-limiting belongs in the route not the pipeline" questions are all answered by existing precedent (`parseRequestBody` already runs the same way in every route) and needed no code change; the "add a dedicated 429 test to every route" suggestion was explicitly marked "not a blocker" and is redundant with the seam-level contract tests plus the two existing route-level 429 tests that already prove the identical wiring pattern; the LESSONS_LEARNED.md MD022 blank-line finding does not apply — running `npx markdownlint-cli2` locally shows the exact same "no blank line below the heading" pattern on every pre-existing dated entry in that file (this repo does not run markdownlint as part of `npm run verify`), so fixing only the new entry would make it inconsistent with the file's established format.
- Context: CodeRabbit posted its full review after Gemini Code Assist and Copilot had already been addressed in the prior commit; several of its findings were therefore stale duplicates of already-fixed issues, and several others were explicit "nitpick"/"heavy lift" suggestions rather than defects.
- Alternatives: Apply every suggestion including the shared rate-limit middleware refactor; rejected because it changes the architecture of all six routes and was not requested — worth a dedicated follow-up decision rather than folding into a review-response commit. Ignore all findings as noise; rejected because several (route-count typo, boundary-test rigor, test-helper duplication, stale info-flow comments) were real and cheap to fix.
- Consequences: `DECISIONS.md`'s rate-limit Cipher Gate summary now correctly says "three" routes; both existing 429 tests (`/api/chat-interpretation`, `/api/image-generation`) now assert every pre-limit request is not `429` in addition to asserting the final one is; `tests/helpers/next-client-address.ts` is the single source for the unique-client-address test pattern, imported by all six `api-*.test.ts` files; every rate-limited route's top comment documents the rate-limit gate in its info-flow line.
- Revisit criteria: Revisit the shared rate-limit wrapper/middleware suggestion if a seventh paid-provider route is added and the copy-pasted wiring becomes harder to keep consistent by hand.
- Plan:
  - Goal: Apply CodeRabbit's still-valid small findings on PR #222 without taking on the heavy-lift middleware refactor it also suggested.
  - Seams: RateLimitSeam.
  - Files: `DECISIONS.md`, `tests/helpers/next-client-address.ts`, `tests/unit/api-chat-interpretation.test.ts`, `tests/unit/api-generate.test.ts`, `tests/unit/api-image-generation.test.ts`, `tests/unit/api-meechie-studio-text-endpoint.test.ts`, `tests/unit/api-tools.test.ts`, `tests/unit/api-wig-try-on.test.ts`, `src/routes/api/generate/+server.ts`, `src/routes/api/image-generation/+server.ts`, `src/routes/api/chat-interpretation/+server.ts`, `src/routes/api/meechie-studio-text/+server.ts`, `src/routes/api/tools/+server.ts`, `src/routes/api/wig-try-on/+server.ts`, `docs/evidence/2026-07-06/rate-limit-coderabbit-fixes-*`.
  - Commands: `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run verify`, `npx markdownlint-cli2 LESSONS_LEARNED.md`.
- Self-critique: The riskiest judgment call is declining the middleware/hooks.server.ts refactor; it is a real maintainability improvement, but changing how all six routes are structured is a bigger architectural decision than a review-response commit should make unilaterally, and CodeRabbit itself labeled it a heavy-lift nitpick rather than a defect. `npm run check/lint/test/build/verify` are all green after the accepted fixes.
- Cipher Gate:
  - Date: 2026-07-06
  - Seams: RateLimitSeam
  - Evidence: docs/evidence/2026-07-06/rate-limit-coderabbit-fixes-check.txt; docs/evidence/2026-07-06/rate-limit-coderabbit-fixes-lint.txt; docs/evidence/2026-07-06/rate-limit-coderabbit-fixes-build.txt; docs/evidence/2026-07-06/rate-limit-coderabbit-fixes-test.txt; docs/evidence/2026-07-06/rate-limit-coderabbit-fixes-verify.txt
  - Summary: Fixed a route-count typo, strengthened two 429 boundary tests, deduplicated the `nextClientAddress` test helper across six test files, and corrected six routes' info-flow comments, in response to CodeRabbit's review of PR #222; declined the heavy-lift middleware refactor suggestion pending explicit user direction.
  - Risks: None new.

## 2026-07-06 - Add RateLimitSeam validators.ts and fixture-sourced mock (Codex review)

- Date: 2026-07-06
- Decision: Add `src/lib/seams/rate-limit-seam/validators.ts` (Zod schema requiring `limit` to be a positive integer and `windowMs` to be a positive number) and validate the rule at `createRateLimitSeam` construction time; move the mock's "block" scenario error object out of `mock.ts` and into a new `rateLimitExceededFixture` in `fixtures.ts`, matching every other seam's established pattern.
- Context: The Codex GitHub reviewer flagged that `src/lib/seams/AGENTS.md` requires every seam folder to include a `validators.ts` and that mocks must load fixtures under specific scenarios rather than invent data inline; a check confirmed all 15 other seam folders under `src/lib/seams/` already have a `validators.ts` and `RateLimitSeam` was the only one missing it. Without a validator, `createRateLimitSeam({ limit: 0, ... })` would silently construct a seam that always returns `RATE_LIMIT_EXCEEDED` for every request — a misconfiguration that previously could only be caught by manually inspecting the route wiring, not by the seam itself. Codex also correctly noted the mock's block-scenario error was hardcoded inline rather than sourced from `fixtures.ts`.
- Alternatives: Rely solely on the earlier NaN-guard fix in `evaluate()` (treating `limit <= 0` as "always block with a finite retry delay") without adding a validator; rejected because that still lets a misconfigured rule reach production silently instead of failing fast at construction, and it left `RateLimitSeam` as the one seam folder without the mandated `validators.ts`. The `evaluate()` NaN guard is kept regardless, since it is still correct defense-in-depth for the pure function in isolation.
- Consequences: `createRateLimitSeam` now throws a `ZodError` immediately if called with a non-positive `limit` or `windowMs`, so a route misconfiguration fails at module load instead of shipping a rate limiter that either always or never blocks; the mock's block scenario now returns the exact `rateLimitExceededFixture` object, so a route test and the seam's own contract tests can never drift on the error shape/message/retry timing.
- Revisit criteria: Revisit if `RateLimitRule` grows additional fields (e.g., a per-route key-derivation strategy) that also need schema-level constraints.
- Plan:
  - Goal: Close the Codex-flagged `validators.ts`/fixture-sourcing gap on `RateLimitSeam` without changing its public contract shape.
  - Seams: RateLimitSeam.
  - Files: `src/lib/seams/rate-limit-seam/validators.ts`, `src/lib/seams/rate-limit-seam/fixtures.ts`, `src/lib/seams/rate-limit-seam/mock.ts`, `src/lib/seams/rate-limit-seam/test.ts`, `src/lib/adapters/rate-limit-seam/index.ts`, `docs/evidence/2026-07-06/rate-limit-codex-fixes-*`.
  - Commands: `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run rewind -- --seam RateLimitSeam`, `npm run verify`.
- Self-critique: The riskiest change is validating at construction time — if any call site ever passed a rule with `limit` as a non-integer or a computed `windowMs` that could be zero, it would now throw at module load instead of degrading gracefully; a repo-wide grep confirmed all six route call sites pass fixed, positive integer literals, so this is safe today. `npm run check/lint/test/build/rewind/verify` are all green after the fix, and the seam-scoped rewind (18 tests) covers the new validator paths directly.
- Cipher Gate:
  - Date: 2026-07-06
  - Seams: RateLimitSeam
  - Evidence: docs/evidence/2026-07-06/rate-limit-codex-fixes-check.txt; docs/evidence/2026-07-06/rate-limit-codex-fixes-lint.txt; docs/evidence/2026-07-06/rate-limit-codex-fixes-build.txt; docs/evidence/2026-07-06/rate-limit-codex-fixes-test.txt; docs/evidence/2026-07-06/rate-limit-codex-fixes-verify.txt; docs/evidence/2026-07-06/rate-limit-codex-fixes-rewind.txt
  - Summary: Added the missing `validators.ts` (Zod-validated `RateLimitRule`) and moved the mock's block-scenario error into a shared fixture, in response to Codex's review of PR #222, bringing `RateLimitSeam` in line with every other seam folder's established structure.
  - Risks: None new; the Codex finding about rate-limit-vs-validation ordering (P2) is a genuine product/security tradeoff rather than a small fix and is being raised to the user separately rather than decided unilaterally.

## 2026-07-06 - Enforce a real hard cap on RateLimitSeam's tracked-key store (Codex follow-up)

- Date: 2026-07-06
- Decision: Replace the earlier "prune-only" bound on the in-memory adapter's `Map` with a true hard cap: after an opportunistic expiry sweep fails to free room for a genuinely new key, evict the single oldest-inserted entry before adding the new one, so `store.size` can never exceed the cap regardless of traffic shape. Parameterize `createRateLimitSeam(rule, maxTrackedKeys = 10_000)` so tests can prove the eviction path deterministically with a tiny cap instead of a 10,001-iteration loop.
- Context: A second Codex review pass (against the commit that added the prune sweep) correctly pointed out that the sweep only removes keys whose entire timestamp history has expired; if more than `MAX_TRACKED_KEYS` distinct clients are all genuinely active within the same window (no expired entries for the sweep to reclaim), the store still grows past the cap on every new key, and every subsequent insert re-scans the whole map for no benefit. The earlier fix and its test only exercised the "all keys eventually expire" case, which is why this gap wasn't caught.
- Alternatives: Increase `MAX_TRACKED_KEYS` and call it acceptable; rejected because it only raises the threshold at which the same unbounded-growth problem recurs, it doesn't fix it. Add a proper LRU library dependency; rejected as unnecessary weight for a documented in-memory stop-gap — `Map` already preserves insertion order, so oldest-entry eviction is a few lines with no new dependency. Track eviction correctness only through a 10,001-iteration black-box loop; rejected because it cannot actually distinguish "the sweep ran and freed room" from "the cap was never truly enforced" (both look identical from the outside when every key eventually expires), which is exactly how the gap slipped through the first time — the fix here is a `maxTrackedKeys` constructor override precisely so the still-active-key eviction path can be tested directly and fast.
- Consequences: `store.size` is now hard-bounded by `maxTrackedKeys` (default 10,000) under every traffic pattern, including sustained high-cardinality traffic where no key ever expires; under that specific pattern, the oldest-inserted client's rate-limit history is evicted to make room, which very slightly weakens rate-limiting precision for that one client (it is treated as new again) in exchange for a real memory bound — an acceptable tradeoff given the alternative is unbounded growth.
- Revisit criteria: Revisit if production traffic data shows this eviction path firing routinely (i.e., genuinely more than 10,000 distinct concurrent clients per route per minute), at which point the documented distributed-store follow-up (Redis/Upstash/KV) becomes the real fix rather than a stop-gap.
- Plan:
  - Goal: Close the Codex-flagged gap where the prune-only sweep does not actually bound store size under sustained active-key traffic.
  - Seams: RateLimitSeam.
  - Files: `src/lib/adapters/rate-limit-seam/index.ts`, `src/lib/seams/rate-limit-seam/test.ts`, `docs/evidence/2026-07-06/rate-limit-hard-cap-fix-*`.
  - Commands: `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run rewind -- --seam RateLimitSeam`, `npm run verify`.
- Self-critique: The riskiest assumption is that oldest-inserted is an acceptable eviction order (versus e.g. least-recently-used by access, or largest-timestamp-count); insertion order was chosen because `Map` gives it for free with no extra bookkeeping, and this path only matters under a rare high-cardinality condition where exact fairness is secondary to bounding memory. The new deterministic test (tiny cap, long window, no possible expiry) exercises exactly the eviction branch the prior test could not distinguish from ordinary pruning. `npm run check/lint/test/build/rewind/verify` are all green.
- Cipher Gate:
  - Date: 2026-07-06
  - Seams: RateLimitSeam
  - Evidence: docs/evidence/2026-07-06/rate-limit-hard-cap-fix-check.txt; docs/evidence/2026-07-06/rate-limit-hard-cap-fix-lint.txt; docs/evidence/2026-07-06/rate-limit-hard-cap-fix-build.txt; docs/evidence/2026-07-06/rate-limit-hard-cap-fix-test.txt; docs/evidence/2026-07-06/rate-limit-hard-cap-fix-verify.txt; docs/evidence/2026-07-06/rate-limit-hard-cap-fix-rewind.txt
  - Summary: Added true oldest-entry eviction so the in-memory rate-limit store's size is hard-capped under all traffic patterns, not just ones where keys eventually expire; added a deterministic test (parameterized cap) that proves the eviction path directly.
  - Risks: None new; this is a stop-gap improvement to the already-documented in-memory/per-instance limitation, not a change to that limitation itself.
