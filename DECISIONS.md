...
- Consequences: `contracts/meechie-voice.contract.ts`, `src/lib/seams/meechie-voice-seam/contract.ts`, `contracts/provider-adapter.contract.ts`, `src/lib/seams/image-generation-seam/validators.ts`, and `src/lib/seams/telemetry-seam/validators.ts` now pass an explicit `z.string()` key schema to every `z.record()` call. `src/lib/adapters/spec-validation.adapter.ts` and `src/lib/adapters/spec-validation-seam/index.ts` now match on the `invalid_format` issue code (was `invalid_string`) for label-regex failures and type `formatPath`'s parameter as `ReadonlyArray<PropertyKey>` instead of `Array<string | number>`. No fixture, mock, or test expectation changed — the `SpecValidationSeam` fault fixture's `LABEL_INVALID_CHARS` output is byte-identical before and after the upgrade. A GitHub review from `chatgpt-codex-connector` correctly caught a second-order drift this same upgrade introduced: every `SpecValidationSeam` issue that falls through to the generic `SPEC_INVALID` branch (any enum field, or the `dedication` regex, since neither has a dedicated issue-code branch) was passing zod's raw `issue.message` straight through, and zod v4's default wording for enum/format failures differs from v3's — so the seam's observable output _did_ drift for those fields even though the specifically-tested `LABEL_INVALID_CHARS` fixture did not. Fixed by making the `SPEC_INVALID` fallback message version-independent: pass through `issue.message` only when `issue.code === 'custom'` (developer-authored text via `ctx.addIssue`, stable across zod versions), otherwise emit a fixed `Invalid value for <field>.` message. Added a regression test in both `tests/contract/spec-validation.test.ts` and `src/lib/seams/spec-validation-seam/test.ts` asserting an invalid `alignment` enum value produces that stable message.
...
- Plan:
  ...
  - Files: `package.json`, `package-lock.json`, `contracts/meechie-voice.contract.ts`, `src/lib/seams/meechie-voice-seam/contract.ts`, `contracts/provider-adapter.contract.ts`, `src/lib/seams/image-generation-seam/validators.ts`, `src/lib/seams/telemetry-seam/validators.ts`, `src/lib/adapters/spec-validation.adapter.ts`, `src/lib/adapters/spec-validation-seam/index.ts`, `tests/contract/spec-validation.test.ts`, `src/lib/seams/spec-validation-seam/test.ts`, `DECISIONS.md`, `LESSONS_LEARNED.md`, `CHANGELOG.md`, `docs/evidence/2026-07-07/`.
  - Commands: `npm install zod@4 --save-exact`, `npm run check`, `npm run lint`, `npm test`, `npm run build`, `npm run verify`.
- Self-critique: The riskiest assumption was that no other zod v4 breaking change (default-value semantics, coercion, `.email()`/format deprecations, custom error maps) was silently swallowed by TypeScript instead of surfacing as a compile error. Validated by running the full test suite and the production build after the bump, not just `svelte-check` — but the first pass still missed the generic `SPEC_INVALID` message-wording drift caught by automated PR review, since no fixture exercised an enum-only failure path. Closed that gap with an explicit regression test rather than relying on manual inspection alone.
