...
const SPACING_CLAUSE =
	'treat blank space as intentional; do not fill empty space.';
...
export const formatAlignmentLine = (spec: ColoringPageSpec): string => {
	...
	const numberClause =
		spec.numberAlignment === 'strict'
			? 'all numbers vertically aligned; '
			: 'numbers readable; ';
	const textClause =
		spec.alignment === 'center' ? 'text centered; ' : 'all text left-aligned; ';
