...
// Every preview placeholder the app has ever shipped, newest first, as the full
// title-plus-items shape rather than the title alone.
//
// Two failure modes bracket this. Matching against only the CURRENT placeholder
// means that the moment the placeholder changes, every older untouched draft looks
// like real generated content and the studio restores retired wording as the user's
// own. Matching on title alone fixes that but introduces the mirror error: a draft
// holding genuine generated output that happens to be titled "I DON'T ACT" gets
// classified as untouched, and the user's saved preview silently disappears.
//
// Matching the whole shape avoids both. Real generated output would have to
// reproduce a retired placeholder's title AND all of its item labels exactly.
//
// Append the outgoing entry whenever DEFAULT_STUDIO_TEXT_OUTPUT changes. Never
// remove one: each is load-bearing for someone's saved draft.
type PlaceholderShape = { title: string; items: readonly string[] };

export const PLACEHOLDER_SHAPES: readonly PlaceholderShape[] = [
	{
		title: 'NOT EVEN A LITTLE BIT',
		items: ['DO NOT OPEN THE DOOR', 'RIGHT HERE IN MY SHADOW', 'AN AREA OF CONTROL']
	},
	{
		title: "I DON'T ACT",
		items: ['RUN THE PLACE', 'DO NOT OPEN THE DOOR', 'LOWER MY VOICE']
	},
	{
		title: 'IN THIS ECONOMY',
		items: ['STAY PRETTY TOMORROW', 'CLOSE THE DOOR', 'LET THE DRAFT WORK']
	}
];

/** True when a draft's spec is an untouched placeholder the user never generated into. */
export const isUntouchedPlaceholder = (intent: {
	title: string;
	// Matches the real item shape (ColoringPageItem / MeechieStudioPageItem) so both a
	// draft spec and a studio text output can be passed without widening at the call site.
	items: readonly { number: number; label: string }[];
}): boolean =>
	PLACEHOLDER_SHAPES.some(
		(shape) =>
			shape.title === intent.title &&
			shape.items.length === intent.items.length &&
			shape.items.every((label, index) => label === intent.items[index]?.label)
	);
