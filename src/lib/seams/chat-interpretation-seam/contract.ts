/*
 * Purpose: Canonical ChatInterpretationSeam contract definition and schemas.
 * Why: Map free-text intent to a structured ColoringPageSpec.
 * Info flow: User text -> structured spec -> validation.
 * Invariants: Message must be non-empty string; spec must conform to ColoringPageSpecSchema.
 */
import { z } from 'zod';
...
export type ChatInterpretationResult = z.infer<typeof ChatInterpretationResultSchema>;
