...
export const createMockCacheSeam = (
	scenario: CacheScenario = 'sample'
): CacheSeam => {
	...
	return {
		primeCache: async (
			...
			try {
				...
				return {
					ok: false,
					error: validationError('CACHE_ADD_ALL_FAILED', err)
				};
			}
			if (scenario === 'fault')
				return { ok: false, error: cacheOpenFailedFixture };
			if (scenario === 'addAllFault')
				return { ok: false, error: cacheAddAllFailedFixture };
		...
		evictStaleCaches: async (
			...
			if (scenario === 'fault')
				return { ok: false, error: cacheEvictFailedFixture };
		...
		matchRequest: async (
			...
			if (scenario === 'fault')
				return { ok: false, error: cacheMatchFailedFixture };
