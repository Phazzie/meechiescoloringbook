...
import { invalidCheckInputFixtures, rateLimitFaultFixture, sampleCheckInput, sampleKey } from './fixtures';
...
describe('RateLimitSeam mock fault scenario', () => {
	it('returns the fault fixture regardless of input and records no state', () => {
		const seam = createMockRateLimitSeam('fault');

		const result = seam.checkAndConsume(sampleCheckInput);

		expect(result).toEqual({ ok: false, error: rateLimitFaultFixture });
	});

	it('keeps returning the fault fixture across repeated calls', () => {
		const seam = createMockRateLimitSeam('fault');

		seam.checkAndConsume(sampleCheckInput);
		const second = seam.checkAndConsume({ ...sampleCheckInput, key: 'a-different-key' });

		expect(second).toEqual({ ok: false, error: rateLimitFaultFixture });
	});
});
