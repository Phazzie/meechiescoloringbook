...
import type {
	Wig,
	WigCatalogSeam
} from '../../seams/wig-catalog-seam/contract';
	...
	> => {
		...
		try {
			...
			if (wigs.length === 0) {
				return {
					ok: false,
					error: { code: 'WIG_CATALOG_EMPTY', message: 'Wig catalog is empty.' }
				};
			}
			cachedWigs = wigs;
			...
			return {
				ok: false,
				error: {
					code: 'WIG_CATALOG_LOAD_FAILED',
					message: 'Failed to parse wig catalog data.'
				}
			};
		}
	};
		...
		getWigById: async (id: string) => {
			...
			if (!wig) {
				return {
					...
					error: {
						code: 'WIG_NOT_FOUND',
						message: `No wig found with id: ${id}`,
						details: { id }
					}
				};
			}
