/*
 * Purpose: Canonical ProviderAdapterSeam adapter implementation.
 * Why: Perform authenticated xAI chat/image requests behind a single boundary with socket cancellation and error redaction.
 * Info flow: Provider input -> xAI HTTP -> normalized output.
 * Invariants: Single attempt for chat POST; signal forwarded for immediate cancellation; provider error secrets redacted; returns Result envelope.
 */
import type {
	ProviderAdapterSeam,
	ProviderChatInput,
	...
	ProviderImageInput,
	ProviderImageOutput
} from '../../seams/provider-adapter-seam/contract';
	...
	createChatCompletion: async (
		...
		try {
			...
			const response = await fetchWithRetry(
				...
				{ ...CHAT_RETRY_OPTIONS, signal: input.signal }
			);
			...
			if (isAbortError(error) && input.signal?.aborted) {
				return {
					ok: false,
					error: buildError('ABORT_ERROR', 'Operation aborted by caller.')
				};
			}
			return {
				ok: false,
