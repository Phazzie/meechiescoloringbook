...
## Ship the Stalled PR-Drain Integration Branch Instead of Adding a Duplicate Fix (2026-06-20)

### Shortcut Check

1. Shortcut a typical AI might take: ignore the existing 42-commit integration branch already sitting on `claude/sweet-mendel-p5s5bt`, and write yet another isolated fix (e.g. a zod v3→v4 upgrade) on top of it, producing a 27th open PR in a repo that already has 26 unmerged PRs and 0 merges in 12+ days.
2. Countermeasure: before writing any new code, diff this branch against `origin/main` (`git rev-list --left-right --count main...claude/sweet-mendel-p5s5bt`, `git log --oneline main..HEAD`) and confirm whether prior sessions already produced mergeable work that simply never got opened as a PR.
3. Lower-debt path: re-verify the existing branch is still green, document the decision, and open the PR for the already-completed work rather than compounding the backlog problem the repo owner flagged in Issue #175.

### Plan

- Goal: Confirm `claude/sweet-mendel-p5s5bt` (ahead of `main` by 42 commits / 467 files / ~43.9k lines, 0 commits behind, no existing PR) is still green, record the decision, and open it as a single consolidated PR instead of adding new duplicate work.
- Exact seams: none changed by this action (no contract/adapter/mock edits); this is a shipping/process decision, not a seam change.
- Exact file paths touched:
  - `plan.md` (this entry)
  - `DECISIONS.md`
  - `LESSONS_LEARNED.md`
  - `docs/evidence/2026-06-20/` (chamber-lock.json, verify.txt, test.txt, seam-ledger.{json,md}, clan-chain.{json,md}, proof-tape.{json,md}, shaolin-lint.json, assumption-alarm.json — generated by `npm run verify`)
- Exact commands run:
  1. `git rev-parse main claude/sweet-mendel-p5s5bt origin/claude/sweet-mendel-p5s5bt`
  2. `git rev-list --left-right --count main...claude/sweet-mendel-p5s5bt`
  3. `git log --oneline main..claude/sweet-mendel-p5s5bt`
  4. `git diff main..claude/sweet-mendel-p5s5bt --stat`
  5. `npm run verify`
  6. `npm run build`
  7. `npm run lint`
  8. `git diff --check origin/main`
...
1. What could be wrong: the branch could contain stale or superseded work that conflicts with reviewer intent on other still-open PRs, or could have been abandoned deliberately rather than by accident.
...
4. Evidence to prove/disprove: `docs/evidence/2026-06-20/verify.txt` (svelte-check 0 errors/0 warnings, 519 tests passed/1 skipped), fresh `npm run build` success, clean `npm run lint`, and `docs/evidence/2026-06-20/clan-chain.md` showing 0 dirty seams.
