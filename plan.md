...
## RateLimitSeam: Abuse Protection for Billable /api/* POST Routes (2026-06-19)

### Shortcut Check

1. Shortcut a typical AI might take: bolt a quick request counter directly into each of the six `+server.ts` route handlers, or skip the seam workflow entirely since "it's just rate limiting."
2. Countermeasure: treat the limiter as a real seam (clock/time is an explicit seam trigger per `AGENTS.md`) with contract/fixtures/mock/probe/test/adapter, and enforce it from one `Handle` hook so no route handler needs to change.
3. Lower-debt path: keep the algorithm pure and clock-injected (no `Date.now()` inside the seam) so contract tests are deterministic, and let the adapter be the only piece that reads env.

### Plan

- Goal: Audit the repo for its ten hardest outstanding problems, then implement the single highest-priority one: billable AI endpoints (`generate`, `image-generation`, `chat-interpretation`, `tools`, `wig-try-on`, `meechie-studio-text`) had no rate limiting or abuse protection of any kind.
- Exact seams: RateLimitSeam (new, self-contained layout).
- Exact file paths to touch:
  ...
  - `src/lib/adapters/rate-limit-seam/index.ts`
  - `src/hooks.server.ts`
  - `tests/unit/rate-limit-seam-adapter.test.ts`
  - `tests/unit/hooks-server-rate-limit.test.ts`
  - `.env.example`
  - `docs/seams.md`, `src/lib/seams/CLAUDE.md`, `CHANGELOG.md`, `DECISIONS.md`
  - `docs/evidence/2026-06-19/*` generated by verification commands.
- Exact commands to run:
  1. `npx vitest run src/lib/seams/rate-limit-seam/test.ts tests/unit/rate-limit-seam-adapter.test.ts tests/unit/hooks-server-rate-limit.test.ts`
  2. `npm run check`
  3. `npm run lint`
  4. `npm test`
  5. `npm run build`
  6. `npm run verify`
  7. `npm run rewind -- --seam RateLimitSeam`
  8. `git diff --check`

### Self-critique

1. What could be wrong: keying the limiter by `event.getClientAddress()` is a volume guardrail, not authentication — clients behind a shared NAT/proxy share a bucket, and the in-memory Map does not survive cold starts or fan out across concurrent serverless instances.
2. What must be proven: the fixed-window algorithm allows exactly `maxRequests` per window, denies the next one with a positive `retryAfterMs`, resets cleanly once the window elapses, and tracks independent keys independently — all proven by the contract test plus the hooks-level test exercising the real `Handle` function with 25 simulated requests.
3. Riskiest assumption: defaulting to 20 requests/60s is a reasonable starting guardrail for a small app; it is fully operator-tunable via `RATE_LIMIT_MAX_REQUESTS`/`RATE_LIMIT_WINDOW_MS` without a code change.
4. Evidence to prove/disprove: focused seam/adapter/hooks tests, Svelte check, lint, full test suite, build, full verify chain, single-seam rewind, and diff-check — all captured under `docs/evidence/2026-06-19/`.
