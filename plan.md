...
## Lazy-load pdf-lib in OutputPackagingSeam (2026-06-25)

### Shortcut Check

1. Shortcut a typical AI might take: claim a bundle-size win just because an `import` statement moved, without verifying the bundler's chunk graph actually changed.
2. Countermeasure: build before and after the change (`git stash`/`git stash pop` around a clean `.svelte-kit/output` rebuild), grep the output chunks for a literal symbol from the dependency (`PDFDocument`), and inspect `.vite/manifest.json`'s `dynamicImports` field to prove the dependency physically moved to its own on-demand chunk.
3. Lower-debt path: keep the fix scoped to a single `await import('pdf-lib')` inside the one branch of `OutputPackagingSeam.package()` that actually needs it, with no contract, mock, or fixture changes.

### Plan

- Goal: Stop shipping the ~177KB-gzip `pdf-lib` dependency to every visitor of the builder routes when only PDF-exporting users need it.
- Exact seams: OutputPackagingSeam.
- Exact file paths to touch:
  - `plan.md`
  - `DECISIONS.md`
  - `LESSONS_LEARNED.md`
  - `src/lib/adapters/output-packaging.adapter.ts`
  - `docs/evidence/2026-06-25/*` generated by verification commands.
- Exact commands to run:
  1. `npm run check`
  2. `npm run lint`
  3. `npx vitest run tests/contract/output-packaging.test.ts tests/unit/output-packaging-helpers.test.ts`
  4. `npm test`
  5. `rm -rf .svelte-kit/output && npm run build` (once before the change via `git stash`, once after via `git stash pop`)
  6. `npm run verify`
  7. `npm run cipher:gate`
  8. `git diff --check`

### Self-critique

1. What could be wrong: The dynamic import could fail in a non-browser/non-ESM runtime, or Rollup could silently re-inline pdf-lib into the same eager chunk, making the "fix" a no-op.
2. What must be proven: `OutputPackagingSeam.package()` still returns identical `Result<OutputPackagingOutput>` values for every format/variant combination, and the build's chunk graph shows pdf-lib split into a separate, on-demand chunk.
3. Riskiest assumption: No call site needs `PDFDocument` synchronously at module-init time; verified by reading every other usage in the adapter (none) and confirming the contract signature was already `Promise`-returning.
4. Evidence to prove/disprove: Focused and full test runs, Svelte check, lint, before/after build manifest comparison, full verify, Cipher Gate, and diff-check output.
