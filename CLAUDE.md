...
| `draft-restore.ts` | Pure decisions about what an autosaved draft's stored mode means when the studio reopens, and the sentence the reader is shown about it. `restoreDraftMode` resolves the stored `modeId` against the live catalogue into `recorded` / `unrecorded` (a draft written before `DraftRecordSchema` had the field) / `retired` (a mode this build no longer has) — three states rather than a boolean, because "this draft is old" and "your question was removed" are different facts and only the second is unrecoverable. The schema deliberately does **not** enumerate the mode ids: the catalogue is application data that changes, and an enum would turn a rename into an unparseable draft. `describeDraftRestore` produces `caution: string \| null`, which is the one thing `StudioInputPanel` branches on, so the panel cannot put a warning tone on a restore that went right |
...
| `studio/StudioInputPanel.svelte` | The evidence box, the AI action buttons, and the restored-draft notice that sits above them. It renders `draftRestoreNotice` and decides nothing about it — whether the studio is showing the reader's own question is settled in `src/lib/core/draft-restore.ts` |
