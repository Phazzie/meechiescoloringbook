...
| `quality-report.ts` | Pure quality-report transforms — turns the drift violations, their severity, the recommended fixes and the spec-validation issues into what the reader is told about the page. Owns `DRIFT_CHECK_FAILED_CODE`, the one definition of the code the generate pipeline emits when the drift check cannot return a verdict, so an unrunnable check is never reported as a clean page. Keeps "checked and clean" and "never checked" as distinct states, and keeps violations and fixes as the two independent lists the contract declares rather than pairing them |
