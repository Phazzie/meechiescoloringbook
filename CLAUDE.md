...
| `storage-failure.ts` | The one place that decides what a reader is told when the app's own **memory** fails — the Quote Vault and the studio draft — and whether trying again is worth offering. The storage twin of `generation-failure.ts`, written because that module abolished the raw error string for AI calls and left storage doing it at twelve call sites, four of them writing a **caught exception's** message. Same invariant: an exception's words are never the reader's sentence, they become `detail`. Same allowlist rule as `public-provider-error.ts`, applied to the three capacity refusals in `vault-capacity.ts` — those come back **verbatim**, because `vaultLinkFor` matches them character-for-character to decide whether the status line offers "Make room in the vault", so rewording one silently removes the link. Where it differs from the AI classifier is the retry: storage retries spend no quota, reach no provider and need no network, so there is no `after` variant and no wait to describe — just the causes where a second press could land differently (`write_failed`, `unknown`) and the ones where it provably could not. The cause is not enough to write the sentence, so the operation is an input: a failed read threatens nothing the reader is holding, a failed save threatens the page on screen |
...
| `VaultStatusLine.svelte` | The **only** rendering of what a surface says after a save, on all four hosts, with the link to `/vault` beside it when — and only when — the save actually succeeded. The decision is `showsVaultLink` in core. Also carries the save's retry, which is why the refusals must reach it **unreworded**: the same string decides the link and the sentence |
...
| `StorageFailureNotice.svelte` | The **only** rendering of a failed storage operation in the app — the vault card, `/vault`, and the studio's draft. Deliberately the same shape as `GenerationFailureNotice` so a reader does not learn two visual languages for "this did not work", and owning its own CSS for the reason every shared piece in this app carries its own: the markup was always copyable and the styling was not. Never renders `failure.detail` |
...
| `describe-page-state.svelte.ts` | `DescribePageState`: the message -> `/api/chat-interpretation` -> read-back -> page lifecycle behind `/describe`. `interpretedFrom` pins the read-back to the words that produced it — captured *before* the await, because the box stays editable while the request is in flight and reading it on arrival reintroduces the exact drift the field exists to stop. `interpretToken` is separate from `pageToken`: `reset()` abandons an interpretation, `resetPage()` abandons only the page. A **failed** interpretation never destroys a page already paid for; a **successful** one does, because that page belongs to the interpretation it replaces |
